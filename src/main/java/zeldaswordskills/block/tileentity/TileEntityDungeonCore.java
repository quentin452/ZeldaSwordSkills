...
public class TileEntityDungeonCore extends TileEntityDungeonBlock {
    ...
    public void setSpawner() {
        if (box == null) {
            ...
            setDungeonBoundingBox(
                new StructureBoundingBox(xCoord - 3, yCoord, zCoord - 3, xCoord + 3, yCoord + 4, zCoord + 3));
        }
    ...
    public void updateEntity() {
        ...
        if (isBossRoom && bossBattle == null) {
            if (box == null) {
                ZSSMain.logger.warn(
                    String.format(
                        "Boss room at %d/%d/%d missing structure bounding box - dungeon is being disabled",
                        xCoord,
                        yCoord,
                        zCoord));
                ...
                EntityPlayer closestPlayer = worldObj.getClosestPlayer(
                    ...
                    yCoord + 0.5D,
                    zCoord + 0.5D,
                    (double) (box.getXSize() - 2) / 2.0D);
                if (closestPlayer != null && box.isVecInside(
                    MathHelper.floor_double(closestPlayer.posX),
                    MathHelper.floor_double(closestPlayer.posY),
                    MathHelper.floor_double(closestPlayer.posZ))) {
        ...
        if (bossBattle != null) {
            ...
            if (!alreadyVerified && box != null && !verifyStructure(false)) {
                ...
                if (isBossRoom) {
                    ...
                    worldObj
                        .playSoundEffect(xCoord + 0.5D, yCoord + 1, zCoord + 0.5D, Sounds.SECRET_MEDLEY, 1.0F, 1.0F);
                    removeCoreBlock();
                }
            } else if (isSpawner()) {
        ...
        if (block.getMaterial() == Material.water) {
            ...
            if (mob == null) {
                switch (worldObj.rand.nextInt(10)) {
                    case 0:
                        mob = new EntityKeeseCursed(worldObj);
                        break;
                    case 1:
                        mob = new EntityKeeseThunder(worldObj);
                        break;
                    case 2: // fall-through
                    case 3:
                        mob = new EntityKeeseIce(worldObj);
                        break;
                    case 4: // fall-through
                    case 5:
                        mob = new EntityKeeseFire(worldObj);
                        break;
                    default:
                        mob = new EntityKeese(worldObj);
                        break;
                }
            }
            ...
            if (mob == null) {
                switch (worldObj.rand.nextInt(8)) {
                    case 0:
                        mob = new EntityChuBlue(worldObj);
                        break;
                    case 1:
                        mob = new EntityChuYellow(worldObj);
                        break;
                    case 2: // fall-through
                    case 3:
                        mob = new EntityChuGreen(worldObj);
                        break;
                    default:
                        mob = new EntityChuRed(worldObj);
                        break;
                }
            }
        } else if (rarity > -2) {
            ...
            mob = EntityWizzrobe
                .getRandomWizzrobeForLocation(this.worldObj, 0.5F, this.xCoord, this.yCoord, this.zCoord);
            // Otherwise just generate a random one
            if (mob == null) {
                switch (worldObj.rand.nextInt(4)) {
                    case 0:
                        mob = new EntityWizzrobeFire(worldObj);
                        break;
                    case 1:
                        mob = new EntityWizzrobeGale(worldObj);
                        break;
                    case 2:
                        mob = new EntityWizzrobeIce(worldObj);
                        break;
                    default:
                        mob = new EntityWizzrobeThunder(worldObj);
                        break;
                }
            }
    ...
    private void clearDungeon() {
        ...
        placeOpenDoor(
            (worldObj.getBlock(x, box.minY + 1, z)
                .getMaterial()
                .isLiquid() ? 2 : 1));
        // Place warp stone
    ...
    private boolean verifyDoor() {
        ...
        switch (doorSide) {
            ...
            default:
                ZSSMain.logger.warn(
                    String.format(
                        "Verifying door in Dungeon Core with invalid door side at %d/%d/%d",
                        xCoord,
                        yCoord,
                        zCoord));
        }
        for (int y = box.minY; y < box.maxY; ++y) {
            if (worldObj.getBlock(x, y, z) == door) {
                return (door instanceof IDungeonBlock)
                    ? ((IDungeonBlock) door).isSameVariant(worldObj, x, y, z, doorMeta)
                    : true;
            }
        }
        return false;
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        if (isBossRoom) {
            if (tag.hasKey("dungeonName")) {
                ...
                ZSSMain.logger.warn(
                    String.format(
                        "Detected old boss dungeon save format at %d/%d/%d - if you still see this message after saving and reloading near this location, please contact the mod author",
                        xCoord,
                        yCoord,
                        zCoord));
            ...
            if (dungeonType == null) {
                ZSSMain.logger.error(
                    String.format(
                        "Error retrieving Boss Type from string %s while loading Dungeon Core from NBT at %d/%d/%d",
                        tag.getString("dungeonName"),
                        xCoord,
                        yCoord,
                        zCoord));
                ...
                if (bossBattle != null) {
                    ...
                    ZSSMain.logger.warn(
                        String.format(
                            "Error retrieving Boss Battle while loading Dungeon Core from NBT at %d/%d/%d - returned NULL",
                            xCoord,
                            yCoord,
                            zCoord));
                }
        ...
        if (tag.getBoolean("isSpawner")) {
            ZSSMain.logger.warn(
                String.format(
                    "Detected old fairy spawner save format at %d/%d/%d - if you still see this message after saving and reloading near this location, please contact the mod author",
                    xCoord,
                    yCoord,
                    zCoord));
