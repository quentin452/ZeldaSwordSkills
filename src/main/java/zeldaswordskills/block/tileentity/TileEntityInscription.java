...
public class TileEntityInscription extends TileEntity {
    ...
    public void setSong(AbstractZeldaSong song) {
        if (song != null && !song.canLearnFromInscription(worldObj, xCoord, yCoord, zCoord, blockType, blockMetadata)) {
            ZSSMain.logger.warn(
                String.format(
                    "%s cannot be learned from inscriptions; coordinates: %d/%d/%d",
                    song.getDisplayName(),
                    xCoord,
                    yCoord,
                    zCoord));
    ...
    public boolean onActivated(EntityPlayer player) {
        if (song == null) {
            ZSSMain.logger.warn(
                String.format("TileEntityInscription at %d/%d/%d does not have a valid song!", xCoord, yCoord, zCoord));
            return false;
        }
            ...
            if (player.isSneaking()) {
                if (!worldObj.isRemote) {
                    ...
                    PlayerUtils.sendTranslatedChat(
                        player,
                        "chat.zss.song.inscription.new",
                        new ChatComponentTranslation(song.getTranslationString()));
                }
            } else if (!worldObj.isRemote) {
                PlayerUtils.sendTranslatedChat(
                    player,
                    "chat.zss.song.inscription.current",
                    new ChatComponentTranslation(song.getTranslationString()));
            }
            return true;
