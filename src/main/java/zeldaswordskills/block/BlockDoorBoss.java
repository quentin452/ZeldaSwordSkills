...
public class BlockDoorBoss extends BlockDoorLocked {
    ...
    protected boolean canUnlock(EntityPlayer player, int meta) {
        return PlayerUtils.consumeHeldItem(player, ZSSItems.keyBig, meta & 0x7, 1)
            || PlayerUtils.consumeHeldItem(player, ZSSItems.keySkeleton, 0, 1);
    }
    ...
    public IIcon getIcon(int side, int meta) {
        ...
        return (meta > 0x7 ? (side == 1 ? iconsTop[meta % 8] : iconsUpper[meta % 8])
            : (side == 0 ? iconsTop[meta % 8] : iconsLower[meta % 8]));
    }
