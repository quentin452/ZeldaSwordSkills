...
public class BlockWarpStone extends Block implements ILiftable, ISmashable {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (stack != null && stack.getItem() instanceof ItemInstrument) {
            ...
            if (!world.isRemote) {
                if (song != null) {// && songs.isSongKnown(song)) { // otherwise have to click again after learning the
                                   // song
                ...
                if (song != null) {
                    ...
                    ZSSMain.logger.warn(
                        String.format("Warp stone at %d/%d/%d had invalid metadata: did not return a song!", x, y, z));
                }
            }
            return true;
            ...
            if (song != null && !world.isRemote) {
                ...
                PlayerUtils.sendTranslatedChat(
                    player,
                    "chat.zss.block.warp_stone.set_default",
                    new ChatComponentTranslation(song.getTranslationString()),
                    warp.toString());
                if (previous != null) {
                    PlayerUtils.sendTranslatedChat(
                        player,
                        "chat.zss.block.warp_stone.previous",
                        new ChatComponentTranslation(song.getTranslationString()),
                        previous.toString());
                }
            }
