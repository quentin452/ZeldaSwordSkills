...
public class BlockGossipStone extends Block implements IHookable, ILiftable, ISmashable, ISongBlock {
    ...
    public void onHeldBlockPlaced(World world, ItemStack stack, int x, int y, int z, int meta) {
        ...
        TileEntity te = world.getTileEntity(x, y, z);
        if (te instanceof TileEntityGossipStone) {
            String msg = stack.getTagCompound()
                .getString("TegsMessage");
    ...
    @Override
    public boolean onSongPlayed(World world, int x, int y, int z, EntityPlayer player, AbstractZeldaSong song,
        int power, int affected) {
        TileEntity te = world.getTileEntity(x, y, z);
        return (te instanceof TileEntityGossipStone
            && ((TileEntityGossipStone) te).onSongPlayed(player, song, power, affected));
    }
    ...
    public void onBlockClicked(World world, int x, int y, int z, EntityPlayer player) {
        ...
        if (PlayerUtils.isSword(player.getHeldItem())) {
            ...
            PlayerUtils.sendTranslatedChat(
                player,
                "chat.zss.block.gossip_stone.time",
                String.format("%02d", h),
                String.format("%02d", m),
                days);
        } else if (world.getBlockMetadata(x, y, z) > 0 && player.getHeldItem() != null
            && player.getHeldItem()
                .getItem() instanceof ItemPickaxe) {
                    world.playSoundAtEntity(
                        ...
                        Sounds.ITEM_BREAK,
                        0.25F,
                        1.0F / (world.rand.nextFloat() * 0.4F + 0.5F));
                }
    }
