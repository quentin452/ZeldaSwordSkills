...
public enum RotationAxis {
    ...
    public void addRotation(ModelRenderer part, float amount) {
        switch (this) {
            case X:
                part.rotateAngleX += amount;
                break;
            case Y:
                part.rotateAngleY += amount;
                break;
            case Z:
                part.rotateAngleZ += amount;
                break;
        }
    }
    ...
    public float getRotationPoint(ModelRenderer part) {
        switch (this) {
            case X:
                return part.rotationPointX;
            case Y:
                return part.rotationPointY;
            case Z:
                return part.rotationPointZ;
            default:
                return 0.0F;
        }
    }
    ...
    public void addRotationPoint(ModelRenderer part, float amount) {
        switch (this) {
            case X:
                part.rotationPointX += amount;
                break;
            case Y:
                part.rotationPointY += amount;
                break;
            case Z:
                part.rotationPointZ += amount;
                break;
        }
    }

    /**
     * Returns the current offset value (i.e. box position relative to the rotation point) for the model part on this
     * axis
     */
    ...
    /**
     * Adds the amount to the current offset value (i.e. box position relative to the rotation point) of the model part
     * on this axis
     */
