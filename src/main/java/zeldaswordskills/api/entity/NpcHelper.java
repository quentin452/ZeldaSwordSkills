...
public class NpcHelper {
    ...
    public static void addVillagerConversion(String name, Class<? extends EntityLiving> npcClass) {
        if (!npcClass.isAssignableFrom(INpcVillager.class)) {
            throw new IllegalArgumentException(
                "Entity class must implement INpcVillager to add a villager conversion mapping");
        } else if (nameToClassMap.containsKey(name)) {
            ZSSMain.logger.warn(
                "Failed to add villager conversion for " + name
        ...
        for (String match : nameToClassMap.keySet()) {
            if (match.equals(name)) {
                ...
                if (npc instanceof INpcVillager) {
                    npc.setLocationAndAngles(
                        villager.posX,
                        villager.posY + 0.2F,
                        villager.posZ,
                        villager.rotationYaw,
                        villager.rotationPitch);
                    result = (rightClick ? ((INpcVillager) npc).canInteractConvert(player, villager)
                        : ((INpcVillager) npc).canLeftClickConvert(player, villager));
    ...
    private static EntityLiving getNpcForName(String name, World world) {
        ...
        try {
            npc = (EntityLiving) nameToClassMap.get(name)
