...
public class EntityAIDynamicRangedAction<T extends EntityCreature & IEntityDynamic> extends EntityAIDynamicAction<T> {
    ...
    public EntityAIDynamicRangedAction(T entity, EntityAction action, float rangeMin, float rangeMax, int minDelay,
        int maxDelay, float speed, boolean require_ground) {
        this(entity, action, rangeMin, rangeMax, minDelay, maxDelay, speed, require_ground, true);
    }

    /**
     * See {@link EntityAIDynamicAction#EntityAIDynamicAction(EntityCreature, EntityAction, float, boolean, boolean)
     * EntityAIDynamicAction}
     * 
     ...
     */
    public EntityAIDynamicRangedAction(T entity, EntityAction action, float rangeMin, float rangeMax, int minDelay,
        int maxDelay, float speed, boolean require_ground, boolean require_sight) {
    ...
    protected boolean canPerformAction() {
        if (this.target == null || !this.target.isEntityAlive()) {
            ...
            return false;
        } else if (this.require_sight && !this.actor.getEntitySenses()
            .canSee(this.target)) {
                return false;
    ...
    public boolean continueExecuting() {
        return super.continueExecuting() || !this.actor.getNavigator()
            .noPath();
    }
    ...
    public boolean shouldExecute() {
        if (super.shouldExecute()) {
            if (this.target == null) {
                ...
                float scale = (this.difficultScaled
                    ? 2.0F - 0.5F * this.actor.worldObj.difficultySetting.getDifficultyId()
                    : 1.0F);
    ...
    protected boolean moveIntoPosition() {
        ...
        boolean flag = this.actor.getEntitySenses()
            .canSee(this.target);
        if (flag) {
        ...
        if (distance <= (double) this.rangeSq && this.visibilityTimer >= 20) {
            this.actor.getNavigator()
                .clearPathEntity();
            return false;
        } else if (this.speed > 0.0F) {
            this.actor.getNavigator()
                .tryMoveToEntityLiving(this.target, this.speed);
        }
        this.actor.getLookHelper()
            .setLookPositionWithEntity(this.target, 30.0F, 30.0F);
