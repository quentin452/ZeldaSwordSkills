...
public class WeaponRegistry {
        ...
        for (String s : names) {
            ...
            if (parts != null) {
                ...
                if (item == null) {
                    ZSSMain.logger.warn(
                        String.format(
                            "[WeaponRegistry] [%s] %s:%s could not be found - the mod may not be installed, or you may have typed it incorrectly",
                            origin,
                            parts[0],
                            parts[1]));
                } else if (isSword) {
    ...
    public boolean registerSword(String origin, String modid, Item item) {
        ...
        if (weapons.contains(item)) {
            ZSSMain.logger.warn(
                String.format(
                    "[WeaponRegistry] [%s] CONFLICT: %s:%s cannot be registered as a sword - it is already registered as a non-sword weapon",
                    origin,
                    modid,
            ...
            ZSSMain.logger.info(
                String.format(
                    "[WeaponRegistry] [%s] Registered %s:%s as a sword",
                    origin,
                    modid,
                    item.getUnlocalizedName()
                        .replace("item.", "")));
            ...
            ZSSMain.logger.warn(
                String.format(
                    "[WeaponRegistry] [%s] %s:%s has already been registered as a sword",
                    origin,
                    modid,
                    item.getUnlocalizedName()
                        .replace("item.", "")));
    ...
    /**
     * Registers an item as a generic melee weapon: it may be used to activate all skills that do not specifically
     * require a sword.
    ...
    public boolean registerWeapon(String origin, String modid, Item item) {
        ...
        if (swords.contains(item)) {
            ZSSMain.logger.warn(
                String.format(
                    "[WeaponRegistry] [%s] CONFLICT: %s:%s cannot be registered as a weapon - it is already registered as a sword",
                    origin,
                    modid,
                    item.getUnlocalizedName()
                        .replace("item.", "")));
            ...
            ZSSMain.logger.info(
                String.format(
                    "[WeaponRegistry] [%s] Registered %s:%s as a non-sword weapon",
                    origin,
                    modid,
                    item.getUnlocalizedName()
                        .replace("item.", "")));
            ...
            ZSSMain.logger.warn(
                String.format(
                    "[WeaponRegistry] [%s] %s:%s has already been registered as a weapon",
                    origin,
                    modid,
                    item.getUnlocalizedName()
                        .replace("item.", "")));
    ...
    public boolean removeSword(String origin, String modid, Item item) {
        ...
        if (forbidden_swords.add(item)) {
            ...
            ZSSMain.logger.info(
                String.format(
                    "[WeaponRegistry] [%s] %s:%s added to FORBIDDEN swords list",
                    origin,
                    modid,
                    item.getUnlocalizedName()
                        .replace("item.", "")));
        ...
        if (swords.remove(item)) {
            ZSSMain.logger.info(
                String.format(
                    "[WeaponRegistry] [%s] Removed %s:%s from list of registered SWORDS",
                    origin,
                    modid,
                    item.getUnlocalizedName()
                        .replace("item.", "")));
            ...
            ZSSMain.logger.warn(
                String.format(
                    "[WeaponRegistry] [%s] Could not remove %s:%s - it was not registered as a sword",
                    origin,
                    modid,
                    item.getUnlocalizedName()
                        .replace("item.", "")));
    ...
    public boolean removeWeapon(String origin, String modid, Item item) {
        ...
        if (forbidden_weapons.add(item)) {
            ...
            ZSSMain.logger.info(
                String.format(
                    "[WeaponRegistry] [%s] %s:%s added to FORBIDDEN weapons list",
                    origin,
                    modid,
                    item.getUnlocalizedName()
                        .replace("item.", "")));
        ...
        if (weapons.remove(item)) {
            ZSSMain.logger.info(
                String.format(
                    "[WeaponRegistry] [%s] Removed %s:%s from list of registered WEAPONS",
                    origin,
                    modid,
                    item.getUnlocalizedName()
                        .replace("item.", "")));
            ...
            ZSSMain.logger.warn(
                String.format(
                    "[WeaponRegistry] [%s] Could not remove %s:%s - it was not registered as a non-sword weapon",
                    origin,
                    modid,
                    item.getUnlocalizedName()
                        .replace("item.", "")));
    ...
    public static String[] parseString(String itemid) {
        ...
        ZSSMain.logger.error(
            String.format(
                "[WeaponRegistry] String must be in the format 'modid:registered_item_name', received: %s",
                itemid));
        return null;
    }
}
