...
public class ItemInstrument extends Item implements IRightClickEntity {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (world.isRemote) {
            if (isScarecrowAt(world, x, y, z) && ZSSPlayerSongs.get(player)
                .canOpenScarecrowGui(true)) {
    ...
    private boolean handleRightClickEntity(ItemStack stack, EntityPlayer player, EntityLiving entity) {
        ...
        if (entity instanceof ISongTeacher) {
            ...
            if (!player.isSneaking() && entity.hasCustomNameTag() && teachersForClass.containsKey(entity.getClass())) {
        ...
        if (result != null) {
            if (player.worldObj.isRemote && result.songToLearn != null) {
                ...
                if (!result.displayChat) {
                    ...
                    .isSongKnown(result.songToLearn)) {
                        PlayerUtils.sendTranslatedChat(
                            player,
                            "chat.zss.npc.ocarina.review",
                            new ChatComponentTranslation(result.songToLearn.getTranslationString()));
                    } else {
                        PlayerUtils.sendTranslatedChat(
                            player,
                            "chat.zss.npc.ocarina.learn",
                            new ChatComponentTranslation(result.songToLearn.getTranslationString()));
                    }
            }
    ...
    public String getUnlocalizedName(ItemStack stack) {
        return super.getUnlocalizedName() + "."
            + Instrument.values()[stack.getItemDamage() % Instrument.values().length].unlocalizedName;
    }
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4) {
        list.add(
            StatCollector
                .translateToLocal("tooltip.zss.instrument." + getInstrument(stack).getUnlocalizedName() + ".desc"));
    }
