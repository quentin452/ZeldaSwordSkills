...
public class ItemDungeonBlock extends ItemMetadataBlock {
    ...
    public Block getBlockFromStack(ItemStack stack) {
        if (stack.hasTagCompound() && stack.getTagCompound()
            .hasKey("renderBlock")) {
            int blockID = stack.getTagCompound()
                .getInteger("renderBlock");
    ...
    public IIcon getIcon(ItemStack stack, int pass) {
        ...
        return (block != null && block != this.field_150939_a ? block.getIcon(1, getMetaFromStack(stack))
            : Blocks.stone.getIcon(1, 0));
    }
            ...
            if (!world.isRemote) {
                ...
                if (block != null && (block.isOpaqueCube() || block == Blocks.ice)) {
                    ...
                    stack.getTagCompound()
                        .setInteger("renderBlock", Block.getIdFromBlock(block));
                    stack.getTagCompound()
                        .setInteger("metadata", meta);
                }
            }
            return false;
        } else {
        ...
        if (stack.getItemDamage() > 7) {
            ...
            list.add(
                StatCollector.translateToLocal(
                    "tooltip.zss.block." + (field_150939_a == ZSSBlocks.dungeonCore ? "core" : "dungeon") + ".desc.0"));
        }
