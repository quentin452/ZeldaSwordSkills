...
public class ItemBomb extends Item implements IHandlePickup, IHandleToss, IUnenchantable {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (!world.isRemote) {
            ...
            fuseTime -= (stack.hasTagCompound() ? stack.getTagCompound()
                .getInteger("time") : 0);
            world.spawnEntityInWorld(
                new EntityBomb(world, player).setType(getType(stack))
                    .addTime(fuseTime));
        }
    ...
    public void tickBomb(ItemStack stack, World world, Entity entity, int slot) {
        ...
        if (type != BombType.BOMB_WATER && world.getBlock((int) entity.posX, (int) entity.posY + 1, (int) entity.posZ)
            .getMaterial() == Material.water) {
            stack.getTagCompound()
                .setInteger("time", 0);
            stack.getTagCompound()
                .setBoolean("inWater", true);
        ...
        if (canTick(
            ...
            stack.getTagCompound()
                .getBoolean("inWater"))) {
            int time = stack.getTagCompound()
                .getInteger("time");
            if (time % 20 == 0) {
                world.playSoundAtEntity(entity, Sounds.BOMB_FUSE, 1.0F, 2.0F + entity.worldObj.rand.nextFloat() * 0.4F);
            }
            boolean flag = world.provider.isHellWorld
                && (type == BombType.BOMB_STANDARD || type == BombType.BOMB_FLOWER);
            stack.getTagCompound()
                .setInteger("time", flag ? Config.getBombFuseTime() : ++time);
            ...
            if (time == fuse && !world.isRemote) {
                ...
                CustomExplosion
                    .createExplosion(bomb, world, entity.posX, entity.posY, entity.posZ, getRadius(type), 0.0F, true);
            }
        }
    }
