...
public class ItemArmorTunic extends ItemArmor {
    ...
    public static boolean onFireDamage(EntityLivingBase entity, float damage) {
        ...
        if (!entity.worldObj.isRemote && stack != null && stack.getItem() == ZSSItems.tunicGoronChest) {
            ...
            damage *= 1.0F + (ZSSEntityInfo.get(entity)
                .getBuffAmplifier(Buff.WEAKNESS_FIRE) * 0.01F);
            damage *= 1.0F - (ZSSEntityInfo.get(entity)
                .getBuffAmplifier(Buff.RESIST_FIRE) * 0.01F);
            if (damage < 0.1F) {
                entity.extinguish();
                return true;
            }
            ...
            if (!stack.getTagCompound()
                .hasKey("lastDamaged") || entity.worldObj.getTotalWorldTime()
                    > (stack.getTagCompound()
                        .getLong("lastDamaged") + 20)) {
                stack.getTagCompound()
                    .setLong("lastDamaged", entity.worldObj.getTotalWorldTime());
    ...
    private boolean shouldDamageArmor(World world, EntityPlayer player, ItemStack stack, int effectID) {
        Material m = world.getBlock((int) player.posX, (int) player.posY + 1, (int) player.posZ)
            .getMaterial();
        if (effectID == Potion.waterBreathing.id) {
            ...
            int time = 50 + (50 * EnchantmentHelper.getRespiration(player))
                + (helm != null && helm.getItem() == ZSSItems.tunicZoraHelm ? 100 : 0);
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4) {
        String[] tooltips = StatCollector.translateToLocal("tooltip." + getUnlocalizedName().substring(5) + ".desc")
            .split("\\\\n");
