...
public class ItemSlingshot extends Item implements ICyclableItem, IFairyUpgrade, IUnenchantable, IZoom {
        ...
        if (!player.worldObj.isRemote && entity instanceof EntityVillager) {
            ...
            if (trades != null) {
                MerchantRecipe trade = new MerchantRecipe(
                    stack.copy(),
                    new ItemStack(Items.emerald, 6 + (2 * seedsFired)));
    ...
    public void handleFairyUpgrade(EntityItem item, EntityPlayer player, TileEntityDungeonCore core) {
        ...
        if (stack.getItem() == ZSSItems.slingshot && core.consumeRupees(Config.getSlingshotCostOne())) {
            ...
            WorldUtils.spawnItemWithRandom(
                core.getWorldObj(),
                new ItemStack(ZSSItems.scattershot),
                core.xCoord,
                core.yCoord + 2,
                core.zCoord);
            core.getWorldObj()
            ...
            WorldUtils.spawnItemWithRandom(
                core.getWorldObj(),
                new ItemStack(ZSSItems.supershot),
                core.xCoord,
                core.yCoord + 2,
                core.zCoord);
            core.getWorldObj()
    ...
    private void addFairyEnchantments(ItemStack stack, EntityPlayer player, TileEntityDungeonCore core) {
        int hearts = ZSSPlayerSkills.get(player)
            .getSkillLevel(SkillBase.bonusHeart);
    ...
    public static void initializeSeeds() {
        if (ItemModeRegistry.SEED_MODES.size() > 0) {
            ZSSMain.logger.error(
                "Slingshot item modes were registered prior to the default ZSS modes; only register item modes during Post Init or later");
        }
    ...
    public static final EntitySeedShot getShotEntity(ItemStack stack, World world, EntityLivingBase shooter,
        float charge, int n, float spread) {
        ...
        if (clazz != null) {
            ...
            try {
                try {
                    shot = clazz
                        .getConstructor(World.class, EntityLivingBase.class, float.class, int.class, float.class)
                        .newInstance(world, shooter, charge, n, spread);
