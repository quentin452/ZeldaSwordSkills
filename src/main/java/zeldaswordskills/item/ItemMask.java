...
public class ItemMask extends ItemArmor implements IEquipTrigger, IUnenchantable, IZoomHelper {
    ...
    public String getArmorTexture(ItemStack stack, Entity entity, int slot, String type) {
        return String.format(
            "%s:textures/armor/%s_layer_%d.png",
            ModInfo.ID,
            getUnlocalizedName().substring(9),
            (slot == 2 ? 2 : 1));
    }
    ...
    public boolean onLeftClickEntity(ItemStack stack, EntityPlayer player, Entity entity) {
        if (player.worldObj.isRemote) {
            return true;
        } else if (entity instanceof EntityNpcMaskTrader && ZSSQuests.get(player)
            .hasCompleted(QuestMaskSales.class)) {
                if (this == ZSSQuests.get(player)
                    .getBorrowedMask()) {
                    player.setCurrentItemOrArmor(0, null);
                    ZSSQuests.get(player)
                        .setBorrowedMask(null);
                ...
                IQuest quest = ZSSQuests.get(player)
                    .get(QuestMaskSales.class);
                EntityVillager villager = (EntityVillager) entity;
                if (quest != null && ZSSVillagerInfo.get(villager)
                    .getMaskDesired() == this) {
    ...
    public boolean onInteract(ItemStack stack, EntityPlayer player, Entity entity) {
        if (!player.worldObj.isRemote) {
            if (entity instanceof EntityVillager) {
                EntityVillager villager = (EntityVillager) entity;
                if (entity.getClass() == EntityVillager.class && this == ZSSVillagerInfo.get(villager)
                    .getMaskDesired()) {
                    QuestMaskSales quest = (QuestMaskSales) ZSSQuests.get(player)
                        .get(QuestMaskSales.class);
                    if (!quest.hasSold(this)) {
                        ...
                        PlayerUtils.sendTranslatedChat(
                            player,
                            "chat.zss.npc.mask_salesman.sales.repeat." + player.worldObj.rand.nextInt(4));
                    }
                } else {
    ...
    public static class ItemMaskBlast extends ItemMask {
        ...
        private boolean isCooling(World world, ItemStack stack) {
            return (stack.hasTagCompound() && world.getTotalWorldTime() < stack.getTagCompound()
                .getInteger("nextUse"));
        }
        ...
        private void setNextUse(World world, ItemStack stack, int time) {
            ...
            stack.getTagCompound()
                .setLong("nextUse", world.getTotalWorldTime() + time);
        }
    }
    ...
    public static class ItemMaskBunny extends ItemMask {
        ...
        private static final AttributeModifier bunnyHoodMoveBonus = (new AttributeModifier(
            bunnyHoodMoveBonusUUID,
            "Bunny Hood Speed Bonus",
            0.3D,
            2)).setSaved(false);
        ...
        public void onArmorEquipped(ItemStack stack, EntityPlayer player, int equipSlot) {
            ...
            player.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                .applyModifier(bunnyHoodMoveBonus);
        ...
        public void onArmorUnequipped(ItemStack stack, EntityPlayer player, int equipSlot) {
            player.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                .removeModifier(bunnyHoodMoveBonus);
            ZSSPlayerInfo.get(player)
                .setFlag(ZSSPlayerInfo.MOBILITY, true);
            ZSSEntityInfo.get(player)
    ...
    public static class ItemMaskCouples extends ItemMask {
        ...
        public void onArmorTick(World world, EntityPlayer player, ItemStack stack) {
            ...
            if (world.getTotalWorldTime() % 64 == 0) {
                List<EntityVillager> villagers = world
                    .getEntitiesWithinAABB(EntityVillager.class, player.boundingBox.expand(8.0D, 3.0D, 8.0D));
                for (EntityVillager villager : villagers) {
                    if (world.rand.nextFloat() < 0.5F) {
                        ZSSVillagerInfo.get(villager)
                            .setMating();
                    }
                }
            }
    ...
    public static class ItemMaskGoron extends ItemMask {
        ...
        public void onArmorEquipped(ItemStack stack, EntityPlayer player, int equipSlot) {
            super.onArmorEquipped(stack, player, equipSlot);
            ZSSEntityInfo.get(player)
                .applyBuff(Buff.RESIST_FIRE, Integer.MAX_VALUE, 100);
        }

        @Override
        public void onArmorUnequipped(ItemStack stack, EntityPlayer player, int equipSlot) {
            ZSSEntityInfo.get(player)
                .removeBuff(Buff.RESIST_FIRE);
        }
    }
    ...
    public static class ItemMaskMajora extends ItemMask {
        ...
        public void onArmorEquipped(ItemStack stack, EntityPlayer player, int equipSlot) {
            super.onArmorEquipped(stack, player, equipSlot);
            ZSSEntityInfo.get(player)
                .applyBuff(Buff.ATTACK_UP, Integer.MAX_VALUE, 100);
        }

        @Override
        public void onArmorUnequipped(ItemStack stack, EntityPlayer player, int equipSlot) {
            ZSSEntityInfo.get(player)
                .removeBuff(Buff.ATTACK_UP);
        }
    }
}
