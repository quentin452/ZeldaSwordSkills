...
public class ItemLonLonMilk extends ItemDrinkable {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (stack.hasTagCompound() && player.worldObj.getWorldTime() > stack.getTagCompound()
            .getLong("expiration")) {
            if (!world.isRemote) {
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int slot, boolean isHeld) {
        ...
        if (!stack.getTagCompound()
            ...
            stack.getTagCompound()
                .setLong("expiration", world.getWorldTime() + 24000);
        }
    }
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean isHeld) {
        long expiration = (stack.hasTagCompound() ? stack.getTagCompound()
            .getLong("expiration") : 0);
        if (expiration > 0 && player.worldObj.getWorldTime() > expiration) {
            ...
            list.add(
                EnumChatFormatting.GREEN
                    + StatCollector.translateToLocalFormatted("tooltip.zss.restore_hp", restore_hp));
            ...
            if (expiration > 0) {
                ...
                list.add(
                    EnumChatFormatting.GRAY
                        + StatCollector.translateToLocalFormatted("tooltip.zss.lon_lon_milk.expires_in", time));
            }
        }
    }
