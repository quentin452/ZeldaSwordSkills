...
public class ItemBombBag extends Item implements IUnenchantable {
    ...
    public IIcon getIcon(ItemStack stack, int pass) {
        ...
        switch (pass) {
            case 0:
                return itemIcon;
            case 1:
                return ones[bombsHeld % 10];
            case 2:
                return tens[(bombsHeld / 10) % 10];
            default:
                return itemIcon;
        }
    }
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean isHeld) {
        ...
        list.add(
            EnumChatFormatting.BOLD + StatCollector
                .translateToLocalFormatted("tooltip.zss.bombbag.desc.bombs", bombName, held, getCapacity(stack)));
    }
    ...
    public int addBombs(ItemStack stack, int amount) {
        ...
        if (amount < 0) {
            stack.getTagCompound()
                .setInteger("bombs", Math.max(0, bombs));
            if (bombs < 1) {
        ...
        stack.getTagCompound()
            .setInteger("bombs", Math.min(bombs, capacity));
    ...
    public int getCapacity(ItemStack stack, boolean trueCapacity) {
        int capacity = (stack.hasTagCompound() ? Math.max(
            BASE_CAPACITY,
            stack.getTagCompound()
                .getInteger("capacity"))
            : BASE_CAPACITY);
    ...
    public void setCapacity(ItemStack stack, int size) {
        verifyNBT(stack);
        stack.getTagCompound()
            .setInteger("capacity", Math.min(size, MAX_CAPACITY));
    }
    ...
    public int getBombsHeld(ItemStack stack) {
        ...
        int bombsHeld = (stack.hasTagCompound() ? stack.getTagCompound()
            .getInteger("bombs") : 0);
        if (bombsHeld < 0) {
            stack.getTagCompound()
                .setInteger("bombs", 0);
    ...
    public boolean areMatchingTypes(ItemStack bag, ItemStack stack, boolean isBomb) {
        ...
        if (isBomb && stack.getItem() instanceof ItemBomb) {
            return getBombsHeld(bag) == 0 || type == -1
    ...
    private void verifyNBT(ItemStack stack) {
        if (!stack.hasTagCompound()) {
            ...
            stack.getTagCompound()
                .setInteger("type", -1);
            stack.getTagCompound()
                .setInteger("capacity", BASE_CAPACITY);
        }
    }
}
