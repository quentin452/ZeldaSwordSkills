...
public class ItemMaskZora extends ItemMask {
    ...
    public boolean hasEffect(ItemStack stack) {
        if (stack.isItemEnchanted()) {
            NBTTagList enchantments = stack.getTagCompound()
                .getTagList("ench", Constants.NBT.TAG_COMPOUND);
            return (enchantments.tagCount() > 1 || ((NBTTagCompound) enchantments.getCompoundTagAt(0)).getShort("id")
                != Enchantment.respiration.effectId);
        }
        return false;
    }
