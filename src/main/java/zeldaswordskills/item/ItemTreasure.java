...
public class ItemTreasure extends Item implements IRightClickEntity, IUnenchantable {
    ...
    public int getItemStackLimit(ItemStack stack) {
        return Treasures.byDamage(stack.getItemDamage())
            .getMaxStackSize();
    }
    ...
    public boolean onLeftClickEntity(ItemStack stack, EntityPlayer player, Entity entity) {
        if (player.worldObj.isRemote) {
            ...
            && ((EntityNpcMaskTrader) entity).checkShopStatus(player, false, true)) {
                return true; // allows quest to complete without having to convert a villager if Salesman already in
                             // town
            } else if (entity instanceof EntityVillager
                && Result.DEFAULT == NpcHelper.convertVillager(player, (EntityVillager) entity, false)) {
                    // villager not converted, try other treasure interactions (note that conversion handles beginnings
                    // of several quests)
        ...
        if (treasure.isBiggoronTrade()) {
            ...
            if (treasure == Treasures.ZELDAS_LETTER) {
                PlayerUtils.sendTranslatedChat(
                    player,
                    "chat.zss.treasure." + treasure.name
                        + (((EntityVillager) entity).isChild() ? ".child" : ".fail")
            ...
            if (entity instanceof EntityAgeable && ((EntityAgeable) entity).isChild()) {
                ...
                IQuest quest = ZSSQuests.get(player)
                    .add(new QuestMaskShop());
                ...
                PlayerUtils
                    .sendTranslatedChat(player, "chat.zss.treasure." + treasure.uninterested + ".uninterested.orca");
            } else {
                PlayerUtils.sendTranslatedChat(player, "chat.zss.treasure." + treasure.uninterested + ".uninterested");
            }
            return true;
     ...
     */
    public boolean handleVillagerTrade(ItemStack stack, EntityPlayer player, EntityVillager villager,
        boolean isLeftClick) {
        Treasures treasure = Treasures.byDamage(stack.getItemDamage());
        ...
        if (villager.isChild()) {
            ...
            if (required == null || PlayerUtils.consumeInventoryItem(player, required, required.stackSize)) {
                ...
                if (stack.stackSize > 1) {
                    --stack.stackSize;
                    PlayerUtils.addItemToInventory(
                        player,
                        trade.getItemToSell()
                            .copy());
                } else {
                ...
                PlayerUtils.sendTranslatedChat(
                    player,
                    "chat.zss.treasure.generic.trade.fail",
                    required.stackSize,
                    new ChatComponentTranslation(required.getUnlocalizedName() + ".name"));
            }
            ...
            new TimedChatDialogue(
                ...
                1000,
                new ChatComponentTranslation(
                    "chat.zss.treasure.hunter.interested.0",
                    new ChatComponentTranslation(
                        trade.getItemToBuy()
                            .getUnlocalizedName() + ".name")),
            ...
            PlayerUtils.sendTranslatedChat(
                player,
                "chat.zss.treasure.generic.interested",
                new ChatComponentTranslation(
                    trade.getItemToSell()
                        .getUnlocalizedName() + ".name"));
        }
    ...
    private void handleChildTrade(ItemStack stack, EntityPlayer player, boolean isLeftClick) {
        ...
        if (isLeftClick) {
            ...
            if (itemRand.nextFloat() < 0.99F || Treasures.byDamage(stack.getItemDamage())
                .isBiggoronTrade()) {
                new TimedChatDialogue(
                    player,
                    1500,
                    1250,
                    new ChatComponentTranslation("chat.zss.treasure.generic.child.return"));
                ...
                new TimedChatDialogue(
                    ...
                    1500,
                    1250,
                    new ChatComponentTranslation("chat.zss.treasure.generic.child.broken"));
            }
        }
    }
