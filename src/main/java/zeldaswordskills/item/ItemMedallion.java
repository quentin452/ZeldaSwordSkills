...
public class ItemMedallion extends Item implements ISpawnParticles {
    ...
    public String getUnlocalizedName(ItemStack stack) {
        return getUnlocalizedName() + "_"
            + BlockAncientTablet.EnumType.byMetadata(stack.getItemDamage())
                .getName();
    }

    @Override
    public int getMaxItemUseDuration(ItemStack stack) {
        return BlockAncientTablet.EnumType.byMetadata(stack.getItemDamage())
            .getItemUseDuration();
    }
        ...
        if (count % 4 == 0) {
            ...
            for (int i = 0; i < 4; ++i) {
                player.worldObj.spawnParticle(
                    "witchMagic",
                    player.posX + player.worldObj.rand.nextGaussian() * 0.13D,
                    player.posY + (player.height / 2.0F) + player.worldObj.rand.nextGaussian() * 0.13D,
                    player.posZ + player.worldObj.rand.nextGaussian() * 0.13D,
                    0.0D,
                    0.0D,
                    0.0D);
            }
        ...
        switch (type) {
            case ETHER:
                ...
                world.playSoundEffect(
                    ...
                    "random.explode",
                    2.0F,
                    0.5F + world.rand.nextFloat() * 0.2F);
                ...
                case ETHER:
                    if (!world.isRemote) {
                        world.addWeatherEffect(
                            new EntityEtherLightning(world, player, entity, entity.posX, entity.posY, entity.posZ));
                    }
                    break;
                case QUAKE: // only affects entities on ground
                    if (entity.onGround) {
                        if (world.isRemote) {
                            ...
                            DamageSource quakeSource = new DamageSourceQuakeIndirect("quake", null, player, duration, 0)
                                .setMagicDamage()
