...
@Optional.InterfaceList(
    value = { @Optional.Interface(iface = "mods.battlegear2.api.IAllowItem", modid = "battlegear2", striprefs = true),
        ...
        @Optional.Interface(
            iface = "mods.battlegear2.api.quiver.ISpecialBow",
            modid = "battlegear2",
            striprefs = true) })
public class ItemHeroBow extends ItemBow
    implements ICyclableItem, IFairyUpgrade, IUnenchantable, IZoom, IAllowItem, ISheathed, ISpecialBow {
    ...
    public int getLevel(ItemStack bow) {
        if (!bow.hasTagCompound() || !bow.getTagCompound()
            .hasKey("zssBowLevel")) {
            setLevel(bow, 1);
        }
        return bow.getTagCompound()
            .getInteger("zssBowLevel");
    }
    ...
    public ItemStack getRenderStackForMode(ItemStack stack, EntityPlayer player) {
        ...
        if (render != null) {
            ...
            for (ItemStack inv : player.inventory.mainInventory) {
                if (inv != null && inv.getItem() == render.getItem()
                    && (inv.getItemDamage() == render.getItemDamage() || !render.getHasSubtypes())) {
    ...
    public ItemStack getArrow(EntityPlayer player) {
        return ZSSPlayerInfo.get(player)
            .getNockedArrow();
    }
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (nockArrowFromInventory(stack, player)) {
            ...
            if (ZSSMain.isBG2Enabled) {
                duration -= (EnchantmentHelper.getEnchantmentLevel(BaseEnchantment.bowCharge.get().effectId, stack)
                    * 20000);
            }
        ...
        if (flag || PlayerUtils.hasItem(player, arrowStack)) {
            ...
            if (arrowEntity != null && confirmArrowShot(arrowStack, player)) {
                ...
                if (arrowEntity instanceof EntityArrowCustom) {
                    applyCustomArrowSettings(
                        event.entityPlayer,
                        event.bow,
                        arrowStack,
                        (EntityArrowCustom) arrowEntity,
                        charge);
                }
    ...
    public boolean canShootArrow(EntityPlayer player, ItemStack bow, ItemStack arrowStack) {
        ...
        if (ItemModeRegistry.ARROW_MODES.contains(arrowStack)) {
            ...
            if (arrowItem instanceof ISpecialAmmunition
                && ((ISpecialAmmunition) arrowItem).getRequiredLevelForAmmo(arrowStack) > this.getLevel(bow)) {
                return false;
            }
            if (arrowItem instanceof IMagicArrow) {
                return ZSSPlayerInfo.get(player)
                    .canUseMagic()
                    && ZSSPlayerInfo.get(player)
                        .getCurrentMagic() >= ((IMagicArrow) arrowItem).getMagicCost(arrowStack, player);
            }
            return true;
        }
    ...
    private ItemStack getAutoBombArrow(ItemStack bow, EntityPlayer player) {
        ...
        for (int i = 0; i < player.inventory.getSizeInventory() && arrow == null; ++i) {
            ...
            if (stack != null) {
                if (stack.getItem() == ZSSItems.arrowBomb || stack.getItem() == ZSSItems.arrowBombFire
                    || stack.getItem() == ZSSItems.arrowBombWater) {
                    arrow = stack;
    ...
    public void registerIcons(IIconRegister register) {
        ...
        for (int i = 0; i < iconArray.length; ++i) {
            iconArray[i] = register
                .registerIcon(ModInfo.ID + ":" + getUnlocalizedName().substring(9) + "_standard_" + (i + 1));
        }
    }
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4) {
        ...
        list.add(
            StatCollector.translateToLocalFormatted(
                "tooltip." + getUnlocalizedName().substring(5) + ".desc.1",
                getLevel(stack)));
        ItemStack mode = ItemModeRegistry.ARROW_MODES.getStack(this.getMode(stack));
        if (mode != null) {
            list.add(
                EnumChatFormatting.YELLOW
                    + StatCollector.translateToLocalFormatted("tooltip.zss.mode", mode.getDisplayName()));
        }
    }
     ...
     */
    private static final void applyCustomArrowSettings(EntityPlayer player, ItemStack bow, ItemStack arrowStack,
        EntityArrowCustom arrowEntity, float charge) {
        Item arrowItem = arrowStack.getItem();
        arrowEntity.setArrowItem(arrowItem);
        if (player.getCurrentArmor(ArmorIndex.WORN_HELM) != null && player.getCurrentArmor(ArmorIndex.WORN_HELM)
            .getItem() == ZSSItems.maskHawkeye) {
    ...
    public static final EntityArrow getArrowEntity(ItemStack arrowStack, World world, EntityLivingBase shooter,
        float charge) {
        ...
        if (clazz != null) {
            ...
            try {
                try {
                    arrow = clazz.getConstructor(World.class, EntityLivingBase.class, float.class)
                        .newInstance(world, shooter, charge);
    ...
    public static void initializeArrows() {
        ...
        if (ItemModeRegistry.ARROW_MODES.size() > 0) {
            ZSSMain.logger.error(
                "ItemHeroBow item modes were registered prior to the default ZSS modes; only register item modes during Post Init or later");
        }
        ...
        if (ZSSMain.isBG2Enabled) {
            for (int i = 0; i < ItemMBArrow.names.length; ++i) {
                ItemModeRegistry.ARROW_MODES
                    .register(new ItemStack(BattlegearConfig.MbArrows, 1, i), ItemMBArrow.arrows[i]);
            }
        }
    }
     ...
     */
    @Optional.Interface(
        iface = "mods.battlegear2.api.quiver.IArrowFireHandler",
        modid = "battlegear2",
        striprefs = true)
    public static class HeroBowFireHandler implements IArrowFireHandler {
        ...
        public boolean canFireArrow(ItemStack arrow, World world, EntityPlayer player, float charge) {
            ...
            if (bow != null) { // allow creative players to shoot custom Zelda arrows using any bow
                return (bow.getItem() instanceof ItemHeroBow
                    ? ((ItemHeroBow) bow.getItem()).canShootArrow(player, bow, arrow)
        ...
        public EntityArrow getFiredArrow(ItemStack arrow, World world, EntityPlayer player, float charge) {
            ...
            if (bow != null && (bow.getItem() instanceof ItemHeroBow || player.capabilities.isCreativeMode)) {
                ...
                if (arrowEntity != null && (!(bow.getItem() instanceof ItemHeroBow)
                    || ((ItemHeroBow) bow.getItem()).confirmArrowShot(arrow, player))) {
                    // vanilla arrow settings will be applied by BG2's arrow loose event
                    if (arrowEntity instanceof EntityArrowCustom) {
                        ItemHeroBow
                            .applyCustomArrowSettings(player, bow, arrow, (EntityArrowCustom) arrowEntity, charge);
                    }
                }
                return arrowEntity;
            }
