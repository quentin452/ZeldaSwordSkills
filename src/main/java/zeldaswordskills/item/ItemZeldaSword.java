...
public class ItemZeldaSword extends ItemSword
    implements IBattlegearWeapon, IFairyUpgrade, IParryModifier, ISacredFlame, ISwingSpeed, IWeapon, IUnenchantable {
    ...
    public ItemZeldaSword(ToolMaterial material, float bonusDamage, boolean twoHanded, int swingSpeed,
        float exhaustion) {
        super(material);
        this.setNoRepair();
    ...
    protected void onStackDamaged(ItemStack stack, EntityLivingBase entity) {
        if (stack.stackSize == 0 && givesBrokenItem && entity instanceof EntityPlayer) {
            PlayerUtils.addItemToInventory(
                (EntityPlayer) entity,
                new ItemStack(ZSSItems.swordBroken, 1, Item.getIdFromItem(this)));
        }
    }
            ...
            if (stack.hasTagCompound() && stack.getTagCompound()
                ...
                list.add(
                    StatCollector.translateToLocalFormatted(
                        "tooltip.zss.sword_tempered.desc.1",
                        stack.getTagCompound()
                            .getInteger("zssHitCount")));
            }
        } else if (stack.getItem() == ZSSItems.swordGolden) {
            if (stack.hasTagCompound() && stack.getTagCompound()
                .hasKey("SacredFlames")) {
                int level = stack.getTagCompound()
                    .getInteger("SacredFlames");
                for (int i = 1; i < 5; ++i) {
                    if (i != 3 && (level & i) != 0) {
                        list.add(
                            StatCollector.translateToLocalFormatted(
                                "tooltip.zss.sword_golden.desc.1",
                                StatCollector.translateToLocal("misc.zss.sacred_flame.name." + i)));
                    }
                }
            }
        }
    ...
    public void handleFairyUpgrade(EntityItem item, EntityPlayer player, TileEntityDungeonCore core) {
        ...
        if (stack.hasTagCompound() && stack.getTagCompound()
            .hasKey("zssHitCount")
            && stack.getTagCompound()
                .getInteger("zssHitCount") > Config.getRequiredKills()) {
            item.setDead();
            WorldUtils.spawnItemWithRandom(
                core.getWorldObj(),
                new ItemStack(ZSSItems.swordGolden),
                core.xCoord,
                core.yCoord + 2,
                core.zCoord);
            core.getWorldObj()
    ...
    public static void onKilledMob(EntityPlayer player, IMob mob) {
        if (!player.worldObj.isRemote && player.getHeldItem() != null
            && player.getHeldItem()
                .getItem() == ZSSItems.swordTempered) {
    ...
    public boolean onClickedSacredFlame(ItemStack stack, World world, EntityPlayer player, int type, boolean isActive) {
        if (world.isRemote) {
            ...
            if ((tag.getInteger("SacredFlames") & type) == 0) {
                ...
                PlayerUtils.sendTranslatedChat(
                    player,
                    "chat.zss.sacred_flame.old.same",
                    new ChatComponentTranslation(stack.getUnlocalizedName() + ".name"));
            }
        } else {
            if (isActive) {
    ...
    private void addSacredFlameEnchantments(ItemStack stack, int type) {
        ...
        boolean flag = false;
        NBTTagList enchList = stack.getTagCompound()
            .getTagList("ench", Constants.NBT.TAG_COMPOUND);
