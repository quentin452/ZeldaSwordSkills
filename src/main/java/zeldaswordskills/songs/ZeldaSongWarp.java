...
public class ZeldaSongWarp extends AbstractZeldaSong {
    ...
    protected WarpPoint getWarpPoint(EntityPlayer player) {
        WarpPoint warp = ZSSPlayerSongs.get(player)
            .getWarpPoint(this);
    ...
    protected void performEffect(EntityPlayer player, ItemStack instrument, int power) {
        ...
        if (warp == null) {
            ...
            if (isBlockValid(player.worldObj, warp.x, warp.y, warp.z)) {
                noBlock = false;
                if (!EntityAITeleport.teleportTo(
                    player.worldObj,
                    player,
                    (double) warp.x + 0.5D,
                    warp.y + 1,
            ...
            if (noBlock || noAir) {
                ...
                PlayerUtils
                    .sendTranslatedChat(player, noAir ? "chat.zss.song.warp.blocked" : "chat.zss.song.warp.missing");
            } else {
