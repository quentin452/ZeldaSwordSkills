...
public class ZeldaSongEpona extends AbstractZeldaSong {
    ...
    protected void performEffect(EntityPlayer player, ItemStack instrument, int power) {
        ...
        List<EntityHorse> horses = player.worldObj
            .getEntitiesWithinAABB(EntityHorse.class, player.boundingBox.expand(8.0D, 4.0D, 8.0D));
        ...
        @SuppressWarnings("unchecked")
        List<EntityCow> cows = player.worldObj
            .getEntitiesWithinAABB(EntityCow.class, player.boundingBox.expand(8.0D, 4.0D, 8.0D));
        ...
        if (!player.worldObj.provider.isSurfaceWorld()) {
            ...
            if (epona == null) {
                ...
                epona.setPosition(
                    player.posX + (vec3.xCoord * 2.0D),
                    player.posY + 1,
                    player.posZ + (vec3.zCoord * 2.0D));
                ...
                PacketDispatcher.sendToPlayersExcept(
                    packet,
                    player,
                    ((WorldServer) player.worldObj).getEntityTracker()
                        .getTrackingPlayers(epona));
