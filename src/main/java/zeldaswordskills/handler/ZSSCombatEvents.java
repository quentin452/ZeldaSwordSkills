...
public class ZSSCombatEvents {
    ...
    public void onSetAttackTarget(LivingSetAttackTargetEvent event) {
        if (event.target instanceof EntityPlayer && event.entity instanceof EntityLiving) {
            ...
            if (mask != null && mask.getItem() == ZSSItems.maskSpooky
    ...
    public void onAttacked(LivingAttackEvent event) {
        if (event.source.getEntity() instanceof EntityLivingBase) {
            event.setCanceled(
                ZSSEntityInfo.get((EntityLivingBase) event.source.getEntity())
        ...
        if (event.entity instanceof EntityPlayer && event.source.getEntity() != null) {
            ...
            if (stack == null || !PlayerUtils.isBlockingWithShield(player)
                ...
                if (stack.getItem() instanceof IReflective) {
                    ...
                    ZSSPlayerInfo.get(player)
                        .onAttackBlocked(stack, event.ammount);
        ...
        if (!event.isCanceled() && event.source.getEntity() != null && ZSSCombatEvents.canEvadeDamage(event.source)) {
            ...
            float evade = ZSSEntityInfo.get(entity)
                .getBuffAmplifier(Buff.EVADE_UP) * 0.01F;
            if (evade > 0.0F && !ZSSEntityInfo.get(entity)
                .isBuffActive(Buff.STUN)) {
                float penalty = ZSSEntityInfo.get(entity)
                    .getBuffAmplifier(Buff.EVADE_DOWN) * 0.01F;
    ...
    public void onPreHurt(LivingHurtEvent event) {
        ...
        if (event.entity instanceof EntityPlayer && event.source.getEntity() != null) {
            EntityPlayer player = (EntityPlayer) event.entity;
            ItemStack stack = player.getHeldItem();
            if (!PlayerUtils.isShield(stack) || !PlayerUtils.isBlocking(player)
                || !ZSSPlayerInfo.get(player)
                    .canBlock()) {
                // nothing to do here if held item is not a shield or not being used to block
            } else if (!TargetUtils.isTargetInFrontOf(player, event.source.getEntity(), 60)) {
    ...
    public static boolean wasProjectileReflected(ItemStack shield, EntityPlayer player, DamageSource source,
        float damage) {
    ...
    private static boolean reflectProjectile(ItemStack shield, EntityPlayer player, DamageSource source) {
        ...
        try {
            ...
            ZSSMain.logger.error(
                String.format(
                    "Unable to reflect projectile - failed to create new instance of class %s: %s",
                    source.getSourceOfDamage()
                        .getClass()
                        .getSimpleName(),
                    e.getMessage()));
        ...
        if (projectile instanceof IReflectable) {
            ...
            if (projectile instanceof EntityThrowable) {
                ...
                // EntityLargeFireball kills ghasts in one hit when reflected by an attack (which results in the player
                // being set as the 'shooting entity')
                // Only let this happen when using a Mirror Shield or it becomes too easy
                if (shield.getItem() instanceof IReflective
                    && ((IReflective) shield.getItem()).isMirrorShield(shield)) {
    ...
    public void onHurt(LivingHurtEvent event) {
        ...
        if (event.source.getEntity() instanceof EntityPlayer && !(event.source instanceof DamageSourceArmorBreak)) {
            ...
            if (combo != null && combo.isComboInProgress()) {
                event.ammount += combo.getCombo()
                    .getNumHits();
            }
            if (skills.isSkillActive(SkillBase.armorBreak)) {
                // LogHelper.info("Entity hurt by armor break; player weapon pre-impact damage: " +
                // player.getHeldItem().getItemDamage());
                ((ArmorBreak) skills.getPlayerSkill(SkillBase.armorBreak)).onImpact(player, event);
                // LogHelper.info("Entity hurt by armor break; player weapon post-impact damage: " +
                // player.getHeldItem().getItemDamage());
                return;
            ...
            if (player.getHeldItem() != null && player.getHeldItem()
                .getItem() instanceof IArmorBreak && event.source.damageType.equals("player")) {
                float damage = (event.ammount * ((IArmorBreak) player.getHeldItem()
                    .getItem()).getPercentArmorIgnored() * 0.01F);
                ...
                DirtyEntityAccessor
                    .damageEntity(event.entityLiving, DamageUtils.causeIArmorBreakDamage(player), damage);
        ...
        if (event.ammount > 0.0F && event.source.getEntity() instanceof EntityPlayer) {
            ZSSPlayerSkills.get((EntityPlayer) event.source.getEntity())
                .onPostImpact(event);
        }
    ...
    public static float applyDamageModifiers(EntityLivingBase defender, DamageSource source, float amount) {
        if (source.getEntity() instanceof EntityLivingBase) {
            ...
            amount *= 1.0F - (ZSSEntityInfo.get(entity)
                .getBuffAmplifier(Buff.ATTACK_DOWN) * 0.01F);
            amount *= 1.0F + (ZSSEntityInfo.get(entity)
                .getBuffAmplifier(Buff.ATTACK_UP) * 0.01F);
        }
