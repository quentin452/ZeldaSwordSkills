...
public class ZSSEntityEvents {
    ...
    public void onJump(LivingJumpEvent event) {
        if (event.entityLiving.getHeldItem() != null && event.entityLiving.getHeldItem()
            .getItem() == ZSSItems.rocsFeather) {
            event.entityLiving.motionY += (event.entityLiving.isSprinting() ? 0.30D : 0.15D);
        }
        if (event.entityLiving.getEquipmentInSlot(ArmorIndex.EQUIPPED_BOOTS) != null
            && event.entityLiving.getEquipmentInSlot(ArmorIndex.EQUIPPED_BOOTS)
                .getItem() == ZSSItems.bootsPegasus) {
    ...
    public void onInteract(EntityInteractEvent event) {
        ...
        if (event.target instanceof EntityVillager
            && Result.DEFAULT != NpcHelper.convertVillager(event.entityPlayer, (EntityVillager) event.target, true)) {
        ...
        if (!event.isCanceled() && stack != null && stack.getItem() instanceof IRightClickEntity) {
            event.setCanceled(
                ((IRightClickEntity) stack.getItem()).onRightClickEntity(stack, event.entityPlayer, event.target));
        }
        ...
        if (!event.isCanceled() && stack != null
            ...
            event.setCanceled(
                ZSSPlayerSongs.get(event.entityPlayer)
                    .milkLonLonCow(event.entityPlayer, (EntityCow) event.target));
        }
        // Finally, check for interactions with the Cursed Man
        if (!event.isCanceled() && event.target.getClass() == EntityVillager.class
            ...
            if (stack == null || (stack.getItem() != ZSSItems.skulltulaToken && stack.getItem() != Items.name_tag)) {
                int tokens = ZSSPlayerInfo.get(event.entityPlayer)
                    .getSkulltulaTokens();
    ...
    public void onLivingUpdate(LivingUpdateEvent event) {
        if (event.entity instanceof EntityPlayer) {
            ...
            if (player.motionY < -0.25D) {
                boolean flag = player.getHeldItem() != null && player.getHeldItem()
                    .getItem() == ZSSItems.rocsFeather;
                if (flag || (player.getCurrentArmor(ArmorIndex.WORN_HELM) != null
                    && player.getCurrentArmor(ArmorIndex.WORN_HELM)
                        .getItem() == ZSSItems.maskDeku)) {
        ...
        if (event.entity instanceof EntityLivingBase) {
            ZSSEntityInfo.get((EntityLivingBase) event.entity)
                .onUpdate();
        }
        if (event.entity instanceof EntityVillager) {
            ZSSVillagerInfo.get((EntityVillager) event.entity)
                .onUpdate();
        }
    }
    ...
    public void onClonePlayer(PlayerEvent.Clone event) {
        ...
        ZSSEntityInfo.get(event.entityPlayer)
            .copy(ZSSEntityInfo.get(event.original));
        ZSSPlayerInfo.get(event.entityPlayer)
            .copy(ZSSPlayerInfo.get(event.original));
    }

    @SubscribeEvent
    public void onEntityJoinWorld(EntityJoinWorldEvent event) {
        if (event.entity instanceof EntityPlayer) {
            ZSSEntityInfo.get((EntityPlayer) event.entity)
                .onJoinWorld();
            ZSSPlayerInfo.get((EntityPlayer) event.entity)
                .onJoinWorld();
        }
    ...
    private static void convertVillagerData(EntityVillager villager) {
        if (villager.getEntityData() != null && villager.getEntityData()
            .hasKey("NextSkulltulaReward")) {
            ZSSVillagerInfo info = ZSSVillagerInfo.get(villager);
            if (info != null) {
                info.setNextSkulltulaReward(
                    villager.getEntityData()
                        .getLong("NextSkulltulaReward"));
            }
            villager.getEntityData()
                .removeTag("NextSkulltulaReward");
        }
    }
    ...
    public void postTeleport(EntityAITeleport.PostEnderTeleport event) {
        ...
        if (event.entity instanceof IEntityTeleport) {
            ((IEntityTeleport) event.entity).getTeleportAI()
                .onPostTeleport(event.targetX, event.targetY, event.targetZ);
        }
    }
