...
public class ZSSItemEvents {
    ...
    private static ItemStack getOrbDrop(EntityLivingBase mob, boolean isBoss) {
        if (dropsList.get(mob.getClass()) != null && mob.worldObj.rand.nextFloat() > Config.getChanceForRandomDrop()) {
            ...
            if (SkillBase.doesSkillExist(id) && SkillBase.getSkill(id)
                .canDrop()) {
                if (dropsList.get(mob.getClass()) != null || isBoss
                    || mob.worldObj.rand.nextFloat() < Config.getRandomMobDropChance()) {
                    orb = (id == SkillBase.bonusHeart.getId() ? new ItemStack(ZSSItems.heartPiece)
                        : new ItemStack(ZSSItems.skillOrb, 1, id));
                }
            }
            return orb;
        }
    ...
    public void onLivingDrops(LivingDropsEvent event) {
        if (event.source.getEntity() instanceof EntityPlayer) {
            ...
            boolean flag = ZSSPlayerSkills.get(player)
                .getSkillLevel(SkillBase.mortalDraw) == SkillBase.mortalDraw.getMaxLevel();
            ItemStack orb = (isBoss && !flag ? new ItemStack(ZSSItems.skillOrb, 1, SkillBase.mortalDraw.getId())
                : getOrbDrop(mob, isBoss));
            if (orb != null && Config.areOrbDropsEnabled()) {
                ...
                float baseChance = Config.getDropChance(
                    orb.getItem() == ZSSItems.heartPiece ? SkillBase.bonusHeart.getId() : orb.getItemDamage());
                if (baseChance > 0.0F
                    && (isBoss || mob.worldObj.rand.nextFloat() < (baseChance + f + (0.005F * event.lootingLevel)))) {
            ...
            if (mob instanceof EntityCreeper && mob.worldObj.rand.nextFloat() < Config.getCreeperDropChance()) {
                event.drops
                    .add(new EntityItem(mob.worldObj, mob.posX, mob.posY, mob.posZ, new ItemStack(ZSSItems.bomb)));
            }
            if (mob instanceof IMob && mob.worldObj.rand.nextInt(Config.getPowerDropRate()) == 0) {
                event.drops.add(
                    new EntityItem(mob.worldObj, mob.posX, mob.posY, mob.posZ, new ItemStack(ZSSItems.powerPiece)));
            }
                ...
                if (mob.worldObj.rand.nextFloat() < chance) {
                    event.drops.add(
                        new EntityItem(
                            ...
                            mob.posY,
                            mob.posZ,
                            new ItemStack(ZSSItems.magicJarBig)));
                }
                int consumable_chance = Config.getMobConsumableFrequency();
                if (consumable_chance > 0
                    && mob.worldObj.rand.nextInt((event.drops.size() + 1) * (12 - consumable_chance)) == 0) {
                    ItemStack stack = new ItemStack(
                        mob.worldObj.rand.nextInt(4) == 0 ? ZSSItems.magicJar : ZSSItems.smallHeart);
    ...
    public void onAnvilUpdate(AnvilUpdateEvent event) {
        ...
        boolean left = (event.left.getItem() instanceof ItemEnchantedBook && event.right.getItem()
            .getItemEnchantability() < 1
            && (Config.areUnenchantablesDisabled() || event.right.getItem() instanceof IUnenchantable));
        boolean right = (event.right.getItem() instanceof ItemEnchantedBook && event.left.getItem()
            .getItemEnchantability() < 1
            && (Config.areUnenchantablesDisabled() || event.left.getItem() instanceof IUnenchantable));
    ...
    public void onBlockHarvest(HarvestDropsEvent event) {
        if (event.harvester != null) {
            if (event.block == Blocks.dirt || event.block == Blocks.grass) {
                if (event.harvester.getCurrentArmor(ArmorIndex.WORN_HELM) != null
                    && event.harvester.getCurrentArmor(ArmorIndex.WORN_HELM)
                        .getItem() == ZSSItems.maskScents
                    && event.world.rand.nextInt(32) == 0) {
                    event.drops.add(
                        event.world.rand.nextInt(4) == 0 ? new ItemStack(Blocks.red_mushroom)
                            : new ItemStack(Blocks.brown_mushroom));
                }
            } else if (event.block == Blocks.tallgrass) {
                if (PlayerUtils.isSword(event.harvester.getHeldItem())
                    && event.world.rand.nextFloat() < Config.getGrassDropChance()) {
                ...
                if (event.world.rand.nextFloat() < (0.005F * event.fortuneLevel)) {
                    ...
                    event.harvester.worldObj.playSoundEffect(
                        event.harvester.posX,
                        event.harvester.posY,
                        event.harvester.posZ,
                        Sounds.SPECIAL_DROP,
                        1.0F,
                        1.0F);
                }
    ...
    public void onItemToss(ItemTossEvent event) {
        ...
        if (stack != null) {
            ...
            if (!item.isDead && (stack.getItem() == Items.emerald || (stack.getItem() instanceof IFairyUpgrade)
                ...
                TileEntityDungeonCore core = WorldUtils
                    .getNearbyFairySpawner(item.worldObj, item.posX, item.posY, item.posZ, true);
                if (core != null) {
        ...
        if (stack != null && stack.getItem() instanceof IHandlePickup) {
            ...
            if (((IHandlePickup) stack.getItem()).onPickupItem(stack, player)) {
                if (stack.stackSize < size) {
                    ...
                    event.item.playSound(
                        Sounds.POP,
                        0.2F,
                        ((event.item.worldObj.rand.nextFloat() - event.item.worldObj.rand.nextFloat()) * 0.7F + 1.0F)
                            * 2.0F);
                    ...
                    if (blockWasSmashed(
                        ...
                        if (event.entityPlayer instanceof EntityPlayerMP) {
                            ...
                            PacketDispatcher.sendTo(
                                new UnpressKeyPacket(UnpressKeyPacket.LMB),
    ...
    private boolean blockWasLifted(World world, EntityPlayer player, ItemStack stack, int x, int y, int z, int side) {
        ...
        if (player.canPlayerEdit(x, y, z, side, stack) || block instanceof ILiftable) {
            ...
            float resistance = (weight != null ? weight.weight
                : (block.getExplosionResistance(null, world, x, y, z, x, y, z) * 5.0F / 3.0F));
            if (isValidBlock && weight != BlockWeight.IMPOSSIBLE
                && strength >= resistance
                && (isLiftable || !block.hasTileEntity(meta))) {
                if (!world.isRemote) {
    ...
    private boolean blockWasSmashed(World world, EntityPlayer player, ItemStack stack, int x, int y, int z, int side) {
        ...
        if (player.canPlayerEdit(x, y, z, side, stack) || isSmashable) {
            ...
            float resistance = (weight != null ? weight.weight
                : (block.getExplosionResistance(null, world, x, y, z, x, y, z) * 5.0F / 3.0F));
            smashResult = (isSmashable ? ((ISmashable) block).onSmashed(world, player, stack, x, y, z, side)
                : smashResult);
            if (smashResult == Result.DEFAULT) {
                boolean isValidBlock = block.isOpaqueCube() || block instanceof BlockBreakable;
                if (isValidBlock && weight != BlockWeight.IMPOSSIBLE
                    && strength >= resistance
                    && (!block.hasTileEntity(meta) || isSmashable)) {
            ...
            ((ISmashBlock) stack.getItem())
                .onBlockSmashed(player, stack, block, meta, (smashResult == Result.ALLOW || wasDestroyed));
        }
