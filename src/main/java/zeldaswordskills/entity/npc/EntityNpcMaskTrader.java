...
public class EntityNpcMaskTrader extends EntityNpcBase implements INpcVillager, ISongTeacher {
            ...
            if (mask != null && stack != null && stack.getItem() == mask) {
                ...
                new TimedChatDialogue(
                    ...
                    new ChatComponentTranslation(
                        "chat.zss.npc.mask_salesman.borrowed.0",
                        new ChatComponentTranslation(mask.getUnlocalizedName(new ItemStack(mask)))),
    ...
    public TeachingResult getTeachingResult(ItemStack stack, EntityPlayer player) {
        ...
        if (!quests.hasCompleted(QuestMaskShop.class)) {
            ...
            .isSongKnown(ZeldaSongs.songHealing)) {
                return new ISongTeacher.TeachingResult(ZeldaSongs.songHealing, true, true);
            } else
            if (((ItemInstrument) stack.getItem()).getInstrument(stack) == ItemInstrument.Instrument.OCARINA_TIME) {
        ...
        // Returning null will call Entity#interact - return true from there to prevent song GUI from opening via
