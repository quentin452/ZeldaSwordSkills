...
public class EntityNpcZelda extends EntityNpcMerchantBase implements INpcVillager, IQuestHandler, ISongTeacher {
    ...
    private static final MerchantRecipe lightArrows = new MerchantRecipe(
        new ItemStack(Items.emerald, 8),
        null,
        new ItemStack(ZSSItems.arrowLight));
    ...
    public MerchantRecipeList getRecipes(EntityPlayer player) {
        ...
        if (list != null) {
            ...
            if (ZSSQuests.get(player)
                .hasCompleted(QuestLightArrows.class)) {
                list.add(
                    0,
                    new MerchantRecipe(
                        lightArrows.getItemToBuy(),
                        lightArrows.getSecondItemToBuy(),
                        lightArrows.getItemToSell()));
            }
        }
        return list;
    }
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (stack != null && stack.getItem() instanceof ItemInstrument
            && (worldObj.isRemote || quests.hasBegun(QuestZeldaTalk.class))) {
            return true; // prevent Item#onItemRightClick so song GUI doesn't open, but still allow quest to begin on
                         // server
    ...
    public Result canInteractConvert(EntityPlayer player, EntityVillager villager) {
        ...
        IQuest quest = ZSSQuests.get(player)
            .add(new QuestZeldaTalk());
        // Even though the held item is checked in #begin, it needs to be checked here as well
        ItemStack stack = player.getHeldItem();
        if (stack != null && stack.getItem() instanceof ItemInstrument
            && ((ItemInstrument) stack.getItem()).getInstrument(stack) == ItemInstrument.Instrument.OCARINA_FAIRY) {
            // allow if player already gave ocarina to Zelda, whether quest completed yet or not
            if (player.worldObj.isRemote) {
                ZSSPlayerSongs.get(player).preventSongGui = true; // prevent song GUI from opening when converting the
                                                                  // Princess
                return Result.DENY;
    ...
    public void onConverted(EntityPlayer player) {
        if (conversionTime < 0) {
            ...
            this.songPos = new int[] { MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY),
                MathHelper.floor_double(this.posZ) };
            PacketDispatcher.sendToAllAround(
                new PlayRecordPacket(ZeldaSongs.songTime.getSoundString(), songPos[0], songPos[1], songPos[2]),
                this,
                64.0D);
        }
    }
    ...
    public void onLivingUpdate() {
        ...
        if (conversionTime > 0) {
            ...
            if (conversionTime == 0) {
                if (songPos != null) {
                    PacketDispatcher
                        .sendToAllAround(new PlayRecordPacket(null, songPos[0], songPos[1], songPos[2]), this, 64.0D);
                    songPos = null;
                }
                ...
                if (!worldObj.isRemote && player != null) {
                    boolean isOwner = (ocarinaOwnerId != null && player.getUniqueID()
                        .compareTo(ocarinaOwnerId) == 0);
                    if (player.getDistanceSqToEntity(this) < 64) {
                        if (isOwner) {
                            IQuest quest = ZSSQuests.get(player)
                                .get(QuestZeldaTalk.class);
    ...
    public TeachingResult getTeachingResult(ItemStack stack, EntityPlayer player) {
        ...
        } else if (((ItemInstrument) stack.getItem()).getInstrument(stack)
            .getPower() < 5) {
