...
public class EntityNpcOrca extends EntityNpcBase implements INpcVillager, IParryModifier {
    ...
    public boolean attackEntityFrom(DamageSource source, float damage) {
        if (source.getEntity() instanceof EntityPlayer) {
            ...
            if (worldObj.getTotalWorldTime() < nextMatch) {
                ...
                && (!(source instanceof IComboDamage) || !((IComboDamage) source).isComboDamage(player))) {
                            ...
                            .isSkillActive(SkillBase.risingCut)) {
                                ...
                                .equals(DamageUtils.NON_SWORD)) {
                                    ...
                                    if (skills.getTargetingSkill() == null) {
                                        sendTranslatedChat(
                                            player,
                                            "chat.zss.npc.orca.match.damage.unskilled." + worldObj.rand.nextInt(3),
                                            false);
                                    } else if (skills.getTargetingSkill()
                                        .getCurrentTarget() == this) {
                                            sendTranslatedChat(
                                                player,
                                                "chat.zss.npc.orca.match.damage.hit." + worldObj.rand.nextInt(4),
                                                false);
                                        } else {
                                            sendTranslatedChat(
                                                player,
                                                "chat.zss.npc.orca.match.damage.wrong_target",
                                                false);
                                        }
                                }
        }
    ...
    public boolean attackEntityAsMob(Entity entity) {
        if (entity instanceof EntityPlayer && ((EntityPlayer) entity).getHealth() < 2.0F) {
            ...
            sendTranslatedChat(
                (EntityPlayer) entity,
                "chat.zss.npc.orca.match.victory." + worldObj.rand.nextInt(4),
                true);
            ...
            if (entity instanceof EntityPlayer) {
                ...
                if (!isBlocking && ++hitCounter > 2) { // hit player 3 times, player loses
                    ...
                    if (isBlocking) {
                        sendTranslatedChat(
                            player,
                            "chat.zss.npc.orca.match.attack.blocked." + worldObj.rand.nextInt(2),
                            false);
                    } else {
                ...
                ICombo combo = ZSSPlayerSkills.get(player)
                    .getComboSkill();
            ...
            return true;
        } else if (entity instanceof EntityPlayer && ZSSPlayerSkills.get((EntityPlayer) entity)
            .isSkillActive(SkillBase.dodge)) {
                sendTranslatedChat(
                    (EntityPlayer) entity,
                    "chat.zss.npc.orca.match.attack.dodged." + worldObj.rand.nextInt(2),
                    false);
            }
        return false;
    }
    ...
    public void setRevengeTarget(EntityLivingBase entity) {
        ...
        if (entity == null && prevTarget instanceof EntityPlayer) {
            ...
            if ((ticksExisted - func_142015_aE()) > 99) { // match timed out
                sendTranslatedChat(
                    (EntityPlayer) prevTarget,
                    "chat.zss.npc.orca.match.timeout." + worldObj.rand.nextInt(3),
                    true);
            }
        }
    }
        ...
        if (getAttackTarget() instanceof EntityPlayer) {
            ...
            if (parryFlag) {
                ...
                if (getHeldItem() == null) {
                    ...
                    sendTranslatedChat(
                        player,
                        "chat.zss.npc.orca.match.defeat.disarmed." + worldObj.rand.nextInt(3),
                        true);
                } else {
                    sendTranslatedChat(
                        player,
                        "chat.zss.npc.orca.match.disarm_attempt." + worldObj.rand.nextInt(3),
                        false);
                }
            } else if (combo != null && combo.isComboInProgress()
                && combo.getCombo()
                    .getConsecutiveHits() > 9) {
                        setRevengeTarget(null);
                        sendTranslatedChat(
                            player,
                            "chat.zss.npc.orca.match.defeat.combo." + worldObj.rand.nextInt(3),
                            true);
                    } else
                if (getHeldItem() == null) { // weapon was probably destroyed via Sword Break
                    setRevengeTarget(null);
                    sendTranslatedChat(
                        player,
                        "chat.zss.npc.orca.match.defeat.disarmed." + worldObj.rand.nextInt(3),
                    ...
                    sendTranslatedChat(
                        player,
                        "chat.zss.npc.orca.match.player_idle." + worldObj.rand.nextInt(3),
                        false);
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (!player.worldObj.isRemote) {
            ...
            } else if (stack != null && stack.getItem() instanceof ItemTreasure
                && stack.getItemDamage() == Treasures.KNIGHTS_CREST.ordinal()) {
    ...
    public Result canLeftClickConvert(EntityPlayer player, EntityVillager villager) {
        if (!villager.worldObj.isRemote && villager.getClass() == EntityVillager.class && !villager.isChild()) {
            ItemStack stack = player.getHeldItem();
            return (stack != null && stack.getItem() == ZSSItems.treasure
                && Treasures.byDamage(stack.getItemDamage()) == Treasures.KNIGHTS_CREST) ? Result.ALLOW
                    : Result.DEFAULT;
        }
        return Result.DEFAULT;
    }
