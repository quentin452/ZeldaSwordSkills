...
public class ZSSVillagerInfo implements IExtendedEntityProperties {
    ...
    private static final Map<Treasures, MerchantRecipe> treasureTrades = new EnumMap<Treasures, MerchantRecipe>(
        Treasures.class);
    ...
    public boolean isHunter() {
        return !villager.isChild() && EnumVillager.BUTCHER.is(villager)
            && villager.getCustomNameTag() != null
    ...
    public boolean isMonsterHunter() {
        return isHunter() && villager.getCustomNameTag()
            .equals("Monster Hunter");
    }
    ...
    public void addHunterTrade(EntityPlayer player, ItemStack toBuy, int price) {
        ...
        if (MerchantRecipeHelper.addToListWithCheck(
            villager.getRecipes(player),
            new MerchantRecipe(toBuy, new ItemStack(Items.emerald, price)))) {
            PlayerUtils.playSound(player, Sounds.SUCCESS, 1.0F, 1.0F);
            PlayerUtils.sendTranslatedChat(
                player,
                "chat.zss.treasure.hunter.new",
                new ChatComponentTranslation(toBuy.getUnlocalizedName() + ".name"));
        } else {
            PlayerUtils.sendTranslatedChat(
                player,
                "chat.zss.treasure.hunter.old",
                new ChatComponentTranslation(toBuy.getUnlocalizedName() + ".name"));
        }
    }
    ...
    public MerchantRecipe getTreasureTrade(Treasures treasure) {
        if (treasureTrades.containsKey(treasure) && treasureVillager.get(treasure) == villager.getProfession()) {
            ...
            return new MerchantRecipe(
                new ItemStack(ZSSItems.treasure, 1, treasure.ordinal()),
    ...
    public int getJelliesReceived(ChuType type) {
        return (data.hasKey("jelliesReceived" + type.ordinal()) ? data.getInteger("jelliesReceived" + type.ordinal())
            : 0);
    }

    /** Returns whether this villager deals at all in Chu Jellies */
    public boolean isChuTrader() {
        return !villager.isChild() && EnumVillager.LIBRARIAN.is(villager)
            && villager.getCustomNameTag()
                .contains("Doc");
    }
    ...
    public void setMating() {
        if (villager.getGrowingAge() == 0 && !isMating()) {
            ...
            if (areSufficientDoors()) {
                Entity e = villager.worldObj.findNearestEntityWithinAABB(
                    EntityVillager.class,
                    villager.boundingBox.expand(8.0D, 3.0D, 8.0D),
                    villager);
    ...
    private void updateMating() {
        if (isMating()) {
            ...
            villager.getLookHelper()
                .setLookPositionWithEntity(mate, 10.0F, 30.0F);
            if (villager.getDistanceSqToEntity(mate) > 2.25D) {
                villager.getNavigator()
                    .tryMoveToEntityLiving(mate, 0.25D);
     ...
     */
    private static final void addTreasureTrade(Treasures treasure, EnumVillager villager, ItemStack required,
        ItemStack output) {
        treasureVillager.put(treasure, (villager != null ? villager.ordinal() : null));
        treasureTrades.put(
            treasure,
            new MerchantRecipe(new ItemStack(ZSSItems.treasure, 1, treasure.ordinal()), required, output));
    }
    ...
    public static void initTrades() {
        addTreasureTrade(
            Treasures.EVIL_CRYSTAL,
            EnumVillager.PRIEST,
            new ItemStack(ZSSItems.arrowLight, 16),
            new ItemStack(ZSSItems.crystalSpirit));
    }
}
