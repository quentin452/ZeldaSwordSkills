...
public class EntityOctorok extends EntityWaterMob implements IMob, IEntityLootable {
    ...
    protected void updateEntityActionState() {
        ...
        if (entityToAttack == null) {
            ...
            if (entityToAttack instanceof EntityPlayer && ((EntityPlayer) entityToAttack).capabilities.disableDamage) {
                entityToAttack = findPlayerToAttack();
        ...
        if (this.entityToAttack != null && this.inWater) {
            ...
            if (this.fleeingTick > 0) {
                this.randomMotionVecX = MathHelper
                    .clamp_float((float) (this.posX - this.entityToAttack.posX), -1.0F, 1.0F) * 0.3F;
                this.randomMotionVecZ = MathHelper
                    .clamp_float((float) (this.posZ - this.entityToAttack.posZ), -1.0F, 1.0F) * 0.3F;
            } else if (distance > 12.0F) {
                // Move closer to target when too far away and not fleeing
                this.randomMotionVecX = MathHelper
                    .clamp_float((float) (this.entityToAttack.posX - this.posX), -1.0F, 1.0F) * 0.2F;
                this.randomMotionVecZ = MathHelper
                    .clamp_float((float) (this.entityToAttack.posZ - this.posZ), -1.0F, 1.0F) * 0.2F;
                ...
                this.randomMotionVecX = MathHelper
                    .clamp_float((float) (this.entityToAttack.posX - this.posX), -1.0F, 1.0F) * f;
                this.randomMotionVecZ = MathHelper
                    .clamp_float((float) (this.entityToAttack.posZ - this.posZ), -1.0F, 1.0F) * f;
            }
        } else if (!this.inWater || this.rand.nextInt(50) == 0
    ...
    public boolean isInWater() {
        return worldObj
            .handleMaterialAcceleration(boundingBox.expand(0.0D, -0.6000000238418579D, 0.0D), Material.water, this);
    }

    @Override
    public boolean getCanSpawnHere() {
        return posY > 45.0D && posY < 63.0D
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (this.isEntityInvulnerable()) {
            return false;
        } else if (source.getSourceOfDamage() instanceof IReflectableOrigin
            && ((IReflectableOrigin) source.getSourceOfDamage()).getReflectedOriginEntity() == this) {
    ...
    public boolean attackEntityAsMob(Entity entity) {
        ...
        if (flag) {
            if (i > 0) {
                entity.addVelocity(
                    (double) (-MathHelper.sin(rotationYaw * (float) Math.PI / 180.0F) * (float) i * 0.5F),
                    0.1D,
                    (double) (MathHelper.cos(rotationYaw * (float) Math.PI / 180.0F) * (float) i * 0.5F));
                motionX *= 0.6D;
                motionZ *= 0.6D;
            }
    ...
    protected void attackEntity(Entity entity, float distance) {
        if (this.attackTime > 0) {
            ...
            && TargetUtils.isTargetInSight(this, entity)) {
                ...
                if (!this.worldObj.isRemote && projectile != null) {
                    this.worldObj
                        .playSoundAtEntity(this, Sounds.CORK, 1.0F, 1.0F / (this.rand.nextFloat() * 0.4F + 1.0F));
    ...
    protected void dropRareDrop(int rarity) {
        switch (rarity) {
            case 1:
                this.entityDropItem(new ItemStack(ZSSItems.treasure, 1, Treasures.TENTACLE.ordinal()), 0.0F);
                break;
            default:
                this.entityDropItem(
                    rand.nextInt(3) == 0 ? new ItemStack(Items.emerald) : new ItemStack(ZSSItems.smallHeart),
                    0.0F);
        }
    }
