...
public class EntityKeese extends EntityBat implements IMob, IEntityLootable {
    ...
    public static EntityKeese getRandomKeeseForLocation(World world, float variance, int x, int y, int z) {
        ...
        if (biome != null) {
            ...
            for (SpawnListEntry entry : spawns) {
                if (EntityKeese.class.isAssignableFrom(entry.entityClass)
            ...
            if (toSpawn != null) {
                try {
                    return (EntityKeese) toSpawn.getConstructor(World.class)
                        .newInstance(world);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(8.0F);
        ZSSEntityInfo.get(this)
            .applyBuff(Buff.EVADE_UP, Integer.MAX_VALUE, 50);
    }
    ...
    protected void dropRareDrop(int rarity) {
        switch (rarity) {
            case 1:
                this.entityDropItem(new ItemStack(ZSSItems.treasure, 1, Treasures.MONSTER_CLAW.ordinal()), 0.0F);
                break;
            default:
                this.entityDropItem(
                    new ItemStack(this.rand.nextInt(3) > 0 ? Items.emerald : ZSSItems.smallHeart),
                    0.0F);
        }
    }
    ...
    public void onCollideWithPlayer(EntityPlayer player) {
        if (attackTime < 1 && canEntityBeSeen(player)
            && getDistanceSqToEntity(player) < 2.0D
            && (player.getCurrentArmor(ArmorIndex.WORN_HELM) == null || player.getCurrentArmor(ArmorIndex.WORN_HELM)
                .getItem() != ZSSItems.maskSkull)) {
    ...
    protected void updateAITasks() {
        ...
        if (!getIsBatHanging()) {
            ...
            if (currentFlightTarget == null || rand.nextInt(30) == 0
                || currentFlightTarget.getDistanceSquared((int) posX, (int) posY, (int) posZ)
                    < (attackingPlayer != null ? 1.0F : 4.0F)) {
                attackingPlayer = getLastAttacker() instanceof EntityPlayer ? (EntityPlayer) getLastAttacker()
                    : worldObj.getClosestPlayerToEntity(this, 8.0D);
                if (attackingPlayer != null && !attackingPlayer.capabilities.isCreativeMode
                    && (attackingPlayer.getCurrentArmor(ArmorIndex.WORN_HELM) == null
                        || attackingPlayer.getCurrentArmor(ArmorIndex.WORN_HELM)
                            .getItem() != ZSSItems.maskSkull)) {
                    currentFlightTarget = new ChunkCoordinates(
                        (int) attackingPlayer.posX,
                        (int) attackingPlayer.posY + 1,
                        (int) attackingPlayer.posZ);
            ...
            moveForward = 0.5F;
            rotationYaw += f1;
            if (attackingPlayer == null && rand.nextInt(100) == 0
    ...
    public boolean getCanSpawnHere() {
        return (worldObj.difficultySetting != EnumDifficulty.PEACEFUL && (posY < 64.0D || rand.nextInt(16) > 13)
            && isValidLightLevel()
