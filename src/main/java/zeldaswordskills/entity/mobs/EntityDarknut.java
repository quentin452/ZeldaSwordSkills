...
public class EntityDarknut extends EntityMob
    implements IEntityBackslice, IEntityEvil, IEntityLootable, IParryModifier, IPowerAttacker {
    ...
    private static final AttributeModifier armorKnockbackModifier = (new AttributeModifier(
        armorKnockbackModifierUUID,
    ...
    private static final AttributeModifier armorMoveBonus = (new AttributeModifier(
        armorMoveBonusUUID,
        "Armor Movement Bonus",
        0.35D,
        1)).setSaved(false);
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        ...
        if (isEntityInvulnerable() || isSpinning()) {
        ...
        } else if (source.isUnblockable() || (isPlayer && ZSSPlayerSkills.get((EntityPlayer) source.getEntity())
            .isSkillActive(SkillBase.armorBreak))) {
                ...
                amount = damageDarknutArmor(
                    amount * (1.25F - (0.25F * (float) worldObj.difficultySetting.getDifficultyId())));
                if (amount < 0.5F) {
                    return false;
                ...
                if (isArmored()) {
                    if (TargetUtils.isTargetInFrontOf(this, source.getEntity(), 120)) {
                        ...
                        if (isPlayer && ZSSPlayerSkills.get((EntityPlayer) source.getEntity())
                            .isSkillActive(SkillBase.backSlice)) {
                            return super.attackEntityFrom(source, amount);
                    ...
                    if (recentHitTimer > 0) {
                        if (++recentHits > 3 && rand.nextFloat() < 0.15F * ((float) recentHits)) {
                            playLivingSound();
                            worldObj
                                .playSoundAtEntity(this, Sounds.SPIN_ATTACK, 1.0F, (rand.nextFloat() * 0.4F) + 0.5F);
                            worldObj.setEntityState(this, SPIN_FLAG);
                            spinAttackTimer = 12;
                            recentHits = 0;
                            targets = worldObj
                                .getEntitiesWithinAABB(EntityLivingBase.class, boundingBox.expand(4.0D, 0.0D, 4.0D));
        ...
        for (int i = -1; i < 2; ++i) {
            for (int j = -1; j < 2; ++j) {
                for (int k = -1; k < 2; ++k) {
                    ...
                    if (block.isNormalCube() && hardness >= 0.0F
                        && hardness < 20.0F
                        && block.canEntityDestroy(worldObj, dx, dy, dz, this)) {
                        flag = true;
    ...
    protected boolean attackEntity(Entity entity, int flag) {
        ...
        if (entity.attackEntityFrom(getDamageSource(flag), damage)) {
            if (k > 0) {
                entity.addVelocity(
                    (double) (-MathHelper.sin(rotationYaw * (float) Math.PI / 180.0F) * (float) k * 0.5F),
                    0.1D,
                    (double) (MathHelper.cos(rotationYaw * (float) Math.PI / 180.0F) * (float) k * 0.5F));
                motionX *= 0.6D;
                motionZ *= 0.6D;
            }
    ...
    protected void dropRareDrop(int rarity) {
        switch (rarity) {
            case 1:
                entityDropItem(new ItemStack(ZSSItems.treasure, 1, Treasures.KNIGHTS_CREST.ordinal()), 0.0F);
                break;
            default:
