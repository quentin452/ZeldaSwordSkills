...
public class EntityDekuFire extends EntityDekuBaba implements IEntityAdditionalSpawnData, IRangedAttackMob {
    ...
    /**
     * @param rangeRatio Distance between the target and shooter divided by the shooter's maximum range, between 0.1F
     *                   and 1.0F but not used here
     */
    @Override
    public void attackEntityWithRangedAttack(EntityLivingBase target, float rangeRatio) {
        ...
        EntityMagicSpell spell = new EntityMagicSpell(
            ...
            0.375F + (0.125F * (float) difficulty),
            (float) (14 - difficulty * 4));
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (this.isFullyAlert() && source.getSourceOfDamage() instanceof IReflectableOrigin
