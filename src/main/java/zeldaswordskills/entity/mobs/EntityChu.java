 ...
 * 
 */
public abstract class EntityChu extends EntityLiving
    implements IMob, IEntityAdditionalSpawnData, IEntityBombEater, IEntityLootable {
    ...
    public static EntityChu getRandomChuForLocation(World world, float variance, int x, int y, int z) {
        ...
        if (biome != null) {
            ...
            for (SpawnListEntry entry : spawns) {
                if (EntityChu.class.isAssignableFrom(entry.entityClass)
                    && (toSpawn == null || world.rand.nextFloat() < variance)) {
                    toSpawn = entry.entityClass;
                }
            }
            if (toSpawn != null) {
                try {
                    return (EntityChu) toSpawn.getConstructor(World.class)
                        .newInstance(world);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected static final AttributeModifier sizeDamageModifier = (new AttributeModifier(
        sizeDamageModiferUUID,
    ...
    protected void setSize(int size) {
        ...
        AttributeModifier newModifier = (new AttributeModifier(sizeDamageModiferUUID, "Size Damage Modifier", size, 0))
            .setSaved(true);
    ...
    protected void dropRareDrop(int rarity) {
        switch (rarity) {
            case 1:
                entityDropItem(new ItemStack(ZSSItems.treasure, 1, Treasures.JELLY_BLOB.ordinal()), 0.0F);
                break;
            default:
    ...
    public boolean getCanSpawnHere() {
        if (worldObj.getWorldInfo()
            .getTerrainType()
            .handleSlimeSpawnReduction(rand, worldObj)) {
            return false;
        } else if (worldObj.difficultySetting != EnumDifficulty.PEACEFUL) {
            if (posY > 50.0D && rand.nextFloat() < 0.5F
            ...
            Chunk chunk = worldObj
                .getChunkFromBlockCoords(MathHelper.floor_double(posX), MathHelper.floor_double(posZ));
            if (rand.nextInt(10) == 0 && chunk.getRandomWithSeed(432191789L)
                .nextInt(10) == 0 && posY < 50.0D) {
                return super.getCanSpawnHere();
            }
        }
        return false;
    ...
    protected float getDamage() {
        return (float) this.getEntityAttribute(SharedMonsterAttributes.attackDamage)
            .getAttributeValue();
    }
    ...
    public void onCollideWithPlayer(EntityPlayer player) {
        if (this.attackTime > 0 || player.boundingBox.maxY < this.boundingBox.minY
            || player.boundingBox.minY > this.boundingBox.maxY) {
            return;
        }
    ...
    public void spawnParticlesOnLanding() {
        ...
        for (int j = 0; j < i * 8; ++j) {
            ...
            EntityFX particle = new EntityBreakingFX(
                worldObj,
                posX + (double) f2,
                boundingBox.minY,
                posZ + (double) f3,
    ...
    private void attemptMerge() {
        ...
        if (!worldObj.isRemote && i < 3 && getHealth() < (getMaxHealth() / 2) && rand.nextInt(16) == 0) {
            List<EntityChu> list = this.worldObj
                .getEntitiesWithinAABB(this.getClass(), this.boundingBox.expand(2.0D, 1.0D, 2.0D));
            for (EntityChu chu : list) {
                if (chu != this && chu.isEntityAlive()
                    && chu.getSize() == this.getSize()
                    && chu.getHealth() < (chu.getMaxHealth() / 2)) {
                    worldObj.playSoundAtEntity(this, Sounds.CHU_MERGE, 1.0F, 1.0F / (rand.nextFloat() * 0.4F + 1.0F));
                    EntityChu newChu = createInstance();
                    newChu.setSize(i * 2);
                    newChu.setLocationAndAngles(
                        (this.posX + chu.posX) / 2,
                        posY + 0.5D,
                        (this.posZ + chu.posZ) / 2,
                        rand.nextFloat() * 360.0F,
                        0.0F);
