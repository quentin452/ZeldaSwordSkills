...
public class EntityKeeseThunder extends EntityKeese {
    ...
    protected void applyEntityAttributes() {
        ...
        ZSSEntityInfo.get(this)
            .applyBuff(Buff.RESIST_SHOCK, Integer.MAX_VALUE, 100);
        ZSSEntityInfo.get(this)
            .applyBuff(Buff.WEAKNESS_WATER, Integer.MAX_VALUE, 100);
    }
        ...
        if (this.getShockTime() > 0) {
            if (source instanceof EntityDamageSourceIndirect) {
                if (source.isMagicDamage()) {
                    ...
                    ZSSEntityInfo.get(this)
                        .stun(20 + this.rand.nextInt((int) (amount * 5) + 1));
                    this.setShockTime(0);
                ...
                && !source.damageType.equals("thorns")) {
                    ...
                    if (stack != null && stack.getItem() instanceof ItemTool) {
                        ...
                        isWood = ((ItemSword) stack.getItem()).getToolMaterialName()
                            .equals(ToolMaterial.WOOD.toString());
                    }
                    if (!isWood) {
                        source.getEntity()
                            .attackEntityFrom(this.getDamageSource(), this.getDamage());
                        this.worldObj
                            .playSoundAtEntity(this, Sounds.SHOCK, 1.0F, 1.0F / (this.rand.nextFloat() * 0.4F + 1.0F));
                    }
                }
            return false;
        }
        ...
        if (time > 0) {
            ...
            if (time % 8 > 6 && this.rand.nextInt(4) == 0) {
                this.worldObj.playSoundAtEntity(
                    ...
                    Sounds.SHOCK,
                    this.getSoundVolume(),
                    1.0F / (this.rand.nextFloat() * 0.4F + 1.0F));
            }
        }
    }
    ...
    protected void updateAITasks() {
        ...
        if (ZSSEntityInfo.get(this)
            .isBuffActive(Buff.STUN) || this.getIsBatHanging()) {
            // because Keese get moved twice per tick due to inherited EntityBat methods
            return;
        }
        if (this.getShockTime() == 0 && !ZSSEntityInfo.get(this)
            .isBuffActive(Buff.STUN)) {
            if (this.attackingPlayer != null
                && ((this.recentlyHit > 0 && this.rand.nextInt(20) == 0) || this.rand.nextInt(300) == 0)) {
                this.setShockTime(
                    this.rand.nextInt(50)
                        + (this.worldObj.difficultySetting.getDifficultyId() * (this.rand.nextInt(20) + 10)));
            }
        }
    }
}
