...
public class EntityDekuBaba extends EntityDekuBase implements IEntityBombEater, IEntityDynamic, IEntityCustomTarget {
    ...
    public static final EntityAction ACTION_BOMB = new EntityAction(
        EntityDekuBase.flag_index++,
        ACTION_ATTACK.duration,
        ACTION_ATTACK.action_frame);
    ...
    /**
     * Alertness level of this Deku Baba; 0 is fully retracted, {@code ACTION_SPROUT#duration} is fully extended and
     * ready to act.
    ...
    public boolean isFullyAlert() {
        return isEntityAlive()
            && getActionTime(ACTION_SPROUT.id) > ACTION_SPROUT.getDuration(getActionSpeed(ACTION_SPROUT.id));
    }
    ...
    public int getActionTime(int action_id) {
        ...
        // action timer starts at max and decrements to 0, but needs to be returned as starting at 0 and increment to
        // max
    ...
    public boolean canExecute(int action_id, IEntityDynamicAI ai) {
        if (isAttack(action_id)) {
            ...
            return action_timer == 0 && entity != null
                && canAttack()
                && TargetUtils.isTargetInFrontOf(this, entity, 15F);
        } else if (action_id == ACTION_PRONE.id) {
            return prone;
        }
        return true;
    ...
    public void performAction(int action_id, IEntityDynamicAI ai) {
        ...
        if (isConfused()) {
            ...
            if (target instanceof IEntityBombIngestible && canAttack()
                && TargetUtils.isTargetInFrontOf(this, target, 15F)) {
                ...
                if (ZSSEntityInfo.get(this)
                    ...
                    double damage = this.getAttributeMap()
    ...
    public int getBombTimer() {
        return (status_timer > 0 && dataWatcher.getWatchableObjectInt(STATUS_INDEX) == STATUS_BOMB
            ? FUSE_TIME - status_timer
            : 0);
    }
    ...
    protected boolean didAttackCauseProne(DamageSource source, float amount) {
        ...
        } else if (source instanceof IPostDamageEffect
            && ((IPostDamageEffect) source).getDuration(EnumDamageType.STUN) > 0) {
        ...
        if (flag && this.getHealth() > 0.0F) { // only set to prone if still alive
            if (!isConfused() && source.getSourceOfDamage() instanceof EntityPlayer
                && ((EntityPlayer) source.getSourceOfDamage()).getHeldItem() == null) {
