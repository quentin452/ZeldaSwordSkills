...
public abstract class EntityDekuBase extends EntityCreature implements IMob, IEntityAnimationOffset {
    ...
    protected void applyEntityAttributes() {
        ...
        ZSSEntityInfo.get(this)
            .applyBuff(Buff.RESIST_STUN, Integer.MAX_VALUE, 100);
    }
    ...
    protected boolean isSourceFatal(DamageSource source) {
        if (source.getSourceOfDamage() instanceof EntityBoomerang
            || source.getSourceOfDamage() instanceof EntitySwordBeam) {
            return true;
        } else if (source.getEntity() instanceof EntityPlayer) {
            return (ZSSPlayerSkills.get((EntityPlayer) source.getEntity())
                .isSkillActive(SkillBase.spinAttack));
        } else if (source.isExplosion() && ZSSEntityInfo.get(this)
            .hasIngestedBomb()) {
                return true;
            }
        return false;
    ...
    protected boolean isSlashing(DamageSource source) {
        if (source.getSourceOfDamage() instanceof EntityLivingBase) {
            ...
            return source.getSourceOfDamage() instanceof EntityBoomerang
                || source.getSourceOfDamage() instanceof EntitySwordBeam;
        }
        return false;
    }
