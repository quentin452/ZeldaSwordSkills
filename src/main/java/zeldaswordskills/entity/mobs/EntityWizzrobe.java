...
public abstract class EntityWizzrobe extends EntityMob
    implements IEntityEvil, IEntityLootable, IEntityTeleport, IMagicUser {
    ...
    public static EntityWizzrobe getRandomWizzrobeForLocation(World world, float variance, int x, int y, int z) {
        ...
        if (biome != null) {
            ...
            for (SpawnListEntry entry : spawns) {
                if (EntityWizzrobe.class.isAssignableFrom(entry.entityClass)
            ...
            if (toSpawn != null) {
                try {
                    return (EntityWizzrobe) toSpawn.getConstructor(World.class)
                        .newInstance(world);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected void applyEntityAttributes() {
        ...
        ZSSEntityInfo.get(this)
            .applyBuff(Buff.RESIST_MAGIC, Integer.MAX_VALUE, 50);
        ZSSEntityInfo.get(this)
            .applyBuff(Buff.RESIST_STUN, Integer.MAX_VALUE, 50);
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (!worldObj.isRemote) {
            ...
            if (spell instanceof IReflectableOrigin
                && ((IReflectableOrigin) spell).getReflectedOriginEntity() == this) {
                wasReflected = true;
                // Reflected spells ignore Wizzrobe's magic and element resistance
                if (source instanceof DamageSourceBaseIndirect) {
                    ((DamageSourceBaseIndirect) source).ignoreResistance(EnumDamageType.MAGIC, 100)
                        .ignoreResistance(
                            this.getMagicType()
                                .getDamageType(),
                            100);
    ...
    public void castRangedSpell(EntityLivingBase target, float range) {
        ...
        EntityMagicSpell spell = new EntityMagicSpell(
            worldObj,
            this,
            target,
            0.8F + (0.25F * difficulty),
            (float) (14 - worldObj.difficultySetting.getDifficultyId() * 4));
    ...
    public void onUpdate() {
        ...
        if (teleportAI.getTeleBounds() == null && this.getEntityToAttack() == null && ++noTargetTime > 400) {
            ...
            if (player instanceof EntityPlayer && !worldObj.isRemote
                && canEntityBeSeen(player)
                && !((EntityPlayer) player).capabilities.disableDamage) {
                setTarget(player);
    ...
    public boolean getCanSpawnHere() {
        return !worldObj.isDaytime() && super.getCanSpawnHere()
            && worldObj.getTotalWorldTime() > Config.getTimeToSpawnWizzrobe();
    }
