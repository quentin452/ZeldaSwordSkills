...
public class EntitySwordBeam extends EntityMobThrowable {
    ...
    public void onUpdate() {
        ...
        if (inGround || ticksExisted > lifespan) {
            ...
            List<MovingObjectPosition> mops = TargetUtils
                .checkForImpact(this.worldObj, this, this.getThrower(), 0.3D, this.ticksExisted >= 5, this.isMaster);
            if (mops != null) {
    ...
    protected void onImpact(MovingObjectPosition mop) {
        if (!worldObj.isRemote) {
            ...
            SwordBeam skill = (player != null ? (SwordBeam) ZSSPlayerSkills.get(player)
                .getPlayerSkill(SkillBase.swordBeam) : null);
            if (mop.typeOfHit == MovingObjectType.ENTITY) {
                Entity entity = mop.entityHit;
                if (entity == thrower || this.entitiesHit.contains(entity.getEntityId())) {
                    return;
                }
                ...
                if (entity.attackEntityFrom(
                    DamageUtils.causeIndirectComboDamage(this, thrower)
                        .setProjectile(),
                    damage)) {
