...
public class EntityCyclone extends EntityMobThrowable {
    ...
    protected DamageSource getDamageSource() {
        return new DamageSourceBaseIndirect("blast.wind", this, this.getThrower(), false, EnumDamageType.WIND)
            .setProjectile()
            .setMagicDamage();
    }
        ...
        if (!worldObj.isRemote) {
            ...
            if (ticksExisted > 40) {
                ...
                worldObj.playSoundAtEntity(
                    this,
                    MagicType.WIND.getMovingSound(),
                    MagicType.WIND.getSoundPitch(rand),
                    MagicType.WIND.getSoundVolume(rand));
            }
        } else {
            spawnParticleRing();
        }
    ...
    protected void onImpact(MovingObjectPosition mop) {
        if (mop.typeOfHit == MovingObjectType.BLOCK) {
            Material m = worldObj.getBlock(mop.blockX, mop.blockY, mop.blockZ)
                .getMaterial();
            ...
            if (!(mop.entityHit instanceof EntityLivingBase)
                || rand.nextFloat() > ((EntityLivingBase) mop.entityHit).getAttributeMap()
                    .getAttributeInstance(SharedMonsterAttributes.knockbackResistance)
                    .getAttributeValue()) {
                ...
                if (mop.entityHit instanceof EntityPlayerMP && !worldObj.isRemote) {
                    ((EntityPlayerMP) mop.entityHit).playerNetServerHandler
                        .sendPacket(new S12PacketEntityVelocity(mop.entityHit));
                }
            }
        }
    }
    ...
    private void attackNearbyEntities() {
        if (getDamage() > 0.0F) {
            ...
            List<EntityLivingBase> entities = worldObj
                .getEntitiesWithinAABB(EntityLivingBase.class, boundingBox.expand(d, d, d));
            for (EntityLivingBase entity : entities) {
                if (affectedEntities.contains(entity.getEntityId()) || (entity == getThrower() && ticksExisted < 8)) {
                    continue;
                }
                entity.attackEntityFrom(
                    new EntityDamageSourceIndirect("tornado", this, getThrower()).setProjectile()
                        .setMagicDamage(),
                    getDamage());
    ...
    private void destroyLeaves() {
        ...
        Set<ChunkPosition> affectedBlockPositions = WorldUtils
            .getAffectedBlocksList(worldObj, rand, getArea(), posX, posY, posZ, null);
        ...
        while (iterator.hasNext()) {
            ...
            Material m = worldObj.getBlock(i, j, k)
                .getMaterial();
            if ((m == Material.leaves && Config.canDekuDenude()) || m == Material.plants
                || m == Material.vine
                || m == Material.web) {
    ...
    private void spawnParticleRing() {
        ...
        FXCycloneRing ring = new FXCycloneRing(
            worldObj,
            posX,
            posY + 0.1D,
            posZ,
