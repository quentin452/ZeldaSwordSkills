...
public class EntityWhip extends EntityThrowable {
    ...
    /**
     * Watchable object index for thrower entity's id, since EntityThrowable#thrower is both private and not synced to
     * client
     */
        ...
        switch (getType()) {
            case WHIP_MAGIC:
                ...
                if (block instanceof BlockSandStone || block instanceof BlockHugeMushroom
                    || (block.getMaterial()
                        .blocksMovement() && block.getBlockHardness(worldObj, x, y, z) > 1.0F)) {
                    return true;
    ...
    protected boolean isSideClear(int x, int y, int z) {
        Material m = worldObj.getBlock(x, y, z)
            .getMaterial();
            ...
            if (!isInGround() && ticksExisted < getMaxDistance()) {
                ...
                if (canGrabBlock(block, mop.blockX, mop.blockY, mop.blockZ, mop.sideHit)) {
                    ...
                    AxisAlignedBB box = block
                        .getCollisionBoundingBoxFromPool(worldObj, mop.blockX, mop.blockY, mop.blockZ);
                    ...
                    if (box != null) {
                        ...
                        // adjusting posX/Y/Z here seems to make no difference to the rendering, even when client side
                        // makes same changes
                        ...
                        switch (mop.sideHit) {
                            ...
                            // case 4: posX -= 0.515D; break; // WEST (a little extra to compensate for block border,
                            // otherwise renders black)
                            // case 3: posZ += 0.5D; break; // SOUTH
                            // case 2: posZ -= 0.515D; break; // NORTH (a little extra to compensate for block border,
                            // otherwise renders black)
                            case SideHit.TOP:
                    ...
                    if (!worldObj.isRemote) {
                        ...
                        boolean drop = (getThrower() instanceof EntityPlayer
            ...
            if (mop.entityHit instanceof EntityLivingBase) {
                ...
                if (getThrower() instanceof EntityPlayer) {
                    ...
                    if (lootTarget(player, target)) {
                        inflictDamage = (target instanceof IEntityLootable
                            ? ((IEntityLootable) target).isHurtOnTheft(player, getType())
                            : Config.getHurtOnSteal());
                    } else if (target.getHeldItem() != null && ZSSPlayerSkills.get(player)
                        .hasSkill(SkillBase.parry)) {
    ...
    private boolean lootTarget(EntityPlayer player, EntityLivingBase target) {
        if (target.getEntityData()
            .getBoolean("LootableEntityFlag")) {
            return false;
        }
        ...
        if (lootable == null || lootable.onLootStolen(player, wasItemStolen)) {
            if (!worldObj.isRemote) {
                target.getEntityData()
                    .setBoolean("LootableEntityFlag", true);
            }
        }
    ...
    protected boolean canUpdate() {
        ...
        if (!isDead && thrower instanceof EntityPlayer && ((EntityPlayer) thrower).isUsingItem()) {
            return thrower.getHeldItem() != null && thrower.getHeldItem()
        ...
        if (thrower != null && !thrower.onGround && isInGround()) {
            if (thrower.worldObj.isRemote) {
                ...
                if (swingVec != null) {
                    ...
                    MovingObjectPosition mop = TargetUtils
                        .checkForImpact(worldObj, thrower, this, -(thrower.width / 4.0F), false);
                    ...
                    if (thrower.fallDistance > 0 && thrower.motionY < 0) {
                        // 0.466885F seems to be roughly the amount added each tick while swinging; round for a little
                        // extra server-side padding
