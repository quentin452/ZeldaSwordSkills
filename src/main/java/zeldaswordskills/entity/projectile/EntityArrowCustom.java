...
public class EntityArrowCustom extends EntityArrow implements IEntityAdditionalSpawnData, IReflectable {
     ...
     * 
     * @param wobble amount of deviation from base trajectory, used by Skeletons and the like; set to 0.0F for no x/z
     *               deviation
     */
    public EntityArrowCustom(World world, EntityLivingBase shooter, EntityLivingBase target, float velocity,
        float wobble) {
    ...
    /**
     * Returns the shootingEntity, or if null, tries to get the shooting player from the world based on shooterName, if
     * available
     */
    ...
    protected void updatePosition() {
        ...
        for (rotationPitch = (float) (Math.atan2(motionY, (double) f) * 180.0D / Math.PI); rotationPitch
    ...
    protected void updateInAir() {
        ...
        List<MovingObjectPosition> mops = TargetUtils
            .checkForImpact(this.worldObj, this, getShooter(), 0.3D, this.ticksInAir >= 5, this.isPiercing());
        if (mops != null) {
            ...
            if (dmg > 0) {
                ...
                if (mop.entityHit.attackEntityFrom(getDamageSource(mop.entityHit), dmg)) {
                    if (mop.entityHit instanceof EntityLivingBase) {
                        ...
                        if (shootingEntity instanceof EntityPlayerMP && mop.entityHit != shootingEntity
    ...
    public void readSpawnData(ByteBuf buffer) {
        ...
        if (shooter instanceof EntityLivingBase) { // why check for EntityLivingBase when shootingEntity can be an
                                                   // Entity?
