...
public abstract class EntityMobThrowable extends EntityThrowable
    implements IEntityAdditionalSpawnData, IReflectableOrigin {
    ...
    /**
     * Constructs a throwable entity heading towards target's initial position with given velocity, with possible
     * abnormal trajectory;
     * 
     * @param wobble amount of deviation from base trajectory, used by Skeletons and the like; set to 0.0F for no x/z
     *               deviation
     */
    public EntityMobThrowable(World world, EntityLivingBase shooter, EntityLivingBase target, float velocity,
        float wobble) {
        super(world, shooter);
    ...
    public void onReflected(ItemStack shield, EntityPlayer player, DamageSource source) {
        ...
        this.originId = (this.getThrower() == null ? -1
            : this.getThrower()
                .getEntityId());
    ...
    public void writeSpawnData(ByteBuf buffer) {
        this.throwerId = (this.getThrower() == null ? -1
            : this.getThrower()
                .getEntityId());
