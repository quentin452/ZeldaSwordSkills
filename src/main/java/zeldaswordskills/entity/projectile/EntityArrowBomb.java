...
public class EntityArrowBomb extends EntityArrowCustom implements IEntityBomb {
    ...
    public EntityArrowBomb(World world, EntityLivingBase shooter, EntityLivingBase target, float velocity,
        float wobble) {
    ...
    protected DamageSource getDamageSource(Entity entity) {
        return new EntityDamageSourceIndirect("bomb arrow", this, shootingEntity).setProjectile()
            .setExplosion();
    }
    ...
    public void onUpdate() {
        super.onUpdate();
        if (!this.worldObj.isRemote && this.worldObj.provider.isHellWorld
            && this.ticksInAir > 0
            && this.explodesInHell()) {
    ...
    protected void onImpact(MovingObjectPosition mop) {
        ...
        boolean inFire = isBurning() || (material == Material.lava || material == Material.fire)
            || this.worldObj.func_147470_e(this.boundingBox);
