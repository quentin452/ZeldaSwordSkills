...
public class EntityArrowLight extends EntityArrowElemental {
    ...
    public EntityArrowLight(World world, EntityLivingBase shooter, EntityLivingBase target, float velocity,
        float wobble) {
    ...
    protected DamageSource getDamageSource(Entity entity) {
        if (entity instanceof EntityEnderman) {
            return new DamageSourceBaseDirect(
                "arrow.light",
                (shootingEntity != null ? shootingEntity : this),
                EnumDamageType.HOLY).setProjectile()
                    .setMagicDamage()
                    .setDamageBypassesArmor();
        }
        DamageSourceBaseIndirect source = new DamageSourceBaseIndirect(
            "arrow.light",
            this,
            shootingEntity,
            EnumDamageType.HOLY);
        ...
        // Witches have a hard-coded 85% magic damage reduction, plus the Buff; skip setting magic damage so they can be
        // killed
    ...
    protected void onImpactEntity(MovingObjectPosition mop) {
        ...
        if (this.canOneHitKill(mop.entityHit) && (!mop.entityHit.isEntityAlive()
            || (mop.entityHit instanceof EntityLivingBase && ((EntityLivingBase) mop.entityHit).getHealth() <= 0.0F))) {
            this.setDead();
        }
    }
    ...
    private boolean canOneHitKill(Entity entity) {
        ...
        return (!(entity instanceof IBossDisplayData))
            && (flag || entity instanceof EntityEnderman || entity instanceof EntityWitch);
    }
