    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) {
            ...
            for (EntityLivingBase target : targets) {
                if (!affectedEntities.contains(target.getEntityId()) && target != getThrower()
                    && !TargetUtils.isTargetInFrontOf(this, target, 30F)) {
        ...
        Block block = worldObj.getBlock(
            (int) (posX + (boundingBox.maxX - boundingBox.minX) / 2),
            (int) posY - 1,
            (int) (posZ + (boundingBox.maxZ - boundingBox.minZ) / 2));
        ...
        for (int i = 0; i < 4; ++i) {
            worldObj
                .spawnParticle(particle, posX, posY, posZ, vX + rand.nextGaussian(), 0.01D, vZ + rand.nextGaussian());
            worldObj
                .spawnParticle(particle, posX, posY, posZ, -vX + rand.nextGaussian(), 0.01D, -vZ + rand.nextGaussian());
        }
    }
    ...
    protected void onImpact(MovingObjectPosition mop) {
        if (!worldObj.isRemote) {
            if (mop.typeOfHit == MovingObjectType.ENTITY) {
                ...
                if (entity instanceof EntityLivingBase && !affectedEntities.contains(entity.getEntityId())
                    && entity != getThrower()) {
