...
public class EntityHookShot extends EntityThrowable {
    ...
    /**
     * Watchable objects for hookshot's impact position - prevents client from occasionally trying to go to 0,0,0 (due
     * to client values not being set)
     * Used instead of super's integer versions and set to inside the struck block to prevent the player's motion from
     * freaking out
     */
    ...
    protected DamageSource getDamageSource() {
        return new DamageSourceBaseIndirect("hookshot", this, getThrower()).setStunDamage(50, 1, true)
            .setProjectile();
    }
        ...
        switch (result) {
            case DEFAULT:
                switch (getType()) {
                    ...
                    case MULTI_SHOT:
                    case MULTI_SHOT_EXT:
                        return material == Material.wood || material == Material.rock
        ...
        switch (result) {
            case DEFAULT:
                ...
                if (getThrower() instanceof EntityPlayer) {
                    canPlayerEdit = ((EntityPlayer) getThrower()).capabilities.allowEdit
                        && Config.canHookshotBreakBlocks();
                }
                Material m = block.getMaterial();
                return (isBreakable && canPlayerEdit
                    && (m == Material.glass
                        || (m == Material.wood && getType().ordinal() / 2 == (HookshotType.CLAW_SHOT.ordinal() / 2))));
            default:
                return result == Result.ALLOW;
        }
    }
        ...
        if (mop.typeOfHit == MovingObjectType.BLOCK && getTarget() == null) {
            ...
            if (!isInGround() && ticksExisted < getMaxDistance()) {
                ...
                if (canGrabBlock(block, mop.blockX, mop.blockY, mop.blockZ, mop.sideHit)) {
                    setInGround(true);
                    // adjusting posX/Y/Z here seems to make no difference to the rendering, even when client side makes
                    // same changes
                    ...
                    switch (mop.sideHit) {
                        ...
                        case 4:
                            posX -= 0.515D;
                            break; // WEST (a little extra to compensate for block border, otherwise renders black)
                        case 3:
                            posZ += 0.5D;
                            break; // SOUTH
                        case 2:
                            posZ -= 0.515D;
                            break; // NORTH (a little extra to compensate for block border, otherwise renders black)
                        case SideHit.TOP:
            ...
            if (player != null && player.getCurrentArmor(ArmorIndex.WORN_BOOTS) != null
                && player.getCurrentArmor(ArmorIndex.WORN_BOOTS)
                    .getItem() == ZSSItems.bootsHeavy
                && player.isSneaking()) {
    ...
    public void onUpdate() {
        ...
        if (canUpdate()) {
            if ((ticksExisted > getMaxDistance() && !isInGround() && getTarget() == null)
                || ticksExisted > (getMaxDistance() * 8)) {
    ...
    protected boolean canUpdate() {
        ...
        if (!isDead && thrower instanceof EntityPlayer && ((EntityPlayer) thrower).isUsingItem()) {
            ...
            return stack.getItem() instanceof ItemHookShot
                && ((ItemHookShot) stack.getItem()).getType(stack.getItemDamage()) == getType();
        } else {
            return false;
        }
    }
    ...
    protected void pullThrower() {
        ...
        if (thrower != null && isInGround()) {
            ...
            double d = thrower.getDistanceSq(
                dataWatcher.getWatchableObjectFloat(HIT_POS_X),
                y,
                dataWatcher.getWatchableObjectFloat(HIT_POS_Z));
            if (!reachedHook) {
            ...
            if (reachedHook && thrower.isSneaking()) {
                ...
                double dy = 0.15D
                    * (dataWatcher.getWatchableObjectFloat(HIT_POS_Y) + (this.height / 3.0F) - thrower.posY + 0.05D);
    ...
    protected void pullTarget() {
        ...
        if (target != null && thrower != null) {
            if (target instanceof EntityLivingBase) {
                ZSSEntityInfo.get((EntityLivingBase) target)
                    .removeBuff(Buff.STUN);
            }
            ...
            if (reachedHook && d < 9.0D) {
                ...
                if (target instanceof EntityLivingBase) {
                    double resist = 1.0D
                        - ((EntityLivingBase) target).getEntityAttribute(SharedMonsterAttributes.knockbackResistance)
                            .getAttributeValue();
                    dx *= resist;
                    dy *= resist;
                    dz *= resist;
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        ...
        dataWatcher.updateObject(
            SHOTTYPE_INDEX,
            HookshotType.values()[compound.getByte("shotType") % HookshotType.values().length]);
