...
public class EntityMagicSpell extends EntityMobThrowable {
    ...
    protected DamageSource getDamageSource() {
        ...
        switch (type) {
            case ICE:
                source = new DamageSourceIceIndirect("blast.ice", this, this.getThrower(), 50, 1, true)
                    .setStunDamage(60, 10, true);
                break;
            case LIGHTNING:
                source = new DamageSourceShockIndirect("blast.lightning", this, this.getThrower(), 50, 1, true);
                break;
            case WATER:
                source = new DamageSourceBaseIndirect(
                    "blast.water",
                    this,
                    this.getThrower(),
                    true,
        ...
        if (worldObj.isRemote && spawnParticles) {
            ...
            for (int i = 0; i < 4; ++i) {
                worldObj.spawnParticle(
                    ...
                    -motionX * 0.25D,
                    -motionY + (flag ? 0.1D : 0.0D),
                    -motionZ * 0.25D);
            }
        } else if (ticksExisted % type.getSoundFrequency() == 0) {
            worldObj
                .playSoundAtEntity(this, type.getMovingSound(), type.getSoundVolume(rand), type.getSoundPitch(rand));
        }
    }
    ...
    protected void onImpact(MovingObjectPosition mop) {
        ...
        if (mop.entityHit instanceof EntityLivingBase) {
            if (mop.entityHit.attackEntityFrom(getDamageSource().setProjectile(), getDamage())
    ...
    private void spawnImpactParticles(String particle, int n, float offsetY) {
        for (int i = 0; i < n; ++i) {
            ...
            worldObj.spawnParticle(
                particle,
                (dx + rand.nextFloat() - 0.5F),
                (dy + rand.nextFloat() - 0.5F),
                (dz + rand.nextFloat() - 0.5F),
                0.25F * (rand.nextFloat() - 0.5F),
                (rand.nextFloat() * 0.25F) + offsetY,
                0.25F * (rand.nextFloat() - 0.5F));
        }
    }
