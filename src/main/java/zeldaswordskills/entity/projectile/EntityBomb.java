...
public class EntityBomb extends EntityMobThrowable implements IEntityBombIngestible {
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote && ticksExisted > 5 && wasBombEaten()) {
            ...
            inFire = inFire || (material == Material.lava || material == Material.fire)
                || worldObj.func_147470_e(boundingBox);
            if (isDud(inFire)) {
                this.addTime(10);
                disarm(worldObj);
    ...
    private boolean wasBombEaten() {
        List<EntityLivingBase> entities = worldObj
            .getEntitiesWithinAABB(EntityLivingBase.class, boundingBox.expand(0.5D, 0.5D, 0.5D));
        for (EntityLivingBase entity : entities) {
            if (!entity.isEntityAlive()) {
                continue;
            }
            Result result = (entity instanceof IEntityBombEater ? ((IEntityBombEater) entity).ingestBomb(this)
                : Result.DENY);
            if (result == Result.ALLOW) {
                return true;
            } else if (result == Result.DEFAULT && ZSSEntityInfo.get(entity)
                .onBombIngested(this)) {
                    return true;
                }
        }
    ...
    public boolean disarm(World world) {
        if (!world.isRemote) {
            if (isEntityAlive() && fuseTime > 4) {
                setDead();
                EntityItem bomb = new EntityItem(
                    world,
                    posX,
                    posY,
                    posZ,
                    new ItemStack(ZSSItems.bomb, 1, getType().ordinal()));
    ...
    private boolean isDud(boolean inFire) {
        switch (getType()) {
            case BOMB_WATER:
                return inFire || worldObj.provider.isHellWorld;
            default:
                return (worldObj.getBlock((int) posX, (int) posY, (int) posZ)
