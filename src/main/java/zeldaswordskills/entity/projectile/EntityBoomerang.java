...
public class EntityBoomerang extends EntityMobThrowable {
    ...
    public EntityBoomerang(World world, EntityLivingBase shooter, EntityLivingBase target, float velocity,
        float wobble) {
    ...
    protected DamageSource getDamageSource() {
        return new DamageSourceBaseIndirect("boomerang", this, getThrower()).setStunDamage(200, 5, true)
            .setProjectile();
    }
    ...
    protected void updateMotion() {
        ...
        if (target != null) {
             ...
             * motionX = (double)(-MathHelper.sin(rotationYaw / 180.0F * (float) Math.PI) * MathHelper.cos(rotationPitch
             * / 180.0F * (float) Math.PI) * f);
             * motionZ = (double)(MathHelper.cos(rotationYaw / 180.0F * (float) Math.PI) * MathHelper.cos(rotationPitch
             * / 180.0F * (float) Math.PI) * f);
    ...
    protected void onImpact(MovingObjectPosition mop) {
        if (mop.typeOfHit == MovingObjectType.ENTITY) {
            ...
            if (block instanceof IBoomerangBlock) {
                flag = ((IBoomerangBlock) block)
                    .onBoomerangCollided(worldObj, mop.blockX, mop.blockY, mop.blockZ, this);
            } else {
                ...
                if (Config.canBoomerangDenude() && !worldObj.isRemote
    ...
    private void captureDrops() {
        if (riddenByEntity == null || captureAll) {
            List<EntityItem> items = worldObj
                .getEntitiesWithinAABB(EntityItem.class, boundingBox.expand(1.0D, 1.0D, 1.0D));
    ...
    private void captureXpOrbs() {
        List<EntityXPOrb> orbs = worldObj
            .getEntitiesWithinAABB(EntityXPOrb.class, boundingBox.expand(1.0D, 1.0D, 1.0D));
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        ...
        dataWatcher
            .updateObject(ITEM_DATAWATCHER_INDEX, ItemStack.loadItemStackFromNBT(compound.getCompoundTag("item")));
        slot = compound.getInteger("invSlot");
        dataWatcher
            .updateObject(TARGET_DATAWATCHER_INDEX, compound.hasKey("target") ? compound.getInteger("target") : -1);
