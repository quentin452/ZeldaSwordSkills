...
public class ZSSPlayerSkills {
    ...
    private static final AttributeModifier hardcoreHeartModifier = (new AttributeModifier(
        hardcoreHeartUUID,
        "Hardcore Zelda Hearts",
        -14.0D,
        0)).setSaved(true);
    ...
    public byte getSkillLevel(byte id) {
        return (skills.containsKey(id) ? skills.get(id)
            .getLevel() : 0);
    }
    ...
    public boolean onKeyPressed(Minecraft mc, KeyBinding key) {
        ...
        // give precedence to currently active skill? except those that need it (e.g. ArmorBreak) may not yet be set as
        // the active skill
        /*
    ...
    public void giveCrest() {
        ...
        } else if (getSkillLevel(SkillBase.superSpinAttack)
            < Math.min(crestsGiven / 20, SkillBase.superSpinAttack.getMaxLevel())) {
                PlayerUtils.sendTranslatedChat(player, "chat.zss.npc.orca.unfit." + player.worldObj.rand.nextInt(4));
            } else if (getSkillLevel(SkillBase.backSlice)
                < Math.min((crestsGiven + 10) / 20, SkillBase.backSlice.getMaxLevel())) {
                    PlayerUtils
                        .sendTranslatedChat(player, "chat.zss.npc.orca.unfit." + player.worldObj.rand.nextInt(4));
                } else if (crestsGiven >= 100) {
                    PlayerUtils
                        .sendTranslatedChat(player, "chat.zss.npc.orca.master." + player.worldObj.rand.nextInt(4));
                } else if (PlayerUtils
                    .consumeInventoryItem(player, ZSSItems.treasure, Treasures.KNIGHTS_CREST.ordinal(), 1)) {
                        ++crestsGiven;
                        List<IChatComponent> chat = new ArrayList<IChatComponent>();
                        chat.add(
                            new ChatComponentTranslation(
                                "chat.zss.npc.orca.redeem." + player.worldObj.rand.nextInt(4)));
                        if (crestsGiven == 1) {
                            ...
                            new TimedAddItem(
                                player,
                                new ItemStack(ZSSItems.skillOrb, 1, SkillBase.backSlice.getId()),
                                (flag ? 4000 : 3000),
                                Sounds.SUCCESS);
                        } else {
                            chat.add(
                                new ChatComponentTranslation(
                                    "chat.zss.npc.orca.more." + player.worldObj.rand.nextInt(4),
                                    crestsGiven));
                        }
    ...
    public void syncClientSideSkill(byte id, NBTTagCompound compound) {
        if (SkillBase.doesSkillExist(id)) {
            SkillBase skill = SkillBase.getNewSkillInstance(id)
    ...
    public void onUpdate() {
        ...
        if (player.worldObj.isRemote) {
            if (ZSSKeyHandler.keys[ZSSKeyHandler.KEY_BLOCK].getIsKeyPressed() && isSkillActive(SkillBase.swordBasic)
