...
public class ZSSPlayerInfo implements IExtendedEntityProperties {
    ...
    public static final IAttribute maxMagic = (new RangedAttribute("zss.max_magic", 50.0D, 0.0D, Double.MAX_VALUE))
        .setDescription("Max Magic")
        .setShouldWatch(true);
    ...
    /**
     * [Hero's Bow] Stores the currently nocked arrow in order to avoid the graphical glitch caused by writing to the
     * stack's NBT
     */
    ...
    public float getMaxMagic() {
        return (float) player.getAttributeMap()
            .getAttributeInstance(maxMagic)
            .getAttributeValue();
    }
    ...
    private boolean useMagic(float amount, boolean consume) {
        if (player.capabilities.isCreativeMode || ZSSEntityInfo.get(player)
    ...
    public void onUpdate() {
        ...
        if (getFlag(MOBILITY) && !player.onGround
            && Math.abs(player.motionY) > 0.05D
            && !player.capabilities.isFlying
            && player.worldObj.getTotalWorldTime() % 2 == 0) {
            player.motionX *= 1.15D;
            player.motionZ *= 1.15D;
        }
        if (hasAutoBombArrow && (player.getHeldItem() == null || !(player.getHeldItem()
            .getItem() instanceof ItemHeroBow))) {
    ...
    public void copy(ZSSPlayerInfo info) {
        ...
        ZSSQuests.get(this.player)
            .copy(ZSSQuests.get(info.player));
    }
