...
public final class QuestZeldasLetter extends QuestBase {
    ...
    public boolean canComplete(EntityPlayer player, Object... data) {
        return !isComplete(player) && ZSSQuests.get(player)
            .hasBegun(QuestMaskShop.class);
    }
    ...
    protected boolean onComplete(EntityPlayer player, Object... data) {
        PlayerUtils.addItemToInventory(
            player,
            new ItemStack(ZSSItems.treasure, 1, ItemTreasure.Treasures.ZELDAS_LETTER.ordinal()));
    ...
    public boolean isComplete(EntityPlayer player) {
        if (!isset(FLAG_COMPLETE) && ZSSQuests.get(player)
    ...
    public IChatComponent getHint(EntityPlayer player, Object... data) {
        ...
        if (!isComplete(player) && !quests.hasBegun(QuestMaskShop.class)
            && quests.hasCompleted(QuestPendants.class)
            && rand.nextInt(8) < 3) {
