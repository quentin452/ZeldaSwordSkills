...
public final class QuestCursedMan extends QuestBase {
    ...
    public boolean canComplete(EntityPlayer player, Object... data) {
        if (this.tokens < MAX_SKULLTULA_TOKENS) {
            return false;
        } else if (this.isComplete(player) && Config.getSkulltulaRewardRate() > 0
            && data != null
            && data[0] instanceof EntityVillager) {
                return player.worldObj.getTotalWorldTime() > ZSSVillagerInfo.get((EntityVillager) data[0])
                    .getNextSkulltulaReward();
            }
    ...
    protected boolean onComplete(EntityPlayer player, Object... data) {
        ...
        new TimedChatDialogue(
            player,
            new ChatComponentTranslation("chat.zss.npc.cursed_man.token." + this.tokens),
            new ChatComponentTranslation(
                "chat.zss.npc.cursed_man.reward." + this.tokens,
        ...
        if (Config.getSkulltulaRewardRate() > 0) {
            ZSSVillagerInfo.get((EntityVillager) data[0])
                .setNextSkulltulaReward(
                    player.worldObj.getTotalWorldTime() + (24000 * Config.getSkulltulaRewardRate()));
        }
        return true;
    }
    ...
    public boolean update(EntityPlayer player, Object... data) {
        ...
        switch (this.tokens) {
            ...
            case 50:
                ...
                for (SkillBase skill : SkillBase.getSkills()) {
                    if (skill.getId() != SkillBase.bonusHeart.getId()
                        && skills.getSkillLevel(skill) < skill.getMaxLevel()
                        && (reward == null || player.worldObj.rand.nextInt(4) == 0)) {
            ...
            default:
                PlayerUtils.sendTranslatedChat(player, "chat.zss.npc.cursed_man.amount", this.tokens);
        }
        if (reward != null) {
            new TimedChatDialogue(
                player,
