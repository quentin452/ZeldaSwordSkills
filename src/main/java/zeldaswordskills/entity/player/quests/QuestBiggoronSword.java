...
public final class QuestBiggoronSword extends QuestBase {
    ...
    private static final ImmutableSet<TradeData> TRADES = new ImmutableSet.Builder<TradeData>()
        .add(
            new TradeData(
                Treasures.TENTACLE,
                new VillagerDescription("Talon", EntityVillager.class, EnumVillager.FARMER.ordinal(), true),
                getTreasure(Treasures.POCKET_EGG)))
        .add(
            new TradeData(
                Treasures.POCKET_EGG,
                new VillagerDescription("Cucco Lady", EntityVillager.class, EnumVillager.FARMER.ordinal()),
                getTreasure(Treasures.COJIRO)))
        .add(
            new TradeData(
                Treasures.COJIRO,
                new VillagerDescription("Grog", EntityVillager.class, EnumVillager.BUTCHER.ordinal()),
                getTreasure(Treasures.ODD_MUSHROOM)))
        .add(
            new TradeData(
                Treasures.ODD_MUSHROOM,
                new VillagerDescription("Old Hag", EntityVillager.class, EnumVillager.LIBRARIAN.ordinal()),
                getTreasure(Treasures.ODD_POTION)))
        .add(
            new TradeData(
                Treasures.ODD_POTION,
                new VillagerDescription("Grog", EntityVillager.class, EnumVillager.BUTCHER.ordinal()),
                getTreasure(Treasures.POACHER_SAW)))
        .add(
            new TradeData(
                Treasures.POACHER_SAW,
                new VillagerDescription("Mutoh", EntityVillager.class, EnumVillager.BLACKSMITH.ordinal()),
                getTreasure(Treasures.GORON_SWORD)))
        .add(
            new TradeData(
                Treasures.GORON_SWORD,
                new VillagerDescription("Biggoron", EntityGoron.class, EnumVillager.BLACKSMITH.ordinal()),
                getTreasure(Treasures.PRESCRIPTION)))
        .add(
            new TradeData(
                Treasures.PRESCRIPTION,
                new VillagerDescription("King Zora", EntityVillager.class, EnumVillager.PRIEST.ordinal()),
                getTreasure(Treasures.EYEBALL_FROG)))
        .add(
            new TradeData(
                Treasures.EYEBALL_FROG,
                new VillagerDescription("Lake Scientist", EntityVillager.class, EnumVillager.LIBRARIAN.ordinal()),
                getTreasure(Treasures.EYE_DROPS)))
        .add(
            new TradeData(
                Treasures.EYE_DROPS,
                new VillagerDescription("Biggoron", EntityGoron.class),
                getTreasure(Treasures.CLAIM_CHECK)))
        .add(
            new TradeData(
                Treasures.CLAIM_CHECK,
                new VillagerDescription("Biggoron", EntityGoron.class),
                new ItemStack(ZSSItems.swordBiggoron)))
        .build();
    ...
    private TradeData getCurrentTrade() {
        return (tradeIndex < TRADES.size() ? TRADES.asList()
            .get(tradeIndex) : null);
    }
    ...
    public boolean update(EntityPlayer player, Object... data) {
        ...
        if (trade != null && trade.villager.matches((Entity) data[0])
            && trade.treasure == Treasures.byDamage(stack.getItemDamage())) {
    ...
    private void rewardAndChat(EntityPlayer player, TradeData trade, boolean first) {
        ...
        if (first) {
            ...
            player.addChatMessage(
                new ChatComponentTranslation("chat.zss.treasure." + trade.treasure.name + ".give.repeat"));
        ...
        chat.add(
            new ChatComponentTranslation(
                "chat.zss.treasure.generic.received",
                new ChatComponentTranslation(reward.getUnlocalizedName() + ".name")));
    ...
    private boolean checkClaim(ItemStack stack, EntityPlayer player) {
        ...
        if (!stack.getTagCompound()
            ...
            stack.getTagCompound()
                .setLong("finishDate", player.worldObj.getTotalWorldTime() + 48000);
            return false;
        } else if (player.worldObj.getTotalWorldTime() < stack.getTagCompound()
            .getLong("finishDate")) {
    ...
    private boolean onCompletedTrade(EntityPlayer player, TradeData trade, ItemStack reward) {
        switch (trade.treasure) {
            case TENTACLE:
                ...
                return false;
            case POCKET_EGG:
                player.triggerAchievement(ZSSAchievements.treasureSecond);
                return false;
    ...
    public IChatComponent getHint(EntityPlayer player, Object... data) {
        ...
        boolean isChild = (entity instanceof EntityAgeable && ((EntityAgeable) entity).isChild()); // set later as a
                                                                                                   // flag
        ...
        ListIterator<TradeData> iterator = TRADES.asList()
            .listIterator(index);
        for (int i = index - 1; iterator.hasPrevious(); --i) { // subtract 1 to compensate for call to #previous
            ...
            if (trade.villager.matches(entity, false) && (!isChild || trade.villager.isChild)) {
                ...
                if (i + 1 == tradeIndex && trade.reward.getItem() instanceof ItemTreasure
                    && treasure == Treasures.byDamage(trade.reward.getItemDamage())) {
                    // matched entity from previous stage while holding the reward from that stage - gives hint for
                    // 'next' (i.e. current) stage
                    return new ChatComponentTranslation(
                        "chat.zss.treasure." + trade.treasure.name + ".hint.next" + (exactMatch ? "" : ".ageable"));
                    ...
                    return new ChatComponentTranslation(
                        "chat.zss.treasure." + trade.treasure.name + ".hint.current" + (exactMatch ? "" : ".ageable"));
                    ...
                    return new ChatComponentTranslation(
                        "chat.zss.treasure." + trade.treasure.name
                            + ".hint.previous"
                            + (treasure == iterator.previous().treasure ? ".match" : ""));
                    ...
                    return new ChatComponentTranslation(
                        "chat.zss.treasure." + trade.treasure.name + ".thanks" + (exactMatch ? "" : ".ageable"));
                }
                return null; // always terminate loop once an exact match is found
            } else if (trade.treasure == treasure && trade.villager.matchClassAndName(entity)
                && !trade.villager.matchProfession(entity)) {
                    ...
                    return new ChatComponentTranslation(
                        "chat.zss.treasure.generic.profession." + (isChild ? "child" : ev.unlocalizedName),
                        new ChatComponentTranslation("entity.villager.profession." + ev.unlocalizedName));
                } else if (trade.treasure == treasure && entity instanceof EntityVillager
                    && checkVillagerTrade(stack, player, (EntityVillager) entity, isLeftClick)) {
                        return new ChatComponentTranslation(""); // empty chat to prevent further action (e.g. to stop
                                                                 // villager trading GUI from opening)
                        ...
                        if (rand.nextInt(8) < 3) {
                            chat = new ChatComponentTranslation(
                                "chat.zss.treasure." + trade.treasure.name
                                    + (isLeftClick ? ".give" : ".show")
                                    + ".random");
                        } else {
                            chat = new ChatComponentTranslation(
                                "chat.zss.treasure.generic." + (isLeftClick ? "give." : "show.") + rand.nextInt(4));
                        }
                    }
        }
        return chat;
    ...
    private boolean checkVillagerTrade(ItemStack stack, EntityPlayer player, EntityVillager villager,
        boolean isLeftClick) {
        Treasures treasure = Treasures.byDamage(stack.getItemDamage());
        return treasure.canSell()
            && ((ItemTreasure) stack.getItem()).handleVillagerTrade(stack, player, villager, isLeftClick);
    }
