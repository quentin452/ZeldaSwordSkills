...
public abstract class QuestBase implements IQuest {
     ...
     * @param handler IQuestHandler will have its methods called at appropriate times; pass
     *                {@link #DEFAULT_QUEST_HANDLER} if none available
     * @param data    Optional arguments passed to the various {@code IQuest} methods, e.g. {@link IQuest#begin begin}
     *                and {@link IQuest#complete complete}
    ...
    public static boolean checkQuestProgress(EntityPlayer player, IQuest quest, IQuestHandler handler, Object... data) {
        ...
        if (!changed) { // check for hint
            ...
            if (hint != null) { // result may be different on client vs. server due to Random
                if (!player.worldObj.isRemote && !hint.getUnformattedText()
                    .equals("")) {
