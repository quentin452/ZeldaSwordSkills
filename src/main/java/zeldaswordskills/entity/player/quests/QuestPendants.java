...
public final class QuestPendants extends QuestBase {
    ...
    public boolean canBegin(EntityPlayer player) {
        return super.canBegin(player) && ZSSQuests.get(player)
            .hasCompleted(QuestZeldaTalk.class);
    }
                ...
                if (!isComplete(player)) {
                    new TimedChatDialogue(
                        ...
                        new ChatComponentTranslation(
                            "chat.zss.npc.zelda.pendant.given." + pendant.unlocalizedName + ".0"),
                        new ChatComponentTranslation(
                            "chat.zss.npc.zelda.pendant.given." + pendant.unlocalizedName + ".1"),
                        new ChatComponentTranslation(
                            "chat.zss.npc.zelda.pendant.given." + pendant.unlocalizedName + ".2"));
                }
            ...
            for (ItemPendant.PendantType pendant : QuestPendants.pendants) {
                if (!isset(pendant.bitFlag)) {
                    return new ChatComponentTranslation(
                        "chat.zss.npc.zelda.pendant.hint." + pendant.unlocalizedName + "." + rand.nextInt(3));
                }
            }
        }
        return null;
