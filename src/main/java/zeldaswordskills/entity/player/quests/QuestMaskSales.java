...
public final class QuestMaskSales extends QuestBase {
    ...
    public static final ImmutableSet<Item> MASKS = new ImmutableSet.Builder<Item>().add(ZSSItems.maskKeaton)
    ...
    public boolean canBegin(EntityPlayer player) {
        return super.canBegin(player) && ZSSQuests.get(player)
            .hasCompleted(QuestMaskShop.class);
    }
    ...
    protected boolean onComplete(EntityPlayer player, Object... data) {
        ...
        ZSSQuests.get(player)
            .setBorrowedMask(ZSSItems.maskTruth);
    ...
    public boolean update(EntityPlayer player, Object... data) {
        ...
        if (data == null || data.length < 1) {
            ...
            switch (currentStage % NUM_STAGES) {
                ...
                case 1: // still need to sell mask
                    PlayerUtils.sendTranslatedChat(
                        player,
                        "chat.zss.npc.mask_salesman.sales.pep." + player.worldObj.rand.nextInt(4));
                    return false;
            ...
            if (hasSold(sell)) { // already sold this mask
                PlayerUtils.sendTranslatedChat(
                    player,
                    "chat.zss.npc.mask_salesman.sales.repeat." + (data.length > 1 ? "pushy." : "")
                        + player.worldObj.rand.nextInt(4));
                ...
                PlayerUtils.sendTranslatedChat(
                    player,
                    "chat.zss.npc.mask_salesman.sales.sold." + player.worldObj.rand.nextInt(4));
                player.triggerAchievement(ZSSAchievements.maskSold);
            } else {
                PlayerUtils.sendTranslatedChat(
                    player,
                    "chat.zss.npc.mask_salesman.sales.later." + player.worldObj.rand.nextInt(4));
            }
            return true;
        }
        return false;
