...
public class ZSSPlayerSongs {
    ...
    public void setHorseRidden(EntityHorse horse) {
        ...
        } else if (horse.isTame() && horse.func_152119_ch()
            .equals(
                player.getUniqueID()
                    .toString())) {
    ...
    public void addLonLonCow(EntityCow cow) {
        ...
        if (!cow.isChild()
            && (!data.hasKey("zss_lon_milk") || cow.worldObj.getWorldTime() > data.getLong("zss_lon_milk"))) {
            lonlonCows.put(cow.getEntityId(), cow.worldObj.getWorldTime() + 6000); // 5 minutes to milk the cow, using
                                                                                   // regular world time
    ...
    public boolean milkLonLonCow(EntityPlayer player, EntityCow cow) {
        boolean match = false;
        Iterator<Entry<Integer, Long>> iterator = lonlonCows.entrySet()
            .iterator();
