...
public class EntityAIPowerAttack extends EntityAIBase {
    ...
    public boolean continueExecuting() {
        return (target != null && target.isEntityAlive()
            && chargeTimer > 0
            && attacker.onGround
            && attacker.getDistanceSqToEntity(target) < (rangeSq * 2.25F));
    }

    @Override
    public void updateTask() {
        attacker.getLookHelper()
        ...
        attacker.getMoveHelper()
            .setMoveTo(target.posX, target.posY, target.posZ, 1.0D / 3.0D);
        if (chargeTimer > 0 && --chargeTimer == 0) {
            ...
            if (d <= reach && attacker.getEntitySenses()
                .canSee(target)) {
