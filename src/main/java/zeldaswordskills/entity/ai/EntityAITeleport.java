...
public class EntityAITeleport extends EntityAIBase {
     ...
     * @param delay     The minimum time before the entity will next attempt to teleport randomly or to the current
     *                  target
     ...
     */
    public EntityAITeleport(EntityCreature entity, double teleRange, int delay, boolean grounded, boolean random,
        boolean approach, boolean flee, boolean hurt) {
        this.entity = entity;
    ...
    public void updateTask() {
        ...
        if (triggerTimer > 0 && --triggerTimer == 0) {
            ...
            flag = true;
        } else if (fleeTele && delayTimer > (teleportDelay / 2)
            && target != null
            && entity.getDistanceSqToEntity(target) < range) {
                flag = true;
                ...
                if (teleBounds == null
                    || teleBounds.isVecInside(Vec3.createVectorHelper(target.posX, target.posY, target.posZ))) {
    ...
    public void onPostTeleport(double originX, double originY, double originZ) {
        teleportDelay = minTeleportDelay + entity.worldObj.rand.nextInt(minTeleportDelay * 2)
            - entity.worldObj.rand.nextInt((minTeleportDelay / 2) + 1);
        delayTimer = 0;
        triggerTimer = 0;
        isTeleporting = false;
    }
     ...
     * 
     * @param range       The maximum range of the teleport; y range is half this value
     * @param restriction Optional bounding box defining teleportation borders - entity will not teleport outside these
     *                    bounds
     * @param grounded    True to require entity to land upon solid ground when teleporting
     * @return True if the entity successfully teleported
     */
    public static boolean teleportRandomly(World world, EntityLivingBase entity, double range,
        AxisAlignedBB restriction, boolean grounded) {
    ...
    public static boolean teleportToEntity(World world, EntityLivingBase entity, Entity target,
        ...
        Vec3 vec3 = Vec3.createVectorHelper(
            ...
            entity.boundingBox.minY + (double) (entity.height / 2.0F) - target.posY + (double) target.getEyeHeight(),
     ...
     */
    public static boolean teleportTo(World world, EntityLivingBase entity, double x, double y, double z,
        AxisAlignedBB restriction, boolean grounded, boolean noLiquid) {
        ...
        if (entity.worldObj.blockExists(i, j, k)) {
            ...
            if (foundSolidBlock) {
                ...
                if (restriction != null
                    && !restriction.isVecInside(Vec3.createVectorHelper(entity.posX, entity.posY, entity.posZ))) {
                    flag = false;
                } else if (world.getCollidingBoundingBoxes(entity, entity.boundingBox)
                    .isEmpty() && (!noLiquid || !world.isAnyLiquid(entity.boundingBox))) {
                        flag = true;
                    }
            }
        }
        ...
        if (!flag) {
            ...
            for (int l = 0; l < 128; ++l) {
                ...
                double d7 = d3 + (entity.posX - d3) * d6
                    + (world.rand.nextDouble() - 0.5D) * (double) entity.width * 2.0D;
                double d8 = d4 + (entity.posY - d4) * d6 + world.rand.nextDouble() * (double) entity.height;
                double d9 = d5 + (entity.posZ - d5) * d6
                    + (world.rand.nextDouble() - 0.5D) * (double) entity.width * 2.0D;
    ...
    public static void disruptTargeting(EntityLivingBase entity) {
        if (entity.worldObj instanceof WorldServer) {
            Set<EntityPlayer> players = ((WorldServer) entity.worldObj).getEntityTracker()
                .getTrackingPlayers(entity);
            for (EntityPlayer player : players) {
                ZSSPlayerSkills skills = ZSSPlayerSkills.get(player);
                if (skills.getTargetingSkill() != null && skills.getTargetingSkill()
                    .getCurrentTarget() == entity) {
                    skills.getTargetingSkill()
                        .setCurrentTarget(player, null);
                }
            }
        }
    }
    ...
    public static class PostEnderTeleport extends EnderTeleportEvent {
        ...
        /**
         * Post teleport event with the original position of the entity; the entity has already been set to its new
         * position.
         */
        public PostEnderTeleport(EntityLivingBase entity, double originX, double originY, double originZ,
            float damage) {
