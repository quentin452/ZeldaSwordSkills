...
public class EntityEtherLightning extends EntityLightningBolt {
    ...
    public EntityEtherLightning(World world, EntityLivingBase thrower, EntityLivingBase target, double posX,
        ...
        if (!world.isRemote && world.getGameRules()
            ...
            && (world.difficultySetting == EnumDifficulty.NORMAL || world.difficultySetting == EnumDifficulty.HARD)
    ...
    public void onUpdate() {
        ...
        if (lightningState == 2) {
            worldObj
                .playSoundEffect(posX, posY, posZ, "ambient.weather.thunder", 10000.0F, 0.8F + rand.nextFloat() * 0.2F);
        ...
        if (lightningState >= 0) {
            if (worldObj.isRemote) {
                ...
                int time = 600
                    + (strength > 0 || !target.onGround ? rand.nextInt(10) + rand.nextInt(Math.max(1, strength)) + 10
                        : 0);
                DamageSource source = new DamageSourceBaseIndirect(
                    "lightningBolt",
                    this,
                    this.thrower,
                    true,
                    EnumDamageType.SHOCK).setMagicDamage()
    ...
    private void extinguishFire(World world) {
        ...
        for (int i = -1; i < 2; ++i) {
            for (int j = -1; j < 2; ++j) {
                for (int k = -1; k < 2; ++k) {
                    if (world.getBlock(dx + i, dy + j, dz + k) == Blocks.fire) {
