...
public class ZSSEntityInfo implements IExtendedEntityProperties {
    ...
    public void removeAllBuffs(boolean removeAll, boolean sendUpdate) {
        if (!entity.worldObj.isRemote) {
            Iterator<Buff> iterator = activeBuffs.keySet()
                .iterator();
            while (iterator.hasNext()) {
                Buff buff = iterator.next();
                if (removeAll || !activeBuffs.get(buff)
                    .isPermanent()) {
                    activeBuffs.get(buff)
                        .onRemoved(entity, sendUpdate);
                    iterator.remove();
                }
            }
        }
    ...
    protected void updateBuffs() {
        Iterator<BuffBase> iterator = activeBuffs.values()
            .iterator();
        while (iterator.hasNext()) {
    ...
    public boolean isImmuneToStun() {
        ...
        return ((entity instanceof EntityPlayer && !Config.canPlayersBeStunned())
    ...
    private void onBombIndigestion() {
        ...
        if (explode) {
            ...
            CustomExplosion
                .createExplosion(ingestedBomb, entity.worldObj, entity.posX, entity.posY, entity.posZ, r, dmg, false);
        }
        ingestedBomb = null;
    }
