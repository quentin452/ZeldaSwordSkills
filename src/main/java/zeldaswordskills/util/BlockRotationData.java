...
public class BlockRotationData {
    ...
    public static enum Rotation {
         ...
         * Facings: 2 - north, 3 - south, 4 - west, 5 - east [for ladders and signs, they are attached to that side of
         * the block]
         */
        PISTON_CONTAINER,
        QUARTZ,
    ...
    public static final boolean registerCustomBlockRotation(Block block, Rotation rotationType, boolean override) {
        if (blockRotationData.containsKey(block)) {
            ZSSMain.logger.warn(
                "Block " + block + " already has a rotation type." + (override ? " Overriding previous data." : ""));
            if (override) {
