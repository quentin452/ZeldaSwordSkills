...
public class PlayerUtils {
    ...
    /**
     * Returns whether the player is blocking with the currently held item, accounting for possibility of Battlegear2
     * offhand shield use
     */
    ...
    /**
     * Returns whether the player is blocking with a shield, accounting for possibility of Battlegear2 offhand shield
     * use
     */
    ...
    public static boolean isHoldingWeapon(EntityPlayer player) {
        AttributeModifier itemDamage = player.getAttributeMap()
            .getAttributeInstance(SharedMonsterAttributes.attackDamage)
            .getModifier(itemDamageUUID);
    ...
    public static boolean isHoldingZeldaSword(EntityLivingBase entity) {
        return (entity.getHeldItem() != null && entity.getHeldItem()
            .getItem() instanceof ItemZeldaSword);
    }
    ...
    public static boolean hasMasterSword(EntityPlayer player) {
        for (ItemStack stack : player.inventory.mainInventory) {
            if (stack != null && stack.getItem() instanceof ItemZeldaSword
                && ((ItemZeldaSword) stack.getItem()).isMasterSword()) {
                return true;
            }
        }
        return false;
    ...
    public static void addItemToInventory(EntityPlayer player, ItemStack stack) {
        ...
        } else if (player instanceof EntityPlayerMP) { // ensure client side notified of inventory change (there are
                                                       // times this method only gets called server-side)
     ...
     * @return True if the entire amount was consumed; if this is not possible, no items are consumed and it returns
     *         false
     */
