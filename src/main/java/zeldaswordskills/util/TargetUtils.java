...
public class TargetUtils {
     ...
     */
    public static MovingObjectPosition checkForImpact(World world, Entity entity, Entity shooter, double hitBox,
        boolean canHitShooter) {
        List<MovingObjectPosition> mops = TargetUtils
            .checkForImpact(world, entity, shooter, hitBox, canHitShooter, false);
        return (mops != null && mops.size() > 0 ? mops.get(0) : null);
    }

    /**
     * Returns one or all MovingObjectPositions (any number of entities and up to one block), or null if nothing was
     * struck.
     * 
     ...
     * @param hitBox        The amount by which to expand the collided entities' bounding boxes when checking for impact
     *                      (may be negative)
     ...
     */
    public static List<MovingObjectPosition> checkForImpact(World world, Entity entity, Entity shooter, double hitBox,
        boolean canHitShooter, boolean allTargets) {
        ...
        vec31 = Vec3.createVectorHelper(
            entity.posX + entity.motionX,
            entity.posY + entity.motionY - entity.yOffset,
            entity.posZ + entity.motionZ);
        if (mop != null) {
        ...
        List<Entity> list = world.getEntitiesWithinAABBExcludingEntity(
            entity,
            entity.boundingBox.addCoord(entity.motionX, entity.motionY, entity.motionZ)
    ...
    private static MovingObjectPosition validateEntityMop(MovingObjectPosition mop, Entity shooter) {
        if (mop != null && mop.entityHit instanceof EntityPlayer) {
            ...
            if (player.capabilities.disableDamage
                || (shooter instanceof EntityPlayer && !((EntityPlayer) shooter).canAttackPlayer(player))) {
                return null;
            }
        }
        return mop;
    ...
    /**
     * Returns the EntityLivingBase closest to the point at which the seeker is looking and within the distance and
     * radius specified
     */
    ...
    /**
     * Returns the EntityLivingBase closest to the point at which the entity is looking and within the distance and
     * radius specified
     * 
     ...
     * @param closestToEntity if true, the target closest to the seeker and still within the line of sight search radius
     *                        is returned
     ...
     */
    public static final EntityLivingBase acquireLookTarget(EntityLivingBase seeker, int distance, double radius,
        boolean closestToSeeker, Class<?> targetClass) {
        ...
        while ((int) distanceTraveled < distance) {
            ...
            List<EntityLivingBase> list = seeker.worldObj.getEntitiesWithinAABB(
            ...
            for (EntityLivingBase target : list) {
                if (target != seeker && target.canBeCollidedWith() && isTargetInSight(vec3, seeker, target)) {
                    double newDistance = (closestToSeeker ? target.getDistanceSqToEntity(seeker)
                        : target.getDistanceSq(targetX, targetY, targetZ));
                    boolean closer = (newDistance < currentDistance);
                    if (closer
                        || (!foundClass && targetClass != null && targetClass.isAssignableFrom(target.getClass()))) {
     ...
     */
    public static final List<EntityLivingBase> acquireAllLookTargets(EntityLivingBase seeker, int distance,
        double radius) {
        if (distance < 0 || distance > MAX_DISTANCE) {
            distance = MAX_DISTANCE;
        }
    ...
    public static final boolean isTargetInFrontOf(Entity seeker, Entity target, float fov) {
        ...
        double dz;
        for (dz = target.posZ - seeker.posZ; dx * dx + dz * dz
            < 1.0E-4D; dz = (Math.random() - Math.random()) * 0.01D) {
    ...
    public static void setEntityHeading(Entity entity, double vecX, double vecY, double vecZ, float velocity,
        ...
        vecY /= vectorLength;
        vecZ /= vectorLength;
        vecX += entity.worldObj.rand.nextGaussian() * (entity.worldObj.rand.nextBoolean() ? -1 : 1)
            * 0.007499999832361937D
            * wobble;
        vecY += entity.worldObj.rand.nextGaussian() * (entity.worldObj.rand.nextBoolean() ? -1 : 1)
            * 0.007499999832361937D
            * wobble;
        vecZ += entity.worldObj.rand.nextGaussian() * (entity.worldObj.rand.nextBoolean() ? -1 : 1)
            * 0.007499999832361937D
            * wobble;
        vecX *= velocity;
        vecY *= velocity;
    ...
    public static boolean isOnTeam(EntityPlayer player, EntityLivingBase entity) {
        if (entity == player) {
            ...
            return ((IEntityOwnable) entity).func_152113_b()
    ...
    public static final void knockTargetBack(EntityLivingBase pushedEntity, EntityLivingBase pushingEntity) {
        if (pushedEntity.canBePushed()) {
            ...
            double dz;
            for (dz = pushedEntity.posZ - pushingEntity.posZ; dx * dx + dz * dz
                < 1.0E-4D; dz = (Math.random() - Math.random()) * 0.01D) {
                dx = (Math.random() - Math.random()) * 0.01D;
            }
