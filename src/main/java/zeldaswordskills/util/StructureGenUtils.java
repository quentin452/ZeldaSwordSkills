...
public class StructureGenUtils {
     ...
     */
    public static void fillWithBlocks(World world, StructureBoundingBox box, int minX, int maxX, int minY, int maxY,
        int minZ, int maxZ, Block block, int meta) {
     ...
     */
    public static void fillWithBlocks(World world, StructureBoundingBox box, int minX, int maxX, int minY, int maxY,
        int minZ, int maxZ, Block block, int meta, boolean ignoreBounds) {
     ...
     */
    public static void fillWithoutReplace(World world, int minX, int maxX, int minY, int maxY, int minZ, int maxZ,
        Block block, int meta, int flag) {
        for (int i = minX; i < maxX; ++i) {
            for (int j = minY; j < maxY; ++j) {
                for (int k = minZ; k < maxZ; ++k) {
    ...
    public static void fillDown(World world, StructureBoundingBox box, Block block, int metadata) {
        for (int i = box.minX; i <= box.maxX; ++i) {
            for (int k = box.minZ; k <= box.maxZ; ++k) {
                for (int j = box.minY - 1; j > 4 && (!world.getBlock(i, j, k)
                    .getMaterial()
                    .isSolid()
                    || world.getBlock(i, j, k)
                        .getMaterial() == Material.leaves); --j) {
     ...
     */
    public static void destroyBlocksAround(World world, int minX, int maxX, int minY, int maxY, int minZ, int maxZ,
        Block block, boolean drop) {
        for (int i = minX; i < maxX; ++i) {
            for (int j = minY; j < maxY; ++j) {
                for (int k = minZ; k < maxZ; ++k) {
     ...
     */
    public static void replaceMaterialWith(World world, int minX, int maxX, int minY, int maxY, int minZ, int maxZ,
        Material material, Block block, int meta) {
        for (int i = minX; i < maxX; ++i) {
            for (int j = minY; j < maxY; ++j) {
                for (int k = minZ; k < maxZ; ++k) {
                    if (world.getBlock(i, j, k)
     ...
     */
    public static void setBlockAtPosition(World world, StructureBoundingBox box, int x, int y, int z, Block block,
        int meta) {
     ...
     */
    public static void adjustCornersForMaterial(World world, StructureBoundingBox box, Material material, int n,
        boolean checkAbove, boolean moveUp) {
        int count = n;
        int i = (moveUp ? 1 : -1);

        while (count > 0 && world.getBlock(box.maxX, (checkAbove ? box.maxY + 1 : box.minY - 1), box.maxZ)
            .getMaterial() == material) {
            --count;
            box.offset(0, i, 0);
        }
        while (count > 0 && world.getBlock(box.maxX, (checkAbove ? box.maxY + 1 : box.minY - 1), box.minZ)
            .getMaterial() == material) {
            --count;
            box.offset(0, i, 0);
        }
        while (count > 0 && world.getBlock(box.minX, (checkAbove ? box.maxY + 1 : box.minY - 1), box.maxZ)
            .getMaterial() == material) {
            --count;
            box.offset(0, i, 0);
        }
        while (count > 0 && world.getBlock(box.minX, (checkAbove ? box.maxY + 1 : box.minY - 1), box.minZ)
            .getMaterial() == material) {
            --count;
            box.offset(0, i, 0);
        }
    ...
    public static void adjustForAir(World world, RoomBase room, StructureBoundingBox box) {
        ...
        while (maxShift > 0 && world.isAirBlock(box.getCenterX(), (shiftUp ? box.minY : box.maxY) - i, box.getCenterZ())
            && box.maxY < worldHeight
            && box.minY > 8) {
            --maxShift;
            box.offset(0, i, 0);
        }
     ...
     * @param minX, minY, and minZ are the lowest values to check - they must not be greater in value than the max
     *              values
     */
    ...
    public static int getNumBlocksOfMaterial(World world, StructureBoundingBox box, Material material, int offY) {
        ...
        count += (world.getBlock(box.getCenterX(), y, box.getCenterZ())
            .getMaterial() == material ? 1 : 0);
        count += (world.getBlock(box.maxX, y, box.maxZ)
            .getMaterial() == material ? 1 : 0);
        count += (world.getBlock(box.maxX, y, box.minZ)
            .getMaterial() == material ? 1 : 0);
        count += (world.getBlock(box.minX, y, box.maxZ)
            .getMaterial() == material ? 1 : 0);
        count += (world.getBlock(box.minX, y, box.minZ)
            .getMaterial() == material ? 1 : 0);
        return count;
    }
    ...
    public static int getNumBlocksOfMaterialInArea(World world, StructureBoundingBox box, Material material, int y) {
        if (y < 0) {
            return getNumBlocksOfMaterialInArea(
                world,
                Material.air,
                box.minX,
                box.maxX + 1,
     ...
     * up to but not including the max boundary
     */
    public static int getNumBlocksOfMaterialInArea(World world, Material material, int minX, int maxX, int minY,
     ...
     * @param meta   This should already be rotated to the correct value, to avoid processing every single call to
     *               setBlock
     */
    public static void rotatedFillWithBlocks(World world, int x, int z, int minX, int maxX, int minY, int maxY,
        int minZ, int maxZ, int facing, Block block, int meta) {
        for (int i = minX; i <= maxX; ++i) {
            for (int j = minY; j <= maxY; ++j) {
                for (int k = minZ; k <= maxZ; ++k) {
     ...
     */
    public static boolean isRotatedAreaClear(World world, int x, int z, int frontX, int backX, int minY, int maxY,
        int leftZ, int rightZ, int facing) {
        ...
        List<Entity> entities = world
            .getEntitiesWithinAABB(Entity.class, AxisAlignedBB.getBoundingBox(minX, minY, minZ, maxX, maxY, maxZ));
