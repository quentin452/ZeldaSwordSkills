...
public class LeapingBlow extends SkillActive {
    ...
    public boolean canUse(EntityPlayer player) {
        return super.canUse(player) && !isActive()
            && PlayerUtils.isSword(player.getHeldItem())
            && !TargetUtils.isInLiquid(player);
    }
    ...
    public void onImpact(EntityPlayer player, float distance) {
        SwordBasic swordSkill = (SwordBasic) ZSSPlayerSkills.get(player)
        ...
        if (isActive() && swordSkill != null && swordSkill.isActive() && PlayerUtils.isSword(player.getHeldItem())) {
            if (player.worldObj.isRemote) {
                ...
                Entity entity = new EntityLeapingBlow(player.worldObj, player).setDamage(getDamage(player))
                    .setLevel(level, isMaster);
