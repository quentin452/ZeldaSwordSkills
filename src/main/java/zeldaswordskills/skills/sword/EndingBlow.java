...
public class EndingBlow extends SkillActive {
    ...
    public boolean canUse(EntityPlayer player) {
        if (!isActive() && super.canUse(player) && PlayerUtils.isWeapon(player.getHeldItem())) {
            ICombo combo = ZSSPlayerSkills.get(player)
                .getComboSkill();
            ILockOnTarget lock = ZSSPlayerSkills.get(player)
                .getTargetingSkill();
            if (combo != null && combo.isComboInProgress()
                && lock != null
                && lock.getCurrentTarget() == combo.getCombo()
                    .getLastEntityHit()) {
                if (lastNumHits > 0) {
                    return combo.getCombo()
                        .getConsecutiveHits() > 1
                        && combo.getCombo()
                            .getNumHits() > lastNumHits + 2;
                } else {
                    return combo.getCombo()
                        .getConsecutiveHits() > 1;
                }
    ...
    protected boolean onActivated(World world, EntityPlayer player) {
        ...
        ICombo skill = ZSSPlayerSkills.get(player)
            .getComboSkill();
        if (skill.getCombo() != null) {
            lastNumHits = skill.getCombo()
                .getNumHits();
        }
        if (world.isRemote) { // only attack after server has been activated, i.e. client receives activation packet
                              // back
            ZSSClientEvents.performComboAttack(
        ...
        if (isActive()) {
            ...
            if (activeTimer == 0 && !player.worldObj.isRemote) {
                ...
                ZSSEntityInfo.get(player)
                    .applyBuff(Buff.DEFENSE_DOWN, getDuration() * 2, 50);
            }
        }
    }
    ...
    private void updateEntityState(EntityPlayer player) {
        if (!player.worldObj.isRemote) {
            if (entityHit.getHealth() <= 0.0F) {
                ...
                WorldUtils.playSoundAtEntity(player, Sounds.GRUNT, 0.3F, 0.8F);
                ZSSEntityInfo.get(player)
                    .applyBuff(Buff.DEFENSE_DOWN, getDuration(), 50);
            }
        }
        entityHit = null;
        xp = 0;
    ...
    public float postImpact(EntityPlayer player, EntityLivingBase entity, float amount) {
        ...
        if (combo != null && combo.isComboInProgress()
            ...
            entityHit = entity;
            xp = level + 1
                + player.worldObj.rand.nextInt(Math.max(2, MathHelper.ceiling_float_int(entity.getHealth())));
        }
        return amount;
    }
}
