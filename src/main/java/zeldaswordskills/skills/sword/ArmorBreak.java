...
public class ArmorBreak extends SkillActive {
    ...
    public boolean isCharging(EntityPlayer player) {
        ILockOnTarget target = ZSSPlayerSkills.get(player)
            .getTargetingSkill();
    ...
    public boolean isKeyPressed() {
        return (ZSSKeyHandler.keys[ZSSKeyHandler.KEY_ATTACK].getIsKeyPressed()
            || (Config.allowVanillaControls && Minecraft.getMinecraft().gameSettings.keyBindAttack.getIsKeyPressed()));
    }
            ...
            if (isKeyPressed() && PlayerUtils.isWeapon(player.getHeldItem())) {
                if (!player.isSwingInProgress) {
                    ...
                    if (charge < maxCharge - 1) {
                        ZSSPlayerInfo.get(player).armSwing = 0.25F
                            + 0.75F * ((float) (maxCharge - charge) / (float) maxCharge);
                    }
                    --charge;
                }
                ...
                if (charge == 0) {
                    ...
                    if (requiresReset) { // activated by vanilla attack key: manually unset the key state (fix for mouse
                                         // event issues)
                        KeyBinding
                            .setKeyBindState(Minecraft.getMinecraft().gameSettings.keyBindAttack.getKeyCode(), false);
                    }
                    SwordBasic skill = (SwordBasic) ZSSPlayerSkills.get(player)
                        .getPlayerSkill(swordBasic);
