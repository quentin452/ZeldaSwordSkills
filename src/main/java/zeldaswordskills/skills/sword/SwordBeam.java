...
public class SwordBeam extends SkillActive {
    ...
    public void addInformation(List<String> desc, EntityPlayer player) {
        ...
        desc.add(
            StatCollector.translateToLocalFormatted(
                getInfoString("info", 1),
                String.format("%.1f", Config.getHealthAllowance(level) / 2.0F)));
        desc.add(
            StatCollector.translateToLocalFormatted(getInfoString("info", 2), String.format("%.2f", getMagicCost())));
    }
    ...
    private float getDamage(EntityPlayer player) {
        return (float) ((double) (getDamageFactor(player)) * 0.01D
    ...
    protected boolean onActivated(World world, EntityPlayer player) {
        ...
        if (!world.isRemote) {
            ...
            beam.setPosition(
                beam.posX + vec3.xCoord * 0.5F,
                beam.posY + vec3.yCoord * 0.5F,
                beam.posZ + vec3.zCoord * 0.5F);
