...
public class BackSlice extends SkillActive {
     ...
     * @return chance that the armor stack will be knocked off of the damaged entity, a value between 0.0F and 1.0F
     *         inclusive
     */
    public static float getDisarmorChance(ItemStack armorStack, ItemStack weapon, int level) {
        ...
        if (armorStack != null && armorStack.getItem() instanceof ItemArmor) {
            ...
            int i = armor.getArmorMaterial()
                .getDamageReductionAmount(armor.armorType);
    ...
    public boolean isKeyListener(Minecraft mc, KeyBinding key) {
        if (isActive()) {
            return (key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_ATTACK]
                || (Config.allowVanillaControls && key == mc.gameSettings.keyBindAttack));
        }
        return key == mc.gameSettings.keyBindForward || key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_LEFT]
            || key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_RIGHT]
            || ((Config.allowVanillaControls
                && (key == mc.gameSettings.keyBindLeft || key == mc.gameSettings.keyBindRight)));
    }
    ...
    public boolean keyPressed(Minecraft mc, KeyBinding key, EntityPlayer player) {
        if (canExecute(player)) {
            ...
            && (key == mc.gameSettings.keyBindAttack || key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_ATTACK])) {
                // Attack targeted entity directly rather than using mouse cursor object due to camera funkiness
                Entity target = ZSSPlayerSkills.get(player)
                    .getTargetingSkill()
                    .getCurrentTarget();
    ...
    public boolean onActivated(World world, EntityPlayer player) {
        ...
        targetingSkill = ZSSPlayerSkills.get(player)
            .getActiveSkill(swordBasic);
        return isActive();
    }
    ...
    public boolean onRenderTick(EntityPlayer player, float partialTickTime) {
        if (player.onGround) {
            ...
            double speed = 1.0D + 10.0D * (player.getAttributeMap()
                .getAttributeInstance(SharedMonsterAttributes.movementSpeed)
                .getAttributeValue() - Dash.BASE_MOVE);
            if (speed > 1.0D) {
                speed = 1.0D;
            }
            ...
            if (keyPressed == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_RIGHT]
    ...
    public boolean onBeingAttacked(EntityPlayer player, DamageSource source) {
        ILockOnTarget targeting = ZSSPlayerSkills.get(player)
            .getTargetingSkill();
        ...
        if (isActive() && dodgeTimer <= (getActiveTime() - 5)) { // can strike any time after 5 ticks have passed
            ...
            if (targeting != null && targeting.getCurrentTarget() == entity) {
                if (!TargetUtils.isTargetInFrontOf(entity, player, getAttackAngle())) {
                    ...
                    // chance of knocking off an armor piece only if extra damage was allowed or IEntityBackside allows
                    // it
                    if (flag && (!isIBackEntity || ((IEntityBackslice) entity).allowDisarmorment(player, amount))) {
                        ...
                        if (armor != null && player.worldObj.rand.nextFloat()
                            < getDisarmorChance(armor, player.getHeldItem(), level)) {
                            WorldUtils
                                .spawnItemWithRandom(entity.worldObj, armor, entity.posX, entity.posY, entity.posZ);
