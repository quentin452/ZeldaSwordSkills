...
public class SpinAttack extends SkillActive {
    ...
    public void addInformation(List<String> desc, EntityPlayer player) {
        byte temp = level;
        if (!isActive()) {
            superLevel = (checkHealth(player) ? ZSSPlayerSkills.get(player)
                .getSkillLevel(superSpinAttack) : 0);
            level = ZSSPlayerSkills.get(player)
                .getSkillLevel(spinAttack);
        }
        desc.add(getChargeDisplay(getChargeTime()));
        desc.add(getRangeDisplay(getRange()));
        desc.add(
            StatCollector.translateToLocalFormatted(getInfoString("info", 1).replace("super", ""), superLevel + 1));
        desc.add(getExhaustionDisplay(getExhaustion()));
        if (this.getId() == superSpinAttack.getId()) { // player's skill is a different instance
            desc.add(
                StatCollector
                    .translateToLocalFormatted(getInfoString("info", 2), String.format("%.2f", getMagicCost())));
        }
        level = temp;
    }
    ...
    private boolean canRefresh(EntityPlayer player) {
        ...
        if (isBombos && ZSSPlayerInfo.get(player)
            .getCurrentMagic() < (10.0F + cost)) {
            isBombos = false;
        }
        if (ZSSPlayerInfo.get(player)
            .getCurrentMagic() < cost) {
            return false;
        }
    ...
    private boolean checkHealth(EntityPlayer player) {
        return player.capabilities.isCreativeMode
            || PlayerUtils.getHealthMissing(player) <= Config.getHealthAllowance(level);
    }
    ...
    /**
     * Returns true if either left or right arrow key is currently being pressed (or both in the case of vanilla
     * controls)
     */
    @SideOnly(Side.CLIENT)
    private boolean isKeyPressed() {
        return ZSSKeyHandler.keys[ZSSKeyHandler.KEY_LEFT].getIsKeyPressed()
            || ZSSKeyHandler.keys[ZSSKeyHandler.KEY_RIGHT].getIsKeyPressed()
    ...
    public boolean isKeyListener(Minecraft mc, KeyBinding key) {
        ...
        (Config.allowVanillaControls && (key == mc.gameSettings.keyBindLeft || key == mc.gameSettings.keyBindRight))
            || key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_LEFT]
            || key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_RIGHT]);
    }
    ...
    protected boolean onActivated(World world, EntityPlayer player) {
        ...
        if (PlayerUtils.isHoldingMasterSword(player)
            && PlayerUtils.hasItem(player, ZSSItems.medallion, BlockAncientTablet.EnumType.BOMBOS.ordinal())) {
            isBombos = ZSSPlayerInfo.get(player)
                .useMagic(10.0F);
        }
    ...
    public void onUpdate(EntityPlayer player) {
        ...
        if (isCharging()) { // check isRemote before accessing @client stuff anyway, just in case charge somehow set on
                            // server
    ...
    private void startSpin(World world, EntityPlayer player) {
        ...
        if (world.isRemote) {
            targets = world
                .getEntitiesWithinAABB(EntityLivingBase.class, player.boundingBox.expand(getRange(), 0.0D, getRange()));
    ...
    private void incrementSpin(EntityPlayer player) {
        ...
        // 0.24D results in a perfect circle per spin, at all levels, taking 21 ticks to complete at level 1, and 15 at
        // level 10
