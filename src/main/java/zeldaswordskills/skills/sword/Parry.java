...
public class Parry extends SkillActive {
    ...
    public void addInformation(List<String> desc, EntityPlayer player) {
        desc.add(
            StatCollector
                .translateToLocalFormatted(getInfoString("info", 1), (int) (getDisarmChance(player, null) * 100)));
        desc.add(
    ...
    public static float getDisarmModifier(EntityLivingBase defender, EntityLivingBase attacker) {
        ...
        if (defender instanceof EntityPlayer) {
            modifier += 0.1F * ZSSPlayerSkills.get((EntityPlayer) defender)
                .getSkillLevel(parry);
        }
        ...
        if (attacker instanceof EntityPlayer) {
            modifier -= Config.getDisarmPenalty() * ZSSPlayerSkills.get((EntityPlayer) attacker)
                .getSkillLevel(parry);
        }
    ...
    public boolean onBeingAttacked(EntityPlayer player, DamageSource source) {
        if (source.getEntity() instanceof EntityLivingBase) {
            ...
            if (attacksParried < getMaxParries() && parryTimer > getParryDelay()
                && attacker.getHeldItem() != null
