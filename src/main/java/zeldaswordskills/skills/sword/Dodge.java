...
public class Dodge extends SkillActive {
    ...
    public void addInformation(List<String> desc, EntityPlayer player) {
        desc.add(
            StatCollector
                .translateToLocalFormatted(getInfoString("info", 1), (int) (getBaseDodgeChance(player) * 100)));
        desc.add(StatCollector.translateToLocalFormatted(getInfoString("info", 2), (getDodgeTime() + level - 5) * 2)); // don't
                                                                                                                       // use
                                                                                                                       // real
                                                                                                                       // time
                                                                                                                       // bonus,
    ...
    private float getBaseDodgeChance(EntityPlayer player) {
        float evadeUp = ZSSEntityInfo.get(player)
            .getBuffAmplifier(Buff.EVADE_UP) * 0.01F;
        float evadeDown = ZSSEntityInfo.get(player)
            .getBuffAmplifier(Buff.EVADE_DOWN) * 0.01F;
        float speedBonus = 2.0F * (float) (player.getAttributeMap()
    ...
    public boolean isKeyListener(Minecraft mc, KeyBinding key) {
        return ((Config.allowVanillaControls
    ...
    public boolean keyPressed(Minecraft mc, KeyBinding key, EntityPlayer player) {
        if (canExecute(player)) {
            ...
            } else if (key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_LEFT]
                || key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_RIGHT]) {
                    PacketDispatcher.sendToServer(new ActivateSkillPacket(this));
                    return true;
                }
