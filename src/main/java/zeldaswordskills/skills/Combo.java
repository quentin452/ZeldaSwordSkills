...
public class Combo {
    ...
    public List<Float> getDamageList() {
        return Collections.unmodifiableList(damageList);
    }
    ...
    public void add(EntityPlayer player, Entity target, float damage) {
        if (getNumHits() < maxComboSize && (comboTimer > 0 || getNumHits() == 0)) {
            ...
            switch (damageList.size()) {
                case 3:
                    player.triggerAchievement(ZSSAchievements.comboBasic);
                    break;
                case 8:
                    player.triggerAchievement(ZSSAchievements.comboPerfect);
                    break;
                case 12:
                    player.triggerAchievement(ZSSAchievements.comboLegend);
                    break;
            }
            comboDamage += damage;
            if (player instanceof EntityPlayerMP) {
    ...
    public static final Combo readFromNBT(NBTTagCompound compound) {
        Combo combo = new Combo(
            compound.getByte("SkillID"),
            compound.getInteger("MaxSize"),
            compound.getInteger("TimeLimit"));
