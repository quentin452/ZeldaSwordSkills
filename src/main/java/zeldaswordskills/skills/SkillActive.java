...
public abstract class SkillActive extends SkillBase {
    ...
    public boolean canUse(EntityPlayer player) {
        return (level > 0 && (player.capabilities.isCreativeMode || player.getFoodStats()
            .getFoodLevel() > 0));
    }
        ...
        if (isActive()) {
            ...
            if (isActive()) {
                ZSSMain.logger.error(
                    getDisplayName()
                        + " is still active after onDeactivated called - this may result in SEVERE errors or even crashes!!!");
        ...
        if (canUse(player)) {
            ...
            if (level > 0) {
                PlayerUtils.sendTranslatedChat(
                    player,
                    "chat.zss.skill.use.fail",
                    new ChatComponentTranslation(getTranslationString()));
            }
            return false;
        }
    }
