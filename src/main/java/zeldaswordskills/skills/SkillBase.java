...
public abstract class SkillBase {
    ...
    protected SkillBase(String name, boolean register) {
        ...
        if (register) {
            if (skillsMap.containsKey(id)) {
                ZSSMain.logger.warn(
                    "CONFLICT @ skill " + id
                        + " id already occupied by "
                        + skillsMap.get(id).unlocalizedName
                        + " while adding "
                        + name);
            }
    ...
    public static final SkillBase getNewSkillInstance(byte id) {
        return (skillsMap.containsKey(id) ? skillsMap.get(id)
            .newInstance() : null);
    }
    ...
    public String getDurationDisplay(int duration, boolean inTicks) {
        return StatCollector.translateToLocalFormatted(
            ...
            (inTicks ? StatCollector.translateToLocal("skill.zss.ticks")
    ...
    /**
     * Called each time a skill's level increases; responsible for everything OTHER than increasing the skill's level:
     * applying any bonuses, handling Xp, etc.
     */
    protected abstract void levelUp(EntityPlayer player);

    /**
     * Recalculates bonuses, etc. upon player respawn; Override if levelUp does things other than just calculate
     * bonuses!
     */
    ...
    /**
     * Attempts to level up the skill to target level, returning true if skill's level increased (not necessarily to the
     * target level)
     */
