...
public abstract class RoomBase {
    ...
    protected void placePedestal(World world, int offsetY) {
        StructureGenUtils
            .setBlockAtPosition(world, bBox, bBox.getXSize() / 2, offsetY, bBox.getZSize() / 2, ZSSBlocks.pedestal, 0);
    ...
    public RoomBase(int chunkX, int chunkZ, int size, int maxHeight, Block blockRequired) {
        ...
        this.bBox = new StructureBoundingBox(
            1,
            1,
            1,
            Math.max(size, 3),
            MathHelper.clamp_int(size, 3, maxHeight),
            Math.max(size, 3));
    }
                ...
                for (int k = bBox.minZ; k <= bBox.maxZ; ++k) {
                    ...
                    if (!canReplaceBlockAt(j, block)) {
                        ...
                        if (block == ZSSBlocks.secretStone
                            || (Config.avoidModBlocks() && Block.getIdFromBlock(block) > 255)) {
                            return false;
    ...
    protected boolean isWellHidden(World world) {
        ...
        if (inOcean) {
            ...
            // StructureGenUtils.getNumBlocksOfMaterialInArea(world, bBox, Material.water, 1) < (getArea() / (difficulty
            // + 1));
        }
        Material material = (!inNether && StructureGenUtils.getNumBlocksOfMaterial(world, bBox, Material.water, 1) > 0
            ? Material.water
            : Material.air);
        ...
        if (inNether) {
            return world.rand.nextFloat() < (0.35F - (difficulty * 0.1F)) || ((difficulty != 3 || (above + below) < 4)
                && StructureGenUtils.getNumBlocksOfMaterialInArea(world, bBox, material, 1)
                    < (getArea() / (difficulty + 1))); // above < (5 - difficulty) &&
        }
    ...
    protected boolean canReplaceBlockAt(int y, Block block) {
        ...
        return (replaceBlocks.contains(block) || flag1
            || flag2
            || (block.getMaterial()
                .isLiquid() && y < (bBox.maxY - 2)));
    }
    ...
    protected void generateAir(World world) {
        if (!inOcean) {
            StructureGenUtils.fillWithBlocks(
                ...
                1,
                bBox.getXSize() - 1,
                (submerged ? (inLava || isLocked ? 2 : 3) : 1),
                bBox.getYSize() - 1,
                1,
                bBox.getZSize() - 1,
                Blocks.air,
    ...
    protected void genSubmerged(World world) {
        if (submerged && bBox.getXSize() > 3) {
            ...
            StructureGenUtils
                .fillWithBlocks(world, bBox, 1, bBox.getXSize() - 1, 1, fillTo, 1, bBox.getZSize() - 1, block, 0);
        }
    }
    ...
    protected boolean placeInOcean(World world, boolean sink) {
        ...
        boolean flag = world.getBiomeGenForCoords(x, z).biomeName.toLowerCase()
            .contains("ocean");
        if (flag && !inLava
            && world.getBlock(x, bBox.maxY, z)
                .getMaterial() == Material.water) {
            int count = 0;
            while (bBox.minY > 16 && count < 8
                && world.getBlock(x, bBox.minY, z)
                    .getMaterial() == Material.water) {
                bBox.offset(0, -1, 0);
                ++count;
            }

            if (world.getBlock(x, bBox.minY, z)
                .getMaterial() != Material.water) {
                inOcean = true;
    ...
    protected boolean placeInNether(World world) {
        while (bBox.minY > 8 && world.getBlock(bBox.getCenterX(), bBox.minY, bBox.getCenterZ())
