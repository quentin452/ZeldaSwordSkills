...
public class MapGenBossRoom extends ZSSMapGenBase {
    ...
    public void generate(IChunkProvider provider, World world, Random rand, int chunkX, int chunkZ) {
        ...
        if (world.villageCollectionObj != null
            && world.villageCollectionObj.findNearestVillage(posX, 64, posZ, 8 * Config.getMinBossDistance()) != null) {
            return;
        }
        ...
        if (type != null) {
            ...
            if (rand.nextFloat() < 0.2F && !areStructuresWithinRange(room, Config.getMinBossDistance())) {
                ...
                if (room.generate(this, world, rand, posX, posY, posZ)) {
                    // LogHelper.finer("Boss room of type " + type.toString() + " successfully generated at " +
                    // room.getBoundingBox().toString());
                    onStructureGenerated(world, room);
                } else {
                    // LogHelper.finest("Boss room of type " + type.toString() + " failed to generate at " +
                    // room.getBoundingBox().toString());
                }
            }
        }
    ...
    public boolean shouldDenyLakeAt(int chunkX, int chunkZ) {
        return (isRoomInChunk(chunkX, chunkZ) || isRoomInChunk(chunkX + 1, chunkZ + 1)
            || isRoomInChunk(chunkX + 1, chunkZ - 1)
            || isRoomInChunk(chunkX - 1, chunkZ + 1)
            || isRoomInChunk(chunkX - 1, chunkZ - 1)
            || isRoomInChunk(chunkX, chunkZ + 1)
            || isRoomInChunk(chunkX, chunkZ - 1)
            ...
            for (int j = room.chunkZ - range; j <= room.chunkZ + range; ++j) {
                if (structureMap.containsKey(Long.valueOf(ChunkCoordIntPair.chunkXZ2Int(i, j)))) {
                    ...
                    if (type != null && (Config.areBossDungeonsRandom() || type.ordinal() == getBossTypeFor(i, j))) {
                        // LogHelper.finer("Boss room of same type found within " + range + " chunks of " + room.chunkX
                        // + "/" + room.chunkZ);
                        return true;
                    } else if (((room.chunkX - i) * (room.chunkX - i) + (room.chunkZ - j) * (room.chunkZ - j))
                        < (range * range) / 2) {
                            // LogHelper.finer("Boss room of different type found within " + ((range * range) / 2) + "
                            // chunks squared of " + room.chunkX + "/" + room.chunkZ);
                            return true;
                        }
                }
            }
    ...
    protected void onStructureGenerated(World world, RoomBoss room) {
        AntiqueAtlasHelper.placeCustomTile(
            world,
            ModInfo.ATLAS_DUNGEON_ID + room.getBossType()
                .ordinal(),
            room.chunkX,
            room.chunkZ);
        structureMap.put(
            Long.valueOf(ChunkCoordIntPair.chunkXZ2Int(room.chunkX, room.chunkZ)),
            room.getBossType()
                .ordinal());
