...
public class RoomSecret extends RoomBase {
    ...
    public boolean generate(ZSSMapGenBase mapGen, World world, Random rand, int x, int y, int z) {
        ...
        int range = (inOcean ? Config.getMinOceanDistance()
            : inNether ? Config.getNetherMinDistance() : Config.getMinLandDistance());
    ...
    private void checkSpecialCases(World world, Random rand) {
        ...
        } else if (flag && (biome.biomeName.toLowerCase()
            .contains("hill")
    ...
    private boolean generateChestContents(World world, Random rand, int x, int y, int z, boolean first) {
        ...
        if (bBox.isVecInside(i1, j1, k1) && !StructureGenUtils.isBlockChest(world, i1, j1, k1)) {
            Block chestBlock = (rand.nextFloat() < Config.getLockedChestChance() ? ZSSBlocks.chestLocked
                : Blocks.chest);
            ...
            if (te instanceof IInventory) {
                ...
                if ((first || chestBlock == ZSSBlocks.chestInvisible)
                    && rand.nextFloat() < Config.getHeartPieceChance()) {
                    WorldUtils.addItemToInventoryAtRandom(rand, new ItemStack(ZSSItems.heartPiece), chest, 3);
                }
                if (door != null) {
                    ItemStack loot = ChestGenHooks.getInfo(DungeonLootLists.BOSS_LOOT)
                        .getOneItem(rand);
                        ...
                        if (door == ZSSBlocks.pegWooden) {
                            ...
                            if ((rand.nextInt() & 1) > 0) { // roughly 50% chance
                                loot = new ItemStack(
                                    ZSSItems.instrument,
                                    1,
                                    ItemInstrument.Instrument.OCARINA_TIME.ordinal());
                            } else {
            ...
            if (world.getBlock(i1, j1 - 1, k1)
                .getMaterial() == Material.lava && bBox.getYSize() > 3) {
