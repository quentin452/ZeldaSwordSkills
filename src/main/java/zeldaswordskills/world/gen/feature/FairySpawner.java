...
public class FairySpawner {
        ...
        if (fairiesSpawned < maxFairies && world.rand.nextFloat() < (world.isDaytime() ? 0.01F : 0.2F)) {
            int nearby = world
                .getEntitiesWithinAABB(
                    EntityFairy.class,
                    AxisAlignedBB.getBoundingBox(x, box.getCenterY(), z, x + 1, box.getCenterY() + 1, z + 1)
            ...
            if (nearby < 4) {
                ...
                if (++fairiesSpawned == maxFairies) {
                    nextResetDate = world.getTotalWorldTime()
                        + 24000 * (world.rand.nextInt(Config.getDaysToRespawn()) + 1);
                }
            }
        }
            ...
            if (player != null) {
                ...
                for (EntityItem item : list) {
                    ...
                    if (stack.getItem() == Items.emerald) {
                        ...
                        item.setDead();
                    } else if (stack.getItem() instanceof IFairyUpgrade
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        nextResetDate = (data.getTag("nextResetDate")
            .getId() == Constants.NBT.TAG_LONG ? data.getLong("nextResetDate") : 0);
