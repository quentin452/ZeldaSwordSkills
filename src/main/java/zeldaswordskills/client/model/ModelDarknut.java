...
public class ModelDarknut extends ModelBase implements IModelBiped {
    ...
    private void setLivingAnimations(EntityDarknut entity, float par2, float par3, float partialTick) {
        ...
        if (entity.isSpinning()) {
            ...
            int isLeft = (i > 0 ? 1 : -1);
            i = Math.abs(i);
            rightArm.rotateAngleX = MathHelper
                .clamp_float(-1.0F + 3.5F * getSwingAmount((float) i - partialTick, 10.0F), -4.5F, 1.5F);
            rightArm.rotateAngleY = 0.0F;
            rightArm.rotateAngleZ = isLeft
                * (attackRotZ[(i + 1) % 9] + (partialTick * (attackRotZ[i % 9] - attackRotZ[(i + 1) % 9])));
