...
public abstract class ModelDekuBase extends ModelBase {
    ...
    public ModelDekuBase() {
        ...
        DEATH_ANIMATION = new ImmutableList.Builder<IAnimation>()
            ...
            .add(
                new AnimationTargetAngle(mouth_base_lower, RotationAxis.X, -30.0F, 0, 12, true)
                    .setAllowInversion(false))
            .add(
                new AnimationTargetAngle(mouth_base_lower, RotationAxis.X, -60.0F, 12, 15, true)
                    .setAllowInversion(false))
            .add(
                new AnimationTargetAngle(mouth_base_upper, RotationAxis.X, -60.0F, 0, 12, true)
                    .setAllowInversion(false))
            .add(
                new AnimationTargetAngle(mouth_base_upper, RotationAxis.X, -40.0F, 12, 15, true)
                    .setAllowInversion(false))
            .add(
    ...
    /**
     * Called from {@link #setLivingAnimations(EntityLivingBase, float, float, float) setLivingAnimations} after
     * resetting the pose
     */
    ...
    protected boolean applyDeathAnimation(EntityDekuBase entity, float par2, float par3, float partialTick) {
        ...
        if (animation != null) {
            IAnimation.Helper
                .applyAnimation(animation, entity.deathTime, partialTick, 1.0F, 1.0F, 0.0F, entity.custom_death < 0);
        }
