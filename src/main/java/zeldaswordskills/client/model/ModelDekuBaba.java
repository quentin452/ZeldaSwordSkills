...
public class ModelDekuBaba extends ModelDekuBase {
    ...
    public ModelDekuBaba() {
        super();
        ATTACK_ANIMATION = new ImmutableList.Builder<IAnimation>().add(
            new AnimationTargetAngle(stem1, RotationAxis.X, -35.0F, 0, 3, true).setProgressType(IProgressType.SQUARED))
            .add(
                new AnimationTargetAngle(stem1, RotationAxis.X, 45.0F, 3, 7, true)
                    .setProgressType(IProgressType.SQUARED))
            ...
            .add(
                new AnimationTargetAngle(stem2, RotationAxis.X, 45.0F, 0, 3, true)
                    .setProgressType(IProgressType.SQUARED))
            .add(
                new AnimationTargetAngle(stem2, RotationAxis.X, 20.0F, 3, 7, true)
                    .setProgressType(IProgressType.SQUARED))
            .add(new AnimationTargetAngle(stem2, RotationAxis.X, 20.0F, 12, 15, true).setAllowOffset(false))
            .add(
                new AnimationTargetAngle(stem3, RotationAxis.X, 50.0F, 0, 3, true)
                    .setProgressType(IProgressType.SQUARED))
            .add(
                new AnimationTargetAngle(stem3, RotationAxis.X, 15.0F, 3, 7, true)
                    .setProgressType(IProgressType.SQUARED))
            ...
            .add(
                new AnimationTargetAngle(head_base, RotationAxis.X, 50.0F, 0, 3, true)
                    .setProgressType(IProgressType.SQUARED))
            .add(
                new AnimationTargetAngle(head_base, RotationAxis.X, 15.0F, 3, 7, true)
                    .setProgressType(IProgressType.SQUARED))
            ...
            .add(
                new AnimationTargetAngle(mouth_base_lower, RotationAxis.X, -45.0F, 0, 5, true).setAllowInversion(false))
            .add(
                new AnimationTargetAngle(mouth_base_lower, RotationAxis.X, -90.0F, 7, 9, true).setAllowInversion(false))
            .add(
                new AnimationTargetAngle(mouth_base_lower, RotationAxis.X, -60.0F, 12, 15, true)
                    .setAllowInversion(false)
                    .setAllowOffset(false))
            .add(new AnimationTargetAngle(mouth_lower, RotationAxis.X, 0.0F, 7, 9, true).setAllowInversion(false))
            .add(
                new AnimationTargetAngle(mouth_lower, RotationAxis.X, -25.0F, 12, 15, true).setAllowInversion(false)
                    .setAllowOffset(false))
            .add(
                new AnimationTargetAngle(mouth_base_upper, RotationAxis.X, -55.0F, 0, 5, true).setAllowInversion(false))
            .add(new AnimationTargetAngle(mouth_base_upper, RotationAxis.X, 0.0F, 7, 9, true).setAllowInversion(false))
            .add(
                new AnimationTargetAngle(mouth_base_upper, RotationAxis.X, -30.0F, 12, 15, true)
                    .setAllowInversion(false)
                    .setAllowOffset(false))
            .add(new AnimationTargetAngle(mouth_upper, RotationAxis.X, -90.0F, 7, 9, true).setAllowInversion(false))
            .add(
                new AnimationTargetAngle(mouth_upper, RotationAxis.X, -70.0F, 12, 15, true).setAllowInversion(false)
                    .setAllowOffset(false))
            .build();
        LOOPING_ANIMATIONS = new ImmutableList.Builder<IAnimation>()
            ...
            .add(
                new AnimationCircle(
                    ...
                    tongue_tip))
            .add(
                new AnimationWave.AnimationWaveSin(
                    RotationAxis.X,
                    0.1F,
                    0.15F,
                    0.15F,
        ...
        SPROUT_ANIMATION = new ImmutableList.Builder<IAnimation>()
            ...
            .add(
                new AnimationTargetOffset(stem1, RotationAxis.Y, 0.0F, 1, EntityDekuBaba.ACTION_SPROUT.duration)
                    .setAllowInversion(false))
    ...
    protected void applyAnimations(EntityDekuBaba entity, float par2, float par3, float partialTick) {
        ...
        if (entity.isFullyAlert()) {
            ...
            IAnimation.Helper.applyAnimation(
                LOOPING_ANIMATIONS,
                entity.ticksExisted + entity.getTicksExistedOffset(-1),
                partialTick,
                speed,
                factor,
                offset,
    ...
    private void animateActions(EntityDekuBaba entity, float par2, float par3, float partialTick) {
        ...
        for (EntityAction action : actions) {
            ...
            if (frame > 0) {
                ...
                float offset = 0.0F;
                if (target != null
                    && (action == EntityDekuBaba.ACTION_ATTACK || action == EntityDekuBaba.ACTION_BOMB)) {
                    offset = 0.075F * MathHelper
                        .clamp_float((float) (entity.boundingBox.maxY - target.boundingBox.maxY), -2F, 1.5F);
                }
                IAnimation.Helper
                    .applyAnimation(getAnimation(action), frame, partialTick, speed, 1.0F, offset, entity.isConfused());
            }
        }
    }
