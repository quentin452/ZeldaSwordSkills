...
public class ComboOverlay extends AbstractGuiOverlay {
    ...
    public boolean shouldRender() {
        ...
        ICombo iCombo = ZSSPlayerSkills.get(mc.thePlayer)
            .getComboSkill();
    ...
    protected void setup(ScaledResolution resolution) {
        ...
        int n = Math.min(
            this.combo.getDamageList()
                .size(),
            Config.hitsToDisplay);
        this.height = (n + 3) * this.mc.fontRenderer.FONT_HEIGHT;
        this.label = (this.combo.isFinished()
            ? StatCollector.translateToLocalFormatted("combo.finished", this.combo.getLabel())
            : StatCollector.translateToLocalFormatted("combo.combo", this.combo.getLabel()));
        this.comboSize = StatCollector
            .translateToLocalFormatted("combo.size", this.combo.getNumHits(), this.combo.getMaxNumHits());
        this.comboDamage = StatCollector
            .translateToLocalFormatted("combo.damage", String.format("%.1f", this.combo.getDamage()));
        this.width = Math.max(
            this.mc.fontRenderer.getStringWidth(this.label),
            this.mc.fontRenderer.getStringWidth(this.comboDamage));
    ...
    protected void render(ScaledResolution resolution) {
        ...
        this.mc.fontRenderer
            .drawString(this.comboDamage, xPos + this.getOffset(this.comboDamage), yPos, 0xFFFFFF, true);
    ...
    private int getOffset(String s) {
        switch (this.getHorizontalAlignment()) {
            ...
            case RIGHT:
                return this.getWidth() - this.mc.fontRenderer.getStringWidth(s) - DEFAULT_PADDING;
            default:
                return 0;
        }
    }
