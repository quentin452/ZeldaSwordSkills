...
public class GuiEditGossipStone extends GuiScreen {
    ...
    protected void keyTyped(char c, int keyCode) {
        if (keyCode == Keyboard.KEY_ESCAPE) {
            ...
            && message.length() < TileEntityGossipStone.MAX_MESSAGE_LENGTH) {
                message.append(c);
            }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float f) {
        ...
        drawCenteredString(
            fontRendererObj,
            StatCollector.translateToLocal("gui.zss.gossip_stone.name"),
            width / 2,
            40,
            16777215);
        ...
        for (int i = 0; i < lines.length; ++i) {
            ...
            fontRendererObj.drawString(
                lines[i],
                (width / 2) - (fontRendererObj.getStringWidth(lines[i]) / 2),
                80 + i * fontRendererObj.FONT_HEIGHT,
                16777215);
        }
