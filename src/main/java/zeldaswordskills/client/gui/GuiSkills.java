...
public class GuiSkills extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        String s = (currentSkill != null ? currentSkill.getDisplayName()
            .toUpperCase() : StatCollector.translateToLocal("skill.zss.gui.description"));
    ...
    private void refreshDescription() {
        ...
        String[] temp = (currentSkill != null ? currentSkill.getFullDescription()
            .split("\\\\n")
            : StatCollector.translateToLocal("skill.zss.gui.explanation")
                .split("\\\\n"));
        for (String s : temp) {
    ...
    protected void mouseMovedOrUp(int mouseX, int mouseY, int which) {
        ...
        if (slot != null && slot.getStack() != null) {
            int id = (slot.getStack()
                .getItemDamage() % SkillBase.getNumSkills());
            if (currentSkill == null || currentSkill.getId() != id) {
                scrollY = 0.0F;
                // clear the current description so it refreshes next time the screen draws
                desc.clear();
            }
            currentSkill = ZSSPlayerSkills.get(mc.thePlayer)
                .getPlayerSkill((byte) id);
        }
    }
    ...
    protected void keyTyped(char c, int key) {
        if (key == 1 || key == mc.gameSettings.keyBindInventory.getKeyCode()
            || key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_SKILLS_GUI].getKeyCode()) {
