...
public class GuiLearnSong extends GuiMusicBase {
    ...
    public GuiLearnSong(int x, int y, int z) {
        ...
        if (songToLearn == null || songToLearn.getNotes() == null
            || songToLearn.getNotes()
                .isEmpty()) {
            throw new IllegalArgumentException(
                "ZSSPlayerSongs#songToLearn may not be null or empty when opening the Song Learning GUI!");
        }
    }
    ...
    public void initGui() {
        super.initGui();
        if (ZSSPlayerSongs.get(mc.thePlayer)
            .isSongKnown(songToLearn)) {
    ...
    public void drawScreen(int mouseX, int mouseY, float f) {
        ...
        if (!demoPlayed && song == null) {
            ...
            fontRendererObj
                .drawString(s, guiLeft + (xSize / 2) - (fontRendererObj.getStringWidth(s) / 2), guiTop + 3, 0xFFFFFF);
        }
    }
    ...
    public void updateScreen() {
        if (!demoPlayed) {
            if (++ticksSinceLastNote == 20) {
                if (currentNoteIndex == songToLearn.getNotes()
                    .size()) {
    ...
    protected void onNoteAdded() {
        int i = melody.size() - 1;
        if (melody.get(i) != songToLearn.getNotes()
            .get(i)) {
