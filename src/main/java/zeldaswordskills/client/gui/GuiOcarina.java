...
public class GuiOcarina extends GuiMusicBase {
    ...
    public GuiOcarina(int x, int y, int z, boolean isScarecrow) {
        super(x, y, z);
        if (isScarecrow) {
            scarecrowNotes = ZSSPlayerSongs.get(mc.thePlayer)
                .getScarecrowNotes();
    ...
    public void onGuiClosed() {
        if (song != null) {
            if (ticksSinceLastNote > song.getMinDuration()
                || (scarecrowNotes != null && currentNoteIndex == scarecrowNotes.size())) {
                if (scarecrowFirst || (scarecrowNotes != null && !ZSSPlayerSongs.get(mc.thePlayer)
                    .isSongKnown(ZeldaSongs.songScarecrow))) {
    ...
    protected void onNoteAdded() {
        ...
        if (scarecrowNotes != null) {
            ...
            song = ZSSPlayerSongs.get(mc.thePlayer)
                .getKnownSongFromNotes(melody);
