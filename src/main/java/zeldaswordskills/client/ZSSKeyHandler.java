...
public class ZSSKeyHandler {
    ...
    public static final String[] desc = { "activate", "next", "attack", "left", "right", "down", "block", "bomb",
        "toggleat", "togglebuff", "skills_gui", "prev_mode", "next_mode" };

    /** Default key values */
    private static final int[] keyValues = { Keyboard.KEY_X, // Activate lockon
        Keyboard.KEY_TAB, // Next target
        Keyboard.KEY_UP, // Attack
        Keyboard.KEY_LEFT, Keyboard.KEY_RIGHT, Keyboard.KEY_DOWN, Keyboard.KEY_RCONTROL, // Block / use item
        ...
        if (mc.inGameHasFocus && mc.thePlayer != null) {
            ...
            if (kb == keys[KEY_SKILL_ACTIVATE].getKeyCode()) {
                ...
                if (mc.thePlayer.isSneaking()) {
                    ChatComponentTranslation desc = new ChatComponentTranslation(
                        Config.toggleTargetPlayers() ? "chat.zss.key.enable" : "chat.zss.key.disable");
                    PlayerUtils.sendTranslatedChat(mc.thePlayer, "chat.zss.key.toggletp", desc);
                } else {
                    ChatComponentTranslation desc = new ChatComponentTranslation(
                        Config.toggleAutoTarget() ? "chat.zss.key.enable" : "chat.zss.key.disable");
                ...
                if (mc.thePlayer.isSneaking()) {
                    ...
                    ChatComponentTranslation desc = new ChatComponentTranslation(
                        Config.isComboHudEnabled ? "chat.zss.key.enable" : "chat.zss.key.disable");
    ...
    private static void handleTargetingKeys(Minecraft mc, int kb, ZSSPlayerSkills skills) {
        ...
        boolean canInteract = skills.canInteract() && !ZSSEntityInfo.get(mc.thePlayer)
        ...
        if (kb == keys[KEY_NEXT_TARGET].getKeyCode()) {
            ...
            Item heldItem = (mc.thePlayer.getHeldItem() != null ? mc.thePlayer.getHeldItem()
                .getItem() : null);
            boolean flag = (heldItem instanceof ItemHeldBlock
            ...
            if (canInteract && !flag) {
                ...
                if (skills.isSkillActive(SkillBase.spinAttack)) {
                    skills.getActiveSkill(SkillBase.spinAttack)
                        .keyPressed(mc, key, mc.thePlayer);
                    return;
