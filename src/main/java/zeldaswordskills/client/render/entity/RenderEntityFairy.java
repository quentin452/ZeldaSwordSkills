    ...
    protected boolean shouldRenderNametag(EntityLiving entity) {
        return Minecraft.isGuiEnabled() && entity != renderManager.livingPlayer
            ...
            && (entity.getAlwaysRenderNameTagForRender()
                || entity.hasCustomNameTag() && entity == renderManager.field_147941_i);
    }

    protected void renderNameTag(EntityLiving entity, double x, double y, double z) {
        int range = (int) (entity.isSneaking() ? RendererLivingEntity.NAME_TAG_RANGE_SNEAK
            : RendererLivingEntity.NAME_TAG_RANGE);
        String name = entity.func_145748_c_()
            .getFormattedText();
