...
public class RenderEntityBoomerang extends Render {
    ...
    public void renderBoomerang(EntityBoomerang entity, double x, double y, double z, float yaw, float partialTick) {
        ...
        if (boomerang != null) {
            ...
            GL11.glRotatef(
                entity.rotationYaw + (entity.prevRotationYaw - entity.rotationYaw) * partialTick - 60.0F,
                0.0F,
                1.0F,
                0.0F);
            GL11.glRotatef(
                Math.abs(entity.rotationPitch + (entity.prevRotationPitch - entity.rotationPitch)) * partialTick
                    - rotation,
                0.0F,
                0.0F,
                1.0F);
            GL11.glTranslatef(-0.5F, -0.5F, 0.0F);
            IIcon icon = boomerang.getItem()
                .getIconFromDamage(0);
            bindTexture(
                renderManager.renderEngine.getResourceLocation(
                    boomerang.getItem()
                        .getSpriteNumber()));
