    ...
    public void processCommand(ICommandSender sender, String[] args) {
        ...
        if (args.length == 2 && ("all").equals(args[1])) {
            ...
            if (flag) {
                ...
                if (commandSender != player) {
                    PlayerUtils.sendTranslatedChat(
                        commandSender,
                        "commands.grantskill.success.all",
                        player.getCommandSenderName());
                }
            } else {
                PlayerUtils.sendTranslatedChat(
                    commandSender,
                    "commands.grantskill.success.partial",
                    player.getCommandSenderName());
            }
            ...
            if (level > oldLevel) { // grants skill up to level or max level, whichever is reached first
                 ...
                 * throw new CommandException("commands.grantskill.failure.disabled", new
                 * ChatComponentTranslation(skill.getTranslationString()));
                 * } else if (skills.grantSkill(skill.getId(), (byte) level)) {
                 */
                if (skills.grantSkill(skill.getId(), (byte) level)) {
                    PlayerUtils.sendTranslatedChat(
                        player,
                        "commands.grantskill.notify.one",
                        new ChatComponentTranslation(skill.getTranslationString()),
                        skills.getSkillLevel(skill));
                    if (commandSender != player) {
                        PlayerUtils.sendTranslatedChat(
                            commandSender,
                            "commands.grantskill.success.one",
                            player.getCommandSenderName(),
                            new ChatComponentTranslation(skill.getTranslationString()),
                            skills.getSkillLevel(skill));
                    }
                    ...
                    throw new CommandException(
                        "commands.grantskill.failure.player",
                        player.getCommandSenderName(),
                        new ChatComponentTranslation(skill.getTranslationString()));
                }
            } else {
                throw new CommandException(
                    "commands.grantskill.failure.low",
                    player.getCommandSenderName(),
                    new ChatComponentTranslation(skill.getTranslationString()),
                    oldLevel);
            }
