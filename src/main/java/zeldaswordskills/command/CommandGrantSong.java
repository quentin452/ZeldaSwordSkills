        ...
        if (args.length != 2 && args.length != 3) {
            ...
            if (!flag) {
                PlayerUtils.sendTranslatedChat(
                    commandSender,
                    "commands.grantsong.success.partial",
                    player.getCommandSenderName());
            } else if (commandSender != player) {
                PlayerUtils
                    .sendTranslatedChat(commandSender, "commands.grantsong.success.all", player.getCommandSenderName());
            }
        } else {
            ...
            if (song == null) {
                ...
                throw new CommandException(
                    "commands.grantsong.failure.denied",
                ...
                PlayerUtils.sendTranslatedChat(
                    commandSender,
                    "commands.grantsong.success.one",
                    player.getCommandSenderName(),
                    new ChatComponentTranslation(song.getTranslationString()));
            } else {
                PlayerUtils.sendTranslatedChat(
                    commandSender,
                    "commands.grantsong.failure.player",
                    player.getCommandSenderName(),
                    new ChatComponentTranslation(song.getTranslationString()));
            }
        }
        ...
        switch (args.length) {
            ...
            case 2:
                return getListOfStringsMatchingLastWord(
                    args,
                    SongAPI.getRegisteredNames()
                        .toArray(new String[SongAPI.getTotalSongs()]));
            case 3:
                return getListOfStringsMatchingLastWord(args, new String[] { "true" });
            default:
                return null;
        }
    }
