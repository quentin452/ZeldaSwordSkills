...
public class CommandSetMagic extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        if (args.length != 4 || (!("add").equals(args[1]) && !("set").equals(args[1]))
            || (!("current").equals(args[2]) && !("max").equals(args[2]))) {
        ...
        int mp = CommandBase.parseInt(commandSender, args[3]);
        if (mp < 0 && !add) {
        ...
        PlayerUtils
            .sendTranslatedChat(commandSender, "commands.zssmagic.success", player.getCommandSenderName(), args[2], mp);
    }
    ...
    public List addTabCompletionOptions(ICommandSender sender, String[] args) {
        switch (args.length) {
            case 1:
                return CommandBase.getListOfStringsMatchingLastWord(args, getPlayers());
            case 2:
