...
public class ContainerPedestal extends AbstractContainer {
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slotIndex) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotIndex < INV_START) {
                ...
                && !mergeItemStack(itemstack1, INV_START, INV_END + 1, false)) {
                    return null;
                }
    ...
    public ItemStack slotClick(int slot, int par2, int par3, EntityPlayer player) {
        if (slot < INV_START && pedestal.getWorldObj()
            .getBlockMetadata(pedestal.xCoord, pedestal.yCoord, pedestal.zCoord) == 0x8) {
            return null;
        } else {
